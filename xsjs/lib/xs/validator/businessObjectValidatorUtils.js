...
async function BusinessObjectValidatorUtils(sBusinessObject, oGenericSyntaxValidator) {
	 ...
	 */
    this.checkMandatoryProperties = async function (oEntity, aMandatoryProperites, sMode) {
    ...
    this.checkUserId = async function (oObject, operation, sObjectType, aResultErrors) {
    ...
    this.createMultipleValidationErrorsResponse = async function (errorMessageCode, operation, sObjectType, oObject, validationInfoCode, aResultErrors) {
        var oResult = {};
	 ...
	 */
    this.parseCheckProjectDetails = async function (oRequest, bEmptyArrayAllowed) {
    ...
    this.getMetadataAttributeKey = async function (oInput, sPropertyKey) {
        return [
            sPropertyKey,
            oInput.metadata[0].PATH,
            oInput.categoryId,
            oInput.subitemState
        ].toString();
    };
	...
	*/
    this.checkEntity = async function (oInput) {
        const that = this;
        ...
        _.each(oInput.entity, async function (oPropertyValue, sPropertyKey) {
            ...
            oValidatedEnity[sPropertyKey] = await genericSyntaxValidator.validateValue(oPropertyValue, aPropertyMetadata[0].SEMANTIC_DATA_TYPE, aPropertyMetadata[0].SEMANTIC_DATA_TYPE_ATTRIBUTES, false, aPropertyMetadata[0].VALIDATION_REGEX_VALUE);
        });
        ...
        oInput.metadata.forEach(oPropertyMetadata => {
            var bIsMandatory = _.some(oPropertyMetadata.ATTRIBUTES, function (oPropertyMetadataAtttribute) {
                return oPropertyMetadataAtttribute.ITEM_CATEGORY_ID === oInput.categoryId && oPropertyMetadataAtttribute.SUBITEM_STATE === oInput.subitemState && oPropertyMetadataAtttribute.IS_MANDATORY === 1;
            });
            if (bIsMandatory === true) {
                aManadatoryProperties.push(oPropertyMetadata.COLUMN_ID);
            }
        });
        await this.checkMandatoryProperties(oValidatedEnity, aManadatoryProperties, oInput.mandatoryPropertyCheckMode);
    ...
    this.checkNonTemporaryMasterdataReferencesForItems = (oEntity, aMasterdataProperties, oValidValuesSet, businessObjectType) => {
        ...
        aMasterdataProperties.forEach(sMasterdataProperty => {
            ...
            if (mdValue !== undefined && mdValue !== null && !oValidValuesSet.has(mdValue)) {
                aErrors.push({
                    itemId: oEntity.ITEM_ID,
                    businessObjectType: businessObjectType,
                    columnId: sMasterdataProperty
                });
            }
        });
        return aErrors;
    ...
    this.extendMetadataCustomFields = async function (oMetaData) {
    ...
    this.getUnitFields = async function (oMetaData, propertyType) {
        var aCustomFields = _.filter(oMetaData, function (oPropertyMetadata) {
    ...
    this.getCustomUoMFields = async function (oMetaData) {
    ...
    this.getCustomCurrencyFields = async function (oMetaData) {
...
export default {_,helpers,GenericSyntaxValidator,MessageLibrary,PlcException,Code,ValidationInfoCode,MessageDetails,sIsManualRegex,sUserIdRegex,iUserIdMaxLen,logError,BusinessObjectValidatorUtils};

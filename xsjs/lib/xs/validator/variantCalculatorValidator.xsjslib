...
function VariantCalculatorValidator(oPersistency, oMetadataProvider, oUtils) {
    this.validate = function validate(oRequest, mValidatedParameters) {
        ...
        function checkSumVariant(oSumVariant){
        ...
        function transposeCompressedItemsToTableType(oItemValues, iVariantId) {
            ...
            if (!_.isArray(oItemValues.ITEM_ID)) {
                const oMessageDetails = new MessageDetails().addVariantObjs({
                    id: iVariantId,
                });
        ...
        function checkItemAttributesLength(aRequestItemsCompressed) {
        ...
        function validatePostRequest() {
            ...
            let aVariantItemMetadata
                = oMetadataProvider.get(BusinessObjectTypes.VariantItem, BusinessObjectTypes.VariantItem, null, null, oPersistency);
            const aSyntacticallyCorrectVariants = [];
            const aSyntacticallyCorrectVariantItems = [];
            aVariants.forEach((oVariant) => {
                checkVariantHasItems(oVariant);
                aVariantMetadata = changeMandatoryMetadataAttribute(aVariantMetadata, ["VARIANT_NAME","REPORT_CURRENCY_ID"], 0);
                ...
                const oSyntacticallyCorrectVariantExpanded
                    = _.extend(getVariantTableTypeTemplate(iCalculationVersionId), oSyntacticallyCorrectVariant);
        ...
        validateSumRequest = () => {
            ...
            checkSumVariant(oSumVariant);

            let aVariantMetadata = oMetadataProvider.get(BusinessObjectTypes.Variant, BusinessObjectTypes.Variant, null, null, oPersistency);
            const aSyntacticallyCorrectVariants = [];
            oSumVariant.VARIANTS.forEach((oVariant) => {
                aVariantMetadata = changeMandatoryMetadataAttribute(aVariantMetadata, ["VARIANT_ID"], 1);
                aVariantMetadata = changeMandatoryMetadataAttribute(aVariantMetadata, ["EXCHANGE_RATE_TYPE_ID", "REPORT_CURRENCY_ID", "VARIANT_NAME"], 0);
                oSyntacticallyCorrectVariant = checkVariant(oVariant, aVariantMetadata);
        ...
        switch (oRequest.method) {
            case $.net.http.POST:
                return oRequest.queryPath.indexOf('sum') !== -1 ? validateSumRequest() : validatePostRequest();
            case $.net.http.PUT:

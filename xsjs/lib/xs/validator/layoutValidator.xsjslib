...
function LayoutValidator(oPersistency, utils) {	
	...
	this.validate = function(oRequest, mValidatedParameters) {
		...
		function validatePostRequest() {
			...
			var aCreateValidProperties = ["LAYOUT_ID", "IS_CURRENT", "LAYOUT_NAME", "LAYOUT_COLUMNS", "HIDDEN_FIELDS", "LAYOUT_TYPE"];
				...
				if(oLayout.LAYOUT_COLUMNS.length>0) {
    				_.each(oLayout.LAYOUT_COLUMNS, function(oLayoutColumn) {						
    					...
    					if(!helpers.isNullOrUndefined(oLayoutColumn.COLUMN_ID)) {
    						...
    						utils.checkMandatoryProperties(oLayoutColumn, ["DISPLAY_ORDER", "PATH", "BUSINESS_OBJECT", "COLUMN_ID"]);
    						var validProps = ["DISPLAY_ORDER", "COLUMN_WIDTH", "PATH", "BUSINESS_OBJECT", "COLUMN_ID"]
    						...
    						utils.checkMandatoryProperties(oLayoutColumn, ["DISPLAY_ORDER", "COSTING_SHEET_ROW_ID"]);
    						utils.checkInvalidProperties(oLayoutColumn, ["DISPLAY_ORDER", "COLUMN_WIDTH", "COSTING_SHEET_ROW_ID"]);
    					} else if(!helpers.isNullOrUndefined(oLayoutColumn.COST_COMPONENT_ID)) {
    						// component split column
    						utils.checkMandatoryProperties(oLayoutColumn, ["DISPLAY_ORDER", "COST_COMPONENT_ID"]);
    						utils.checkInvalidProperties(oLayoutColumn, ["DISPLAY_ORDER", "COLUMN_WIDTH", "COST_COMPONENT_ID"]);
    					} else if (helpers.isNullOrUndefined(oLayoutColumn.COLUMN_ID)) {
    						// first column of calculation view only has DISPLAY_ORDER and COLUMN_WIDTH; all other fields are NULL
    						utils.checkMandatoryProperties(oLayoutColumn, ["DISPLAY_ORDER"]);
    						utils.checkInvalidProperties(oLayoutColumn, ["DISPLAY_ORDER", "COLUMN_WIDTH"]);
    					}
    				});
    				//check if oField can be found in oCurrentMetadata, i.e. if there is valid metadata for the column defined in the layout
    				...
    				_.each(aColumnsMetadataFields, oField => {
                    	var bFieldExists = _.some(oCurrentMetadata, oDbMetadata => {
                    		return  oField.PATH.toUpperCase() === oDbMetadata.PATH.toUpperCase() &&
                    			    oField.BUSINESS_OBJECT .toUpperCase() === oDbMetadata.BUSINESS_OBJECT.toUpperCase() &&
                    			    oField.COLUMN_ID.toUpperCase() === oDbMetadata.COLUMN_ID.toUpperCase();
                    	});
                    	if(!bFieldExists){
			...
			if (!helpers.isNullOrUndefined(oLayout.HIDDEN_FIELDS)) {
				...
				if(oLayout.HIDDEN_FIELDS.length>0) {
			    	_.each(oLayout.HIDDEN_FIELDS, function(oHiddenField) {
				    	utils.checkMandatoryProperties(oHiddenField, ["PATH", "BUSINESS_OBJECT", "COLUMN_ID"]);
				    	utils.checkInvalidProperties(oHiddenField, ["PATH", "BUSINESS_OBJECT", "COLUMN_ID"]);
				    });
		...
		function validatePutRequest() {
			...
			var aUpdateValidProperties = ["LAYOUT_ID", "IS_CURRENT", "LAYOUT_NAME", "LAYOUT_COLUMNS", "HIDDEN_FIELDS", "LAYOUT_TYPE"];

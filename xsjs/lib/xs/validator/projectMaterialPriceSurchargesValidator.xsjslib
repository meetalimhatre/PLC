...
function ProjectMaterialPriceSurchargesValidator(oPersistency, utils) {
	...
	const aSurchargeMandatoryProperties = [ "PLANT_ID", "ACCOUNT_GROUP_ID", "MATERIAL_GROUP_ID", "MATERIAL_TYPE_ID", "PERIOD_VALUES", "MATERIAL_ID" ];
	const aSurchargeValidProperties = [ "PLANT_ID", "PLANT_DESCRIPTION", "ACCOUNT_GROUP_ID", "ACCOUNT_GROUP_DESCRIPTION", "MATERIAL_GROUP_ID", "MATERIAL_GROUP_DESCRIPTION", 
										"MATERIAL_TYPE_ID", "MATERIAL_TYPE_DESCRIPTION", "PERIOD_VALUES", "MATERIAL_ID", "MATERIAL_DESCRIPTION" ];
	// mandatory and valid properties for surcharge values
	const aValueMandatoryProperties = [ "LIFECYCLE_PERIOD_FROM", "VALUE" ];
	const aValueValidProperties = [ "LIFECYCLE_PERIOD_FROM", "LIFECYCLE_PERIOD_FROM_DATE", "VALUE" ];
		...
		function validateUpdateRequest() {
			...
			_.each(aProjectMaterialPriceSurcharges, function(oProjectSurcharges) {
				utils.checkArrayProperties(oProjectSurcharges, aSurchargeMandatoryProperties, aSurchargeValidProperties, 
						{
							PLANT_ID: 			["String", "length=8;", true],
							ACCOUNT_GROUP_ID: 	["Integer", undefined, true], 
							MATERIAL_GROUP_ID: 	["String", "length=9;", true],
							MATERIAL_TYPE_ID:	["String", "length=4;", true],
							MATERIAL_ID:	    ["String", "length=40;", true],
						});
				...
				_.each(oProjectSurcharges.PERIOD_VALUES, function(oSurchargeValue) {
					utils.checkArrayProperties(oSurchargeValue, aValueMandatoryProperties, aValueValidProperties, 
							{
								LIFECYCLE_PERIOD_FROM: 	["Integer", undefined, true],
								VALUE: 					["Decimal", "precision=28; scale=7;", true],
							});
			...
			let aPropertiesToBeChecked = ['PLANT_ID', 'ACCOUNT_GROUP_ID', 'MATERIAL_GROUP_ID', 'MATERIAL_TYPE_ID', 'MATERIAL_ID'];
			let oProjectMasterdata = helpers.prepareSurchargesMasterdataValuesForValidation(aPropertiesToBeChecked, aProjectMaterialPriceSurcharges);
...
ProjectMaterialPriceSurchargesValidator.prototype = Object.create(ProjectMaterialPriceSurchargesValidator.prototype);

...
function VariantValidator(oPersistency, oMetadataProvider, oUtils) {
    this.validate = function validate(oRequest, mValidatedParameters) {
        ...
        function checkVariant(oVariant, aVariantMetadata) {
            ...
            const oVariantToValidate = _.omit(oVariant, ["ITEMS", "CHANGES_ACCEPTED"]);
            ...
            if (!helpers.isNullOrUndefined(bChangesAccepted)) {
                ...
                oSyntacticallyCorrectVariant.CHANGES_ACCEPTED = genericSyntaxValidator.validateValue(bChangesAccepted, "BooleanInt", null, false);  //eslint-disable-line
            }
        ...
        function validateUpdateOrderRequest(oBody) {
            ...
            oBody.forEach((oVariantId) => {
                oUtils.checkMandatoryProperties(oVariantId, ["VARIANT_ID", "LAST_MODIFIED_ON"]);
                oUtils.checkInvalidProperties(oVariantId, ["VARIANT_ID", "LAST_MODIFIED_ON"]);

                const oValidatedVariant = {};
                oValidatedVariant.VARIANT_ID = genericSyntaxValidator.validateValue(oVariantId.VARIANT_ID, "PositiveInteger", null, true);
                oValidatedVariant.LAST_MODIFIED_ON = genericSyntaxValidator.validateValue(oVariantId.LAST_MODIFIED_ON, "UTCTimestamp", null, true);
            ...
            if (aVariantMetadata) {
                ...
                aFieldsNotMandatoryForPatch.forEach((sField) => {
                    if (_.isUndefined(oBody[sField])) {
                        aVariantMetadata = changeMandatoryMetadataAttribute(aVariantMetadata, sField, 0);
                    }
                });
                aVariantMetadata = changeMandatoryMetadataAttribute(aVariantMetadata, "LAST_MODIFIED_ON", 1);
            }

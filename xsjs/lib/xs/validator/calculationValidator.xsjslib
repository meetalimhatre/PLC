...
async function CalculationValidator(oPersistency, sSessionId, metadataProvider, utils) {
    ...
    const cvValidator = await new CalculationVersionValidator(oPersistency, sSessionId, metadataProvider, utils);
    ...
    this.validate = async function (oRequest, mValidatedParameters) {
        ...
        async function validateDeleteRequest() {
            ...
            _.each(aCalcs, function (oCalc) {
                ...
                var oValidatedCalc = {};
                oValidatedCalc.CALCULATION_ID = await genericSyntaxValidator.validateValue(oCalc.CALCULATION_ID, 'PositiveInteger', undefined, true);
        ...
        async function validateUpdateRequest() {
            ...
            _.each(aCalcs, function (oCalc) {
                aValidatedCalcs.push(utils.checkEntity({
                    ...
                    categoryId: -1,
                    subitemState: -1,
                    metadata: metadataProvider.get(BusinessObjectTypes.Calculation, BusinessObjectTypes.Calculation, null, null, oPersistency, $.getPlcUsername(), $.getPlcUsername())
                }));
            });
            return aValidatedCalcs;
        }
        ...
        async function validateCreateRequest() {
            ...
            _.each(aCalcs, function (oCalc) {
                ...
                var oExistingNonTemporaryMasterdata = oPersistency.CalculationVersion.getExistingNonTemporaryMasterdata({ project_id: oCalc.PROJECT_ID });
        ...
        async function validateCopyVersionRequest() {
            ...
            oValidatedProject.PROJECT_ID = await genericSyntaxValidator.validateValue(oProject.PROJECT_ID, 'String', undefined, true);
...
export default {_,helpers,BusinessObjectTypes,GenericSyntaxValidator,CalculationVersionValidator,ItemValidator,MessageLibrary,MessageDetails,PlcException,Code,CalculationValidator};

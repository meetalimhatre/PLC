...
var getCachedOldItemsInUpdateValidation = function(aBodyItems, sessionId) {
	...
	if (!helpers.isNullOrUndefined(sessionId) && (sessionId in oCachedOldItemsInUpdateValidation)) {
		...
		if (aBodyItems.length === oCachedItems.aBodyItems.length
			&& aBodyItems[0].CALCULATION_VERSION_ID === oCachedItems.aBodyItems[0].CALCULATION_VERSION_ID) {
			let convert = function (item) { return item.ITEM_ID; }
...
let setCachedOldItemsInUpdateValidation = function(aBodyItems, sessionId, aOldItems) {
...
let clearCachedOldItemsInUpdateValidation = function(sessionId) {
	if (!helpers.isNullOrUndefined(sessionId)) {
...
function ItemValidator($, persistency, sessionId, userName, metadataProvider, utils) {
	...
	var aMandatoryPropertiesStatic = [ "ITEM_ID", "CALCULATION_VERSION_ID" ];
	...
	this.validate = function(oRequest, mValidatedParameters) {
		...
		function checkURLParameters() {
			if(helpers.isNullOrUndefined(mValidatedParameters.getPrices)) {
				...
				genericSyntaxValidator.validateValue(mValidatedParameters.getPrices, "Boolean", undefined, true);
			}
			...
			if(helpers.isNullOrUndefined(mValidatedParameters.calculation_version_id)) {
				...
				genericSyntaxValidator.validateValue(mValidatedParameters.calculation_version_id, "PositiveInteger", undefined, true);
			}
		...
		function checkCalculationVersion(aBodyItems) {
			...
			if (aItemsRefDifferentCv.length > 0) {
				...
				const sServerMsg = `${sClientMsg} Items: ${_.map(aItemsRefDifferentCv, function(oItem) { return oItem.ITEM_ID; }).toString()}.`;
		...
		function checkReferencedCalculationVersion(aBodyItems) {
			_.each(aBodyItems, function(oBodyItem) {
				if (_.has(oBodyItem, "ITEM_CATEGORY_ID") && genericSyntaxValidator.validateValue(oBodyItem.ITEM_CATEGORY_ID,
						"PositiveInteger", undefined, true) === constants.ItemCategory.ReferencedVersion) {
					...
					if (oBodyItem.CALCULATION_VERSION_ID === iReferencedVersionId) {
						const sClientMsg = `The selected calculation version cannot be referenced because this would lead to a circular dependency. `
					...
					authorizationManager.checkPrivilege(authorizationManager.BusinessObjectTypes.CalculationVersion, iReferencedVersionId,
						authorizationManager.Privileges.READ,persistency.getConnection(), userName);
				}
			});
		}
		 ...
		 *            aBodyItems - An array containing JS objects for every item entity that shall be updated.
		 */
		function checkControllingAreaOfReferencedCalculationVersion(aBodyItems) {
		...
		function validateDeleteRequests() {
			...
			_.each(aBodyItems, function(oBodyItem, iIndex) {
				...
				oValidatedItem.CALCULATION_VERSION_ID = genericSyntaxValidator.validateValue(oBodyItem.CALCULATION_VERSION_ID,
						"PositiveInteger", undefined, true);
		...
		function validateCreateRequest() {
			...
			checkControllingAreaOfReferencedCalculationVersion(aBodyItems);
			return validateItemsCreate(aBodyItems, oItemTree.itemTree);
		}
		...
		function validateUpdateRequest() {
			...
			checkCalculationVersion(aBodyItems);
			checkReferencedCalculationVersion(aBodyItems);
			return validateItemsUpdate(aBodyItems);
		}
		...
		function validateItemStructure(aBodyItems, oItemTree) {
			...
			if (aBodyItems.length > 1) {
				if (mValidatedParameters[ServiceParameters.mode.name] !== ServiceParameters.mode.values.normal) {
					...
					if (!(_.has(mValidatedParameters, ServiceParameters.mode.name) 
							&& ( mValidatedParameters[ServiceParameters.mode.name] === ServiceParameters.mode.values.replace 
									|| mValidatedParameters[ServiceParameters.mode.name] === ServiceParameters.mode.values.updateMasterDataAndPrices 
									|| mValidatedParameters[ServiceParameters.mode.name] === ServiceParameters.mode.values.noUpdateMasterDataAndPrices ))) {
		...
		function compareWithItemToReplace(oNewItem) {
			...
			if ((helpers.isNullOrUndefined(oNewItem.PARENT_ITEM_ID) !== helpers.isNullOrUndefined(oExistingRootItem.PARENT_ITEM_ID))
					|| helpers.isNullOrUndefined(oNewItem.PREDECESSOR_ITEM_ID) !== helpers
							.isNullOrUndefined(oExistingRootItem.PREDECESSOR_ITEM_ID)) {
				$.trace.error(sServerMsg);
				throw new PlcException(Code.GENERAL_VALIDATION_ERROR, sClientMsg);				
			}
		 ...
		 */
		function getParentItemIds(aBodyItems, iCalculationVersionId) {
			const oParentItemIds = new Set(persistency.Item.getParentItemIds(iCalculationVersionId, sessionId));
			aBodyItems.forEach(oItem => {
		...
		function createItemsTree(aBodyItems){
		    ...
		    var bModeNotReplace = mValidatedParameters[ServiceParameters.mode.name] === ServiceParameters.mode.values.updateMasterDataAndPrices 
				|| mValidatedParameters[ServiceParameters.mode.name] === ServiceParameters.mode.values.noUpdateMasterDataAndPrices;
		...
		function validateItemsUpdate(aBodyItems) {
			var aValidatedItems = [];
			...
			const oValidItemCategoryIds = persistency.Item.getItemCategories();
			...
			const mDbItems = new Map(aOldItems.map(oItem => [oItem.ITEM_ID, oItem])); 
			...
			const oParentItemIds = getParentItemIds(aBodyItems, iCvId);
			...
			_.each(aBodyItems, function(oBodyItem) {
				...
				if (oDbItem.ITEM_CATEGORY_ID === constants.ItemCategory.CalculationVersion && _.has(oBodyItem, "ITEM_CATEGORY_ID")
						&& oBodyItem.ITEM_CATEGORY_ID !== constants.ItemCategory.CalculationVersion) {
				...
				var iCategoryId = _.has(oBodyItem, "ITEM_CATEGORY_ID") ? genericSyntaxValidator.validateValue(oBodyItem.ITEM_CATEGORY_ID,
						"PositiveInteger", undefined, true) : oDbItem.ITEM_CATEGORY_ID;

				//check child item category
				var iChildCategoryId = _.has(oBodyItem, "CHILD_ITEM_CATEGORY_ID") ? genericSyntaxValidator.validateValue(oBodyItem.CHILD_ITEM_CATEGORY_ID,
					"PositiveInteger", undefined, true) : constants.ItemCategory.CalculationVersion;
		...
		function validateItemsCreate(aBodyItems, mItems) {
			var aValidatedItems = [];		
			var oMetaData = metadataProvider.get(BusinessObjectTypes.Item, BusinessObjectTypes.Item, null, null, persistency, sessionId, userName);
			var oMetaDataCF = utils.extendMetadataCustomFields(oMetaData);
			...
			const oDbParentItemIds = new Set(persistency.Item.getParentItemIds(iCvId, sessionId));
			...
			aBodyItems.forEach((oBodyItem) => {
				...
				if (iCategoryId !== constants.ItemCategory.CalculationVersion 
						&& (oBodyItem.PARENT_ITEM_ID === null || oBodyItem.PARENT_ITEM_ID === undefined) ) {					
	...
	this.checkMasterdataReferences = function (aItems, oNonTemporaryMasterdata, oMetaData) {
		...
		const aPropertiesToCheckConfiguration = [
			...
			{
				propertiesToValidate: _.union(["QUANTITY_UOM_ID", "TOTAL_QUANTITY_UOM_ID", "PRICE_UNIT_UOM_ID", "QUANTITY_PER_BASE_UNIT_UOM_ID"], aCustomUoMs),
			...
			{
				propertiesToValidate: _.union(["TRANSACTION_CURRENCY_ID", "TARGET_COST_CURRENCY_ID"], aCustomCurrencies),

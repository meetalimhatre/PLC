    ...
    this.validate = function validate(oRequest) {
        function validatePostRequest() {
            ...
            if (!helpers.isNullOrUndefined(oRequestBody.TARGET_CALCULATION_ID)) {
                oRequestBody.TARGET_CALCULATION_ID = genericSyntaxValidator.validateValue(oRequestBody.TARGET_CALCULATION_ID, 'PositiveInteger', null, false);
            }
            if (!helpers.isNullOrUndefined(oRequestBody.CALCULATION_VERSION_NAME)) {
                oRequestBody.CALCULATION_VERSION_NAME = genericSyntaxValidator.validateValue(oRequestBody.CALCULATION_VERSION_NAME, 'String', 'length=500', false);
            }
            return oRequestBody;
        }

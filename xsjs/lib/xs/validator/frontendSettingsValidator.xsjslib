...
async function FrontendSettingsValidator(oPersistency, utils) {
    ...
    this.validate = async function (oRequest, mValidatedParameters) {
            ...
            if (!_.isArray(aFrontendSettings) || _.isEmpty(aFrontendSettings)) {
                ...
                _.each(aFrontendSettings, async function (oFrontendSetting) {
                    ...
                    if (oFrontendSetting.SETTING_TYPE === 'MassChange') {
                        try {
                            var aSettingContentDecoded = JSON.parse(await helpers.arrayBufferToString($.util.codec.decodeBase64(oFrontendSetting.SETTING_CONTENT)));
                        } catch (e) {
        ...
        async function validatePutRequest() {
            ...
            _.each(aFrontendSettingMassChangeIds, async function (oFrontendSettingId) {

                const oFrontendSettingContent = aFrontEndSettingsIdAndContent.get(oFrontendSettingId.SETTING_ID);
                try {
                    var aSettingContentDecoded = JSON.parse(await helpers.arrayBufferToString($.util.codec.decodeBase64(oFrontendSettingContent)));
                } catch (e) {

...
function UrlValidator(mResources) {
    ...
    this.validateUrl = function(oRequest, sResourceDefinitionKey) {
        ...
        // path variables and request parameters are both described using UrlParameterInfo objects and so, can be processed together
        // to do this, first a map of path variable and parameter name to the UrlParameterInfo object is created
        ...
        // second step is to parse the values of path variables out of the request url; the values are kept in key-value-pairs
        // temporarly (aRequestParameterTuples) to check for parameters duplication later
        ...
        sResourceDefinitionKey.split("/").forEach((sSegment, iIndex) => {
            ...
            if (bIsPathVariable) {
                // remove braces from variable definiton: {variable-name} => variable-name
                const sVariableName = sSegment.replace(/\{|\}/g, "");
                aRequestParameterTuples.push([sVariableName, aRequestSegments[iIndex]]);
            }
        });

        // collect values of request parameters in key-value-pairs in order to enable common processing
        _.each(oRequest.parameters, function(oParameter, iIndex) {
            var oRequestParameter = oRequest.parameters[iIndex];
            aRequestParameterTuples.push([oRequestParameter.name, oRequestParameter.value]);
        });
    ...
    function validateParameterValues(mRequestParameters, mAllowedParameters) {
        ...
        Array.from(mRequestParameters.keys()).forEach(sRequestParameterName => {
            ...
            // _.values must be used since validValues can also be an object or array; _.values is handling also object correctly
            if (!_.isEmpty(oParameterInfo.validValues) && _.values(oParameterInfo.validValues).indexOf(vValidatedValue) < 0) {
                const sLogMessage =
                    `The value ${vValidatedValue} is not valid for for parameter ${oParameterInfo.name}. Valid values are: ${oParameterInfo.validValues.toString()}`;
                    logError(sLogMessage);
                throw new PlcException(Code.GENERAL_VALIDATION_ERROR, sLogMessage);
            }

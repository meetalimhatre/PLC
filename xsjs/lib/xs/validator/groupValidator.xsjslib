...
async function GroupValidator(oPersistency, oMetadataProvider, utils) {


    //mandatory and valid properties for the object
    const aValidPropertiesOperation = [
        'CREATE',
        'DELETE',
        'UPDATE'
    ];
    const aValidPropertiesBatch = [
        'GROUPS',
        'MEMBERS',
        'SUBGROUPS'
    ];
    ...
    const aValidPropertiesGroup = [
        'GROUP_ID',
        'DESCRIPTION'
    ];
    const aMandatoryPropertiesSubgroup = [
        'GROUP_ID',
        'SUBGROUP_ID'
    ];
    const aMandatoryPropertiesMember = [
        'GROUP_ID',
        'USER_ID'
    ];
    var aUserMembers = [];
                ...
                if (_.has(oGroup.UPDATE, 'GROUPS')) {
                    if (!_.isArray(oGroup.UPDATE.GROUPS)) {
                        ...
                        oGroup.UPDATE.GROUPS.forEach(oGroup => {
                            try {
                                await checkPropertiesValues(oGroup);
                            } catch (e) {
        ...
        async function checkPropertiesValues(oEntity) {
            ...
            Object.keys(oGroup).forEach(key => {
                ...
                if (aPropertyMetadata[0]) {
                    await genericSyntaxValidator.validateValue(oGroup[key], aPropertyMetadata[0].SEMANTIC_DATA_TYPE, aPropertyMetadata[0].SEMANTIC_DATA_TYPE_ATTRIBUTES, false, aPropertyMetadata[0].VALIDATION_REGEX_VALUE);
                }
            });
        }
        ...
        async function checkMandatoryAndInvalidPropertiesForObjects(aObject, operation, oMandatoryProperties, oValidProperties, aResultErrors) {
            if (!_.isArray(aObject)) {
                ...
                _.each(aObject, async function (oObjectEntity) {
                    try {
                        await utils.checkMandatoryProperties(oObjectEntity, oMandatoryProperties);
                    } catch (e) {

...
async function AdministrationValidator(oPersistency, sSessionId, metadataProvider, utils) {
    ...
    async function validatePostRequest(oRequest, mValidatedParameters) {
        ...
        (await oPersistency.getConnection()).executeUpdate(`lock table "${ sMainTable }" in exclusive mode`);
        ...
        if (sBusinessObjectParameter === BusinessObjectTypes.WorkCenter) {
            ...
            (await oPersistency.getConnection()).executeUpdate(`lock table "${ sWorkCenterProcessTable }" in exclusive mode`);

            var sWorkCenterActivityTable = MasterdataResource[BusinessObjectTypes.WorkCenterActivity].dbobjects.plcTable;
            (await oPersistency.getConnection()).executeUpdate(`lock table "${ sWorkCenterActivityTable }" in exclusive mode`);
        }

        if (sBusinessObjectParameter === BusinessObjectTypes.AccountGroup || sBusinessObjectParameter === BusinessObjectTypes.ComponentSplit || sBusinessObjectParameter === BusinessObjectTypes.CostingSheet || sBusinessObjectParameter === BusinessObjectTypes.CostingSheetRow || sBusinessObjectParameter === BusinessObjectTypes.ExchangeRateType) {
        ...
        var oMasterDataProxy = await new MasterDataObjectHandlerProxy(await oPersistency.getConnection(), oPersistency.getHQueryPlc(), sBusinessObjectParameter, sIgnoreBadDataParameter);
    ...
    async function validateCostingSheetRowFormula(oRecord, isCreate) {
        if (isCreate) {
            ...
            if (!helpers.isNullOrUndefined(oRecord.FORMULA_ID) && await helpers.isNullOrUndefined(oRecord.FORMULA_STRING) && await helpers.isNullOrUndefined(oRecord.OVERHEAD_CUSTOM)) {
...
export default {_,helpers,XRegExp,Constants,BusinessObjectTypes,MasterDataObjectsAllowedReqBodyEntities,MessageLibrary,MessageDetails,ValidationInfoCode,PlcException,Code,BatchOperation,BusinessObjectsEntities,aNotMaintainableBusinessObjects,MasterDataObjectHandlerProxy,MasterdataResource,GenericSyntaxValidator,genericSyntaxValidator,trace,AdministrationValidator};

...
function CustomfieldsformulaValidator(persistency, sessionId, metadataProvider, utils) {
	...
	this.validate = function(oRequest, mValidatedParameters) {
		...
		var aSemanticDataTypes = ["String", "BooleanInt", "Integer", "Decimal", "LocalDate","Link"]; //Semantic Data Types		
		...
		var aRollupType = [0, 2, 3]; //for String, LocalDate and UTCTimestamp
		var aValidStandardFieldsForFormula = ["QUANTITY","PRICE_FIXED_PORTION","PRICE_VARIABLE_PORTION","PRICE_UNIT","TARGET_COST","LOT_SIZE","BASE_QUANTITY", "BOM_COMPARE_KEY"];
		...
		function validateBatchRequest() {
			...
			_.each(oBodyData, function(value, key){
				if(!helpers.isNullOrUndefined(mValidatedParameters) && !helpers.isNullOrUndefined(mValidatedParameters.checkCanExecute)){ 
				    if(mValidatedParameters.checkCanExecute === true && (key == 'CREATE' || key == 'UPDATE') && 
				            value.length != 0){
				...
				_.each(aCreatedFields, function(oBodyItem, iIndex) {
					...
					if(colRegexUnit.exec(oBodyItem.COLUMN_ID)) {
						if(helpers.isNullOrUndefined(oBodyItem.PROPERTY_TYPE) && oBodyItem.UOM_CURRENCY_FLAG === 1) {
							...
							oMessageDetails.validationObj = {"columnIds":[{"columnId" : "PROPERTY_TYPE"}], "validationInfoCode": ValidationInfoCode.MISSING_MANDATORY_ENTRY};
					...
					if (!(_.includes(aBusinessObjects,oBodyItem.BUSINESS_OBJECT))) {
						const oMessageDetails = new MessageDetails();
						oMessageDetails.addMetadataObjs(oBodyItem);
						oMessageDetails.validationObj = {"columnIds":[{"columnId" : "PATH"}, {"columnId" : "BUSINESS_OBJECT"}], "validationInfoCode": ValidationInfoCode.METADATA_ERROR};
					...
					if (!colRegex.exec(oBodyItem.COLUMN_ID)) {
						const oMessageDetails = new MessageDetails();
						oMessageDetails.addMetadataObjs(oBodyItem);
						oMessageDetails.validationObj = {"columnIds":[{"columnId" : "COLUMN_ID"}], "validationInfoCode": ValidationInfoCode.METADATA_ERROR};
					...
					if (!(_.includes(aSemanticDataTypes, oBodyItem.SEMANTIC_DATA_TYPE))) {
						const oMessageDetails = new MessageDetails();
						oMessageDetails.addMetadataObjs(oBodyItem);
						oMessageDetails.validationObj = {"columnIds":[{"columnId" : "SEMANTIC_DATA_TYPE"}] , "validationInfoCode": ValidationInfoCode.METADATA_ERROR};
					...
					if (!helpers.isNullOrUndefined(oBodyItem.REF_UOM_CURRENCY_COLUMN_ID) && oBodyItem.REF_UOM_CURRENCY_COLUMN_ID !== "") {
						...
						var checkRef = _.find(aCreatedFields, function(oItemUnit) {
							return oItemUnit.COLUMN_ID === oBodyItem.REF_UOM_CURRENCY_COLUMN_ID && oItemUnit.ROLLUP_TYPE_ID === oBodyItem.ROLLUP_TYPE_ID
							    && (oItemUnit.PROPERTY_TYPE === 6 || oItemUnit.PROPERTY_TYPE === 7);
						});
						if(!checkRef) {
							const oMessageDetails = new MessageDetails();
							oMessageDetails.addMetadataObjs(oBodyItem);
					...
					if(_.includes(constants.aCustomFieldMasterdataBusinessObjects, oBodyItem.BUSINESS_OBJECT) && oBodyItem.ROLLUP_TYPE_ID != 0) {
						const oMessageDetails = new MessageDetails();
						oMessageDetails.addMetadataObjs(oBodyItem);
						oMessageDetails.validationObj = {"columnIds":[{"columnId" : "ROLLUP_TYPE_ID"}], "validationInfoCode": ValidationInfoCode.METADATA_ERROR};
					...
					//SIDE_PANEL_GROUP_ID must be a valid id
					...
					if (!helpers.isNullOrUndefined(aFormulas) && _.isArray(aFormulas) && aFormulas.length > 0) {
						_.each(aFormulas, function(oFormula, iIndex) {
							if(_.includes(constants.aCustomFieldMasterdataBusinessObjects, oFormula.BUSINESS_OBJECT)) {
								...
								oMessageDetails.validationObj = {"columnIds":[{"columnId" : "FORMULA_ID"}], "validationInfoCode": ValidationInfoCode.METADATA_ERROR};
		...
		function validateUpdateCF(aUpdatedFields) { 
			...
			if (_.isArray(aUpdatedFields) && aUpdatedFields.length > 0) {				
				_.each(aUpdatedFields, function(oBodyItem, iIndex) {
					...
					//check that the updated rollup type id is defined for semantic data type
					if (!helpers.isNullOrUndefined(oBodyItem.ROLLUP_TYPE_ID) && !colRegexUnit.exec(oBodyItem.COLUMN_ID)) {
						oMetaData = metadataProvider.get(oBodyItem.PATH, oBodyItem.BUSINESS_OBJECT, oBodyItem.COLUMN_ID, null, persistency,
							sSessionId, sUserId);
					...
					//formulas
					// for standard fields check that the formula is changed for a valid field
		...
		function checkRollupType(iRollupType, iSemanticDataType) {
			// 0 - No Rollup, 1 - Sum, 2 - Min, 3 - Max, 4 - Avg, 5 - Count 
			genericSyntaxValidator.validateValue(iRollupType, "Integer", undefined, true);
			if (iRollupType < 0 || iRollupType > 5) {
				const oMessageDetails = new MessageDetails();
				oMessageDetails.validationObj = {"columnIds":[{"columnId" : "ROLLUP_TYPE"}], "validationInfoCode": ValidationInfoCode.METADATA_ERROR};
		...
		function checkAttribute(oAttribute, oBodyItem, oMetaData) {
			...
			/** run validation only if the value is not NULL or UNDEFINED, in DB default value can be not specified*/
			if (!helpers.isNullOrUndefined(oAttribute.DEFAULT_VALUE)) {
				try {
					...
					if(e.code.code === Code.GENERAL_VALIDATION_ERROR.code) {
	                    const oMessageDetails = new MessageDetails();
	                    oMessageDetails.validationObj = {"columnId":"DEFAULT_VALUE", "validationInfoCode": ValidationInfoCode.VALUE_ERROR};
			...
			//check that default value is set for boolean custom fields
			if (sSemanticDataType === 'BooleanInt' && helpers.isNullOrUndefined(oAttribute.DEFAULT_VALUE)) {
		...
		function checkFormula(oFormula, oBodyItem, oMetaData) {
			...
			if (existingAttribute === undefined) {
				const oMessageDetails = new MessageDetails();
				oMessageDetails.addMetadataObjs(oMetaData);
				oMessageDetails.validationObj = {"columnIds":[{"itemCategoryId" : "ITEM_CATEGORY_ID"}], "validationInfoCode": ValidationInfoCode.METADATA_ERROR};
			...
			// formulas can be created for a field that has the semantic data type Integer, Decimal , String or Link
		...
		function checkSidePanelGroupId(oBodyItem){
            ...
            if(bInvalidId){
                const oMessageDetails = new MessageDetails();
                oMessageDetails.addMetadataObjs(oBodyItem);
                oMessageDetails.validationObj = {"columnIds":[{"columnId" : "SIDE_PANEL_GROUP_ID"}], "validationInfoCode": ValidationInfoCode.METADATA_ERROR};

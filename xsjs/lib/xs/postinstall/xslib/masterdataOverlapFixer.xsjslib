...
function MasterdataOverlapFixer(oRequest, oResponse, oConnection) {
    ...
    function getTableColumns(sTableName){
        const aColumns = Array.from(oConnection.executeQuery(
            `select column_name,
                is_nullable,
                index_type
        ...
        return [aColumns, aKeyColumnNames, aKeyColumnNamesWithoutValidFrom];
    }
     ...
     */     
    function backupTable(sTableName) {
     ...
     */     
    function repairOverlaps(sTableName) {
        ...
        const aValueColumnNames = aColumns.filter(oColumn => {
            ...
            const bIsNotProtected = ["_VALID_TO", "_SOURCE", "_CREATED_BY"].indexOf(sName) === -1;
    ...
    function generateResponse() {
        ...
        const fCreateOverlapOutput = (oOverlapResult) => {
            ...
            Object.keys(oOverlapResult).forEach(sKey => aOverlapBuffer.push(
                `${sKey}: ${oOverlapResult[sKey] instanceof Date ? oOverlapResult[sKey].toJSON() : oOverlapResult[sKey] }`));
    ...
    this.getMasterdataTables = () => {
        return Array.from(oConnection.executeQuery(
        ...
        `
        )).map(oResultObject => oResultObject.TABLE_NAME)
          .filter(sTableName => !sTableName.endsWith(MasterdataOverlapFixer.BACKUP_SUFFIX));
    };
    ...
    this.getOverlaps = (sTableName) => {
        ...
        const [aColumns, aKeyColumnNames, aKeyColumnNamesWithoutValidFrom] = getTableColumns(sTableName);
        const sStmt =
            `select ${aKeyColumnNamesWithoutValidFrom.map(sColumnName => `a.${sColumnName}`).join(", ")},
                    a._valid_from,
                    a._valid_to,

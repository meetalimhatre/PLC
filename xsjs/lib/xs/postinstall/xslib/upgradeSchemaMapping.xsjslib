...
function tableNotRemoved(sTableName) {
    return !aSchemaModification.some(oSchemaModification => (
        oSchemaModification.originalTable === sTableName && oSchemaModification.mappingTable === ""
    ));
}
...
function getTargetTableName(sTableName) {
    ...
    const bTableRenamed = aSchemaModification.some(oSchemaModification => (
        oSchemaModification.originalTable === sTableName &&
        oSchemaModification.mappingTable !== sTableName &&
        !!(oTargetTable = oSchemaModification.mappingTable)
    ));
    return oTargetTable;
}

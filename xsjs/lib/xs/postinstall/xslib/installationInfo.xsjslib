...
function checkPreviousInstallationRun(response, oConnection) {
    ...
    } else if (errOrFinishResult[0].STATE == 'finished' && (errOrFinishResult[0].NAME.includes('99_setup_completed') || errOrFinishResult[0].NAME.includes('PreUpgrade'))) {
        response.setBody(null);
    } else {
...
async function getPrepareForUpgradeInfo(request, oConnection) {
    let bLogEmpty = await isLogEmpty(oConnection);
    if (bLogEmpty) {
        ...
        try {
            ...
            const iCopyResult = await copyLog(oSecondaryConnection);
            // If SAP_PLC t_installation_log is empty, returns an error object
            if (iCopyResult === 0) {
                var sMessage = 'An error occurred when checking version information of previous PLC. Details: ' + 'No entry found in ';
                return {
                    status: 'error',
        ...
        if (oBaseRelease.version === 0 && oBaseRelease.version_sp === 0 && oBaseRelease.version_patch === 0) {
            ...
            if (oLastAction.name != 'PreUpgrade' && !oLastAction.name.includes('99_setup_completed')) {
                ...
                return { version: `${ oBaseRelease.version }.${ oBaseRelease.version_sp }.${ oBaseRelease.version_patch }` };
            }
        }
    }
}

async function getUpgradeInfo(request, oConnection) {
    let bLogEmpty = await isLogEmpty(oConnection);
    if (bLogEmpty) {
        let oSecondaryConnection;
        try {

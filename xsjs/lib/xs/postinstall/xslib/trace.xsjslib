...
const gc_level = {fatal: 'f', info: 'i', warning: 'w', error: 'e', debug: 'd'};
const sLevel = "feiwd";
...
function setTransientMode(level) {
     transientTrace = transientTrace || {rawEntries: [], entries: "", level: gc_level.debug};
...
function getTransientTrace() {
    var output= "";
    if (!(transientTrace && transientTrace.rawEntries)) { return ""; }
    ...
    if (!everythingOK) {
        ...
        if (transientTrace) {
            if (sLevel.indexOf(level) <= sLevel.indexOf(transientTrace.level)) {
                transientTrace.rawEntries.push({level: level, message: output});
            }
        } else {
            switch (theLevel) {

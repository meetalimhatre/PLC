...
const enTexts = ['Standard Price Determination Strategy For Materials (SAP PLC)', 'Standard Price Determination Strategy For Activities (SAP PLC)'];
const deTexts = ['Standard Preisfindung Materialiensstrategie (SAP PLC)', 'Standard Preisfindung Materialiensstrategie (SAP PLC)'];
...
function run(oConnection) {
    ...
    [materialTypeID, activityTypeID].forEach((typeID) =>
        {
            result = oConnection.executeUpdate(
                `SELECT "PRICE_DETERMINATION_STRATEGY_ID" FROM "${sCurrentSchema}"."${sPriceDeterminationStrategyTable}"
            ...
            if(result.length == 0) {
                oConnection.executeUpdate(
                    `INSERT INTO "${sCurrentSchema}"."${sPriceDeterminationStrategyTable}"
                ...
                oConnection.executeUpdate(
                    `INSERT INTO "${sCurrentSchema}"."${sPriceDeterminationStrategyTextTable}"
            ...
            oConnection.executeUpdate(
                `DELETE FROM "${sCurrentSchema}"."${sPriceDeterminationStrategyPriceSourceTable}"
            ...
            oConnection.executeUpdate(
                `INSERT INTO "${sCurrentSchema}"."${sPriceDeterminationStrategyPriceSourceTable}"
    ...
    oConnection.executeUpdate(
        `UPDATE "${sCurrentSchema}"."${sProjectTable}"
    ...
    oConnection.executeUpdate(
        `UPDATE "${sCurrentSchema}"."${sCalculationVersionTable}"
        SET MATERIAL_PRICE_STRATEGY_ID = '${defaultPriceDeterminationStrategyID}',
            ACTIVITY_PRICE_STRATEGY_ID = '${defaultPriceDeterminationStrategyID}';`
    );

...
function run(oConnection) {
    ...
    try{
        ...
        aProjects.forEach(project => {
            ...
            while(iCurrentEntry <= iLastEntry){
                aValues = [project.PROJECT_ID, iCurrentEntry, 1, 'YEARLY', dDate, sUser];
    ...
    validateAllDataHasBeenCopied(oConnection, sLifecycleValuesOld, sLifecycleValuesNew);
    validateAllDataHasBeenCopied(oConnection, sProjectTotalQuantites, sProjectLifecycleConfiguration);

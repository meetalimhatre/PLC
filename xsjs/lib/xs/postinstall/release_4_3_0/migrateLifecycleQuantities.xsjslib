...
async function run(oConnection) {
    ...
    try {
        ...
        let aProjects = await oConnection.executeQuery(`select PROJECT_ID, YEAR(START_OF_PROJECT) as FIRST_YEAR, YEAR(END_OF_PROJECT) as LAST_YEAR  from "sap.plc.db::basis.t_project" where START_OF_PROJECT IS NOT NULL and END_OF_PROJECT IS NOT NULL and LIFECYCLE_VALUATION_DATE IS NOT NULL `);
        if (aProjects.length != 0) {
            ...
            await oConnection.executeUpdate(`INSERT INTO "sap.plc.db::basis.t_project_lifecycle_period_type" (PROJECT_ID, YEAR, IS_YEAR_SELECTED, PERIOD_TYPE, LAST_MODIFIED_ON, LAST_MODIFIED_BY)
...
async function validateAllDataHasBeenCopied(oConnection, sOldTable, sNewTable) {

    let iNoOfNewEntriesOldTable = await oConnection.executeQuery(`select count(RULE_ID) as NUMBER_OF_ENTRIES  from "${ sOldTable }"`);
    let iNoOfNewEntriesNewTable = await oConnection.executeQuery(`select count(PROJECT_ID) as NUMBER_OF_ENTRIES  from "${ sNewTable }"`);
    ...
    if (iNoOfNewEntriesOldTable.NUMBER_OF_ENTRIES === iNoOfNewEntriesNewTable.NUMBER_OF_ENTRIES) {
        await oConnection.executeUpdate(`delete from "${ sOldTable }"`);
    }

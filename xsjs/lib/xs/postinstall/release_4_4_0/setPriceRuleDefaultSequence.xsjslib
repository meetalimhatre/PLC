      ...
      aValues.forEach((record) => {
        if(oPriceDetStrategy.PRICE_DETERMINATION_STRATEGY_TYPE_ID === record.RULE_TYPE_ID){
          oConnection.executeUpdate(
            `INSERT INTO "${sCurrentSchema}"."${sPriceDetStrategyRuleTable}"("PRICE_DETERMINATION_STRATEGY_ID", "PRICE_DETERMINATION_STRATEGY_TYPE_ID", "RULE_CODE", "PRIORITY")
                    VALUES (?, ?, ?, ?)`,
              [[oPriceDetStrategy.PRICE_DETERMINATION_STRATEGY_ID, record.RULE_TYPE_ID, record.RULE_CODE, record.DEFAULT_PRIORITY]]
          )
        }
      })
    });

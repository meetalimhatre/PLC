...
async function getPLCTables() {
    var tables = await oSqlccConnection.executeQuery(`SELECT TABLE_NAME FROM "SYS"."M_CS_TABLES" WHERE SCHEMA_NAME = '${ plcSchema }' AND TABLE_NAME not in (${ aDeletedTables.map(sTableName => {
        return `'${ sTableName }'`;
...
async function getXSCPLCTableColumns(tableName) {
    var plcTable = await oSqlccConnection.executeQuery(`SELECT COLUMN_NAME FROM SYS.COLUMNS WHERE SCHEMA_NAME = '${ plcSchema }' AND TABLE_NAME = '${ tableName }' ORDER BY POSITION`).map(function (item, index) {
        return item.COLUMN_NAME;
    });

...
const aDeletedTables = [
    'sap.plc.db::basis.t_account_group_account_group',
    'sap.plc.db::basis.t_activity_rate_ext',
    'sap.plc.db::basis.t_add_in_configuration',
    'sap.plc.db::basis.t_cost_component',
    'sap.plc.db::basis.t_cost_component__text',
    'sap.plc.db::basis.t_cost_component_values',
    'sap.plc.db::basis.t_replication_mapping',
    'sap.plc.db::basis.t_account_staging',
    'sap.plc.db::basis.t_account__text_staging',
    'sap.plc.db::basis.t_account_account_group_staging',
    'sap.plc.db::basis.t_account_group_staging',
    'sap.plc.db::basis.t_account_group__text_staging',
    'sap.plc.db::basis.t_account_group_account_group_staging',
    'sap.plc.db::basis.t_account_group_cost_component_staging',
    'sap.plc.db::basis.t_activity_type_staging',
    'sap.plc.db::basis.t_activity_type__text_staging',
    'sap.plc.db::basis.t_add_in_configuration_staging',
    'sap.plc.db::basis.t_addin_configuration_header_staging',
    'sap.plc.db::basis.t_addin_configuration_items_staging',
    'sap.plc.db::basis.t_addin_version_staging',
    'sap.plc.db::basis.t_business_area_staging',
    'sap.plc.db::basis.t_business_area__text_staging',
    'sap.plc.db::basis.t_company_code_staging',
    'sap.plc.db::basis.t_company_code__text_staging',
    'sap.plc.db::basis.t_component_split_staging',
    'sap.plc.db::basis.t_component_split__text_staging',
    'sap.plc.db::basis.t_controlling_area_staging',
    'sap.plc.db::basis.t_controlling_area__text_staging',
    'sap.plc.db::basis.t_cost_center_staging',
    'sap.plc.db::basis.t_cost_center__text_staging',
    'sap.plc.db::basis.t_cost_component_staging',
    'sap.plc.db::basis.t_cost_component__text_staging',
    'sap.plc.db::basis.t_cost_component_values_staging',
    'sap.plc.db::basis.t_costing_sheet_staging',
    'sap.plc.db::basis.t_costing_sheet__text_staging',
    'sap.plc.db::basis.t_costing_sheet_base_staging',
    'sap.plc.db::basis.t_costing_sheet_base_row_staging',
    'sap.plc.db::basis.t_costing_sheet_overhead_staging',
    'sap.plc.db::basis.t_costing_sheet_overhead_row_staging',
    'sap.plc.db::basis.t_costing_sheet_row_staging',
    'sap.plc.db::basis.t_costing_sheet_row__text_staging',
    'sap.plc.db::basis.t_costing_sheet_row_dependencies_staging',
    'sap.plc.db::basis.t_default_settings_staging',
    'sap.plc.db::basis.t_material_account_staging',
    'sap.plc.db::basis.t_material_group_staging',
    'sap.plc.db::basis.t_material_group__text_staging',
    'sap.plc.db::basis.t_material_type_staging',
    'sap.plc.db::basis.t_material_type__text_staging',
    'sap.plc.db::basis.t_price_ext',
    'sap.plc.db::basis.t_overhead_group_staging',
    'sap.plc.db::basis.t_overhead_group__text_staging',
    'sap.plc.db::basis.t_plant_staging',
    'sap.plc.db::basis.t_plant__text_staging',
    'sap.plc.db::basis.t_valuation_class__text_staging',
    'sap.plc.db::basis.t_activity_rate',
...
const excludeTables = [
    'sap.plc.db::basis.t_lock',
    'sap.plc.db::basis.t_task',
    'sap.plc.db::basis.t_log'
];
const preMigrationTableName = [
    'sap.plc.db::basis.t_metadata__text',
    'sap.plc.db::basis.t_formula'
];
 ...
 */
async function getPLCTables() {
    var tables = oSqlccConnection.executeQuery(`SELECT TABLE_NAME FROM "SYS"."M_CS_TABLES" WHERE SCHEMA_NAME = '${ plcSchema }' AND TABLE_NAME not in (${ aDeletedTables.map(sTableName => {
        return `'${ sTableName }'`;
    }).join() }) `);    //IS_USER_DEFINED_TYPE is used to distinguish temp tables and tables.
    return tables.map(item => {
        return item.TABLE_NAME;
    }).filter(sTableName => {
        return await tableNotRemoved(sTableName);
    });
}
...
async function compareTableColumns(tableName, currentSchemaName) {
    var currentTable = await getSchemaTableColumns(await getTargetTableName(tableName), currentSchemaName);
 ...
 */
async function getXSCPLCTableColumns(tableName) {
    var plcTable = oSqlccConnection.executeQuery(`SELECT COLUMN_NAME FROM SYS.COLUMNS WHERE SCHEMA_NAME = '${ plcSchema }' AND TABLE_NAME = '${ tableName }' ORDER BY POSITION`).map(function (item, index) {
        return item.COLUMN_NAME;
    });
    var aRemovdColumns = await getDeletedColumns(tableName);
    return plcTable.filter(sColumn => {
        return !aRemovdColumns.find(oRemovedColumn => {
            return oRemovedColumn.originalColumn === sColumn;
        });
    }).map(sColumn => {
        return await getRenameColumn(tableName, sColumn);
    });
}
...
async function run(oConnection, oLibraryMeta, oRequestArgs) {
    if (oSqlccConnection !== null) {
        try {
            ...
            await migrateMetadataCustomFields(currentSchemaName, oSqlccConnection);
            ...
            var oDbArtefactController = await new DbArtefactController($, oConnection);
...
//the table t_afl_signature is a no primary key table which can't use upsert..select.. sql
        ...
        var aXSCColumns = await getSchemaTableColumns(sTableName, plcSchema);
        var aRemovdColumns = await getDeletedColumns(sTableName);
        aXSCColumns = aXSCColumns.filter(sColumn => {
            return !aRemovdColumns.find(oRemovedColumn => {
                return oRemovedColumn.originalColumn === sColumn;
            });
        });
        var sPreSql = `UPSERT "${ await getTargetTableName(sTableName) }" (`;
        var sPostSql = `SELECT `;
        aXSCColumns.forEach(sColumn => {
            var sRenameColumn = await getRenameColumn(sTableName, sColumn);
        ...
        var iRecordNumber = await checkTableSize(sTableName, oSqlccConnection);
    ...
    await removeOldVersionData(sTableName, oSqlccConnection);
}

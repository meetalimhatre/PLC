const replaceColumns = ["_CREATED_BY", "LAST_MODIFIED_BY", "USER_ID"];
...
function run(oConnection, oLibraryMeta ,oRequestArgs) {
    try {
        ...
        const sSchema = getCurrentSchema(oConnection);
        const aPLCTables = getPLCTables(oConnection, sSchema);
        replaceInstanceBasedUsers(oConnection, aPLCTables, sSchema, oRequestArgs.file);
        return true;
    } catch(e) {
 ...
 */
function replaceInstanceBasedUsers(oConnection, aPLCTables, sSchema, aMappingUserList) {
    ...
    if (aMappingUserList && aMappingUserList.length && aMappingUserList !== "no data" && aMappingUserList !== "[]") {
        aPLCTables.map((sTable) => {
            for (let column of replaceColumns) {
                const sSql = createUserReplaceSql(sTable, oConnection, column, sSchema);
                if (sSql && sSql.length) {

...
async function run(oConnection) {
    ...
    await oConnection.executeUpdate(`UPDATE "${ sCurrentSchema }"."${ sProjectTable }" SET ENTITY_ID = "${ sCurrentSchema }"."${ sEntityIdSequence }".NEXTVAL;`);

    //Step 2 - update entity relation: add all projects on the root location.
    await oConnection.executeUpdate(`INSERT INTO "${ sCurrentSchema }"."${ sEntityRelationTable }" 
                                SELECT "ENTITY_ID",
    ...
    await oConnection.executeUpdate(`ALTER TABLE "${ sCurrentSchema }"."${ sProjectTable }" ALTER ("ENTITY_ID" INTEGER DEFAULT NULL);`);
...
export default {sProjectTable,sEntityRelationTable,sEntityIdSequence,check,getCurrentSchemaName,run,clean};

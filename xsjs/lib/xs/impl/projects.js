 ...
 */
    async function checkEntityData(sPath, oPersistency, sTable, sPathType) {
        ...
        const iEntityId = await helpers.getEntityIdFromPath(sPath);
        ...
        async function triggerLifecycleVersionCalculation() {
            ...
            let oFollowUp = {
                ...
                parameter: {
                    ...
                    ONE_TIME_COST_ITEM_DESCRIPTION: await helpers.isNullOrUndefined(oProject) === true ? null : await helpers.isNullOrUndefined(oProject[0].oneTimeCostItemDescription) === true ? null : oProject[0].oneTimeCostItemDescription
                }
            };
    ...
    this.update = async function (oProject, aParameters, oServiceOutput, oPersistency) {
        ...
        async function checkProjectDates(sStartProperty, sEndProperty) {
            ...
            if (!(await helpers.isNullOrUndefined(sStart) === true && await helpers.isNullOrUndefined(sEnd) === true)) {
                sStart = await helpers.isNullOrUndefined(sStart) === false ? sStart : oCurrentProject[sStartProperty];
                sEnd = await helpers.isNullOrUndefined(sEnd) === false ? sStart : oCurrentProject[sEndProperty];
        ...
        if (oProject.TARGET_PATH && oProject.PATH) {
            ...
            const iSourceEntityId = await helpers.getEntityIdFromPath(oProject.PATH);

...
module.exports.VariantCalculator = async function ($) {
    ...
    this.calculateTransient = async function (oRequestVariantsAndItems, mParameters, oServiceOutput, oPersistency) {
        ...
        const oCalculatedVariantResult = Array.from(oProcedureResult.CALCULATED_VARIANT_HEADER).map(oCalculatedVariant => {
            ...
            oVariant.ITEMS = await helpers.transposeResultArrayOfObjects(mCalculatedVariantItems.get(oVariant.VARIANT_ID), false);
    ...
    this.calculatePersistent = async function (oEmptyBody, mParameters, oServiceOutput, oPersistency) {
        ...
        const aExistingVariantItems = await oPersistency.Variant.getVariantItems(iVariantId);
        const aVariantItemReadOnlyFields = [
            'TOTAL_QUANTITY',
            'TOTAL_COST',
            'QUANTITY_CALCULATED',
        ...
        await oPersistency.Variant.update(aCalculatedVariantResult, aVariantToBeCalculated, iCalculationVersionId);
 ...
 */
    this.calculateSumVariant = (oRequestVariants, mParameters, oServiceOutput, oPersistency) => {
        ...
        if (oRequestVariants.VARIANTS.length > iMaximumNumberOfVariants) {
            const oDetails = new MessageDetails();
            oDetails.addCalculationVersionObjs({ id: iCalculationVersionId });
    ...
    async function persistVariantsAfterRecalculation(oVariant, iCalculationVersionId, oPersistency) {
        const oExistingVariant = await oPersistency.Variant.getVariant(iCalculationVersionId, oVariant.VARIANT_ID);
        if (_.isUndefined(oExistingVariant)) {
            const sClientMsg = 'No variant exists for the given id.';
        ...
        oVariant.LAST_CALCULATED_ON = new Date();
        oVariant.LAST_CALCULATED_BY = $.getPlcUsername();
        await oPersistency.Variant.update([oVariant], [oExistingVariant], iCalculationVersionId);
    }
};
export default {_,helpers,VariantService,MessageLibrary,Constants,PlcException,MessageDetails,MessageCode,CalcEngineErrors,PlcMessage,Severity};

...
this.get = function(oBodyData, oParameters, oServiceOutput, oPersistency) {
	...
	try{
		...
		if (oParameters.business_object === sVendor) {
			...
			auditLog.read({ type: sVendor, id: { key: `TimeStamp: [${new Date().toString()}]` } })
			.attribute({ name: 'TimeStamp', successful: true })
			.dataSubject({ type: 'SAP', id: { key: 'PLC' }, role: 'VendorRead' })
			.accessChannel('UI')
			.by(`[${sUserId}]`)
			.log(function (err) {
				if (err) {
		...
		if (oParameters.business_object === sCustomer) {
			...
			auditLog.read({ type: sCustomer, id: { key: `TimeStamp: [${new Date().toString()}]` } })
			.attribute({ name: 'TimeStamp', successful: true })
			.dataSubject({ type: 'SAP', id: { key: 'PLC' }, role: 'CustRead' })
			.accessChannel('UI')
			.by(`[${sUserId}]`)
			.log(function (err) {
				if (err) {
...
this.edit = function(oBodyData, oParameters, oServiceOutput, oPersistency) {
	...
	if((sObjectName === BusinessObjectTypes.AccountGroup)||(sObjectName === BusinessObjectTypes.ComponentSplit)
		||(sObjectName === BusinessObjectTypes.CostingSheet)||(sObjectName === BusinessObjectTypes.CostingSheetRow)
		...
		if((sObjectName === BusinessObjectTypes.AccountGroup)||(sObjectName === BusinessObjectTypes.ComponentSplit)
			...
			if(sObjectName === BusinessObjectTypes.CostingSheet && 
				(oResultSetInsert !== {} || oResultSetUpdate !== {}) &&
			  	(helpers.isNullOrUndefined(oResultSetInsert.hasErrors) || oResultSetInsert.hasErrors === false) &&
			  	(helpers.isNullOrUndefined(oResultSetUpdate.hasErrors) || oResultSetUpdate.hasErrors === false)
			){
		...
		if (sObjectName === sVendor) {
			...
			const message = auditLog.update({ type: sVendor, id: { key: `TimeStamp: [${sMasterDataDate.toString()}]` } })
							.attribute({ name: 'TimeStamp', old: 'oldValue', new: `[${sMasterDataDate.toString()}]`})
							.dataSubject({ type: 'SAP', id: { key: 'PLC' }, role: 'VendorEdit' })
							.by(`[${sUserId}]`);
		...
		if (sObjectName === sCustomer) {
			...
			const message = auditLog.update({ type: sCustomer, id: { key: `TimeStamp: [${sMasterDataDate.toString()}]` } })
							.attribute({ name: 'TimeStamp', old: 'oldValue', new: `[${sMasterDataDate.toString()}]`})
							.dataSubject({ type: 'SAP', id: { key: 'PLC' }, role: 'CustEdit' })
							.by(`[${sUserId}]`);
			message.logPrepare(function (err) {
				message.logSuccess(function (err) { 
					if (err) { 
...
}; // end of module.exports.Administration
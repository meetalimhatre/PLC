...
this.get = function(oBodyData, oParameters, oServiceOutput, oPersistency) {    
    ...
    // Prepare Output
    _.each(oAddins, function(oResult, iIndex) {        
...
function prepareAddinObject(oAddinResult) {
    return _.extend(
                prepareAddinVersionObject(oAddinResult),
                { 
                    CONFIGURATION : {
...
function prepareAddinVersionObject(oAddinVersionObject) {
    ...
    _.each(aMandatoryProperties, function(sProperty, iIndex) {
    ...
    return {
            ...
            ADDIN_VERSION : 		[	oAddinVersionObject.ADDIN_MAJOR_VERSION,

...
this.create = function(oBodyItems, aParameters, oServiceOutput, oPersistency) {
	...
	if(!helpers.isNullOrUndefined(oBodyItems.LAYOUT_NAME) && !oPersistency.Layout.isNameUnique(oBodyItems.LAYOUT_ID, oBodyItems.LAYOUT_NAME, sUserId)){
	...
	var iLayoutId = oPersistency.Layout.create(oBodyItems, sUserId, iIsCorporate);
...
this.update = function(oBodyItems, aParameters, oServiceOutput, oPersistency) {
	...
	var iLayoutId = oPersistency.Layout.update(oBodyItems, sUserId, iIsCorporate);

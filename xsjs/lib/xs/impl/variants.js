    ...
    this.get = async function (aBodyItems, mParameters, oServiceOutput, oPersistency) {
        ...
        if (iVariantId) {
            ...
            if (bIsExpandItems) {
                ...
                oVariant.ITEMS = await oPersistency.Variant.getVariantItems(iVariantId);
            }
            aResultVariants.push(_.omit(oVariant, 'VARIANT_ORDER'));
        } else {
            // return all variants headers
            const aReturnedVariants = await oPersistency.Variant.getVariants(iCalculationVersionId);
 ...
 */
    async function upsertItems(aRequestItems, iVariantId, oPersistency, iCalculationVersionId) {
        if (aRequestItems.length === 0) {
            return;
        }
        const aItemsIds = aRequestItems.map(oItem => oItem.ITEM_ID);
        const aDatabaseItems = await oPersistency.Variant.getVariantItems(iVariantId, aItemsIds);
        const mDatabaseItems = new Map(aDatabaseItems.map(oItem => [
            oItem.ITEM_ID,
            _.clone(oItem)
        ]));
 ...
 */
    async function checkItemsMapToBaseVersion(oPersistency, aRequestVariantItems, iCalculationVersionId) {
        ...
        const mRequestVariantItems = new Map(aRequestVariantItems.map(oItem => [
            oItem.ITEM_ID,
            _.clone(oItem)
        ]));
...
*/
    this.createVariant = async function (oBody, mParameters, oServiceOutput, oPersistency) {
        const iCalculationVersionId = mParameters.calculation_version_id;
        await VariantService.checkCalculationVersionExists(oPersistency, iCalculationVersionId);
        ...
        if (oBody.VARIANT_TYPE === 1) {
            await VariantService.checkSumVariantIsNotDuplicated(oPersistency, iCalculationVersionId);
        }

        await VariantService.isVariantNameUnique(oBody.VARIANT_NAME, iCalculationVersionId, oPersistency);
        await checkItemsMapToBaseVersion(oPersistency, oBody.ITEMS, iCalculationVersionId);
        ...
        const iVariantId = await oPersistency.Variant.createVariant(_.omit(oVariantToCreate, 'ITEMS'), iCalculationVersionId, dCurrentTimestamp);
 ...
 */
    this.update = async function (oRequestVariant, mParameters, oServiceOutput, oPersistency) {
        const iCalculationVersionId = mParameters.calculation_version_id;
        const sUserId = $.getPlcUsername();
        ...
        const oBeforeVariant = await oPersistency.Variant.getVariant(iCalculationVersionId, iVariantId);
        ...
        if (aItemsToUpdate && aItemsToUpdate.length > 0) {
            ...
            await VariantService.checkQuantityStateValues(oPersistency, aItemsToUpdate, iCalculationVersionId);
            ...
            await upsertItems(aItemsToUpdate, iVariantId, oPersistency, iCalculationVersionId);
        ...
        await oPersistency.Variant.update([oVariantToUpdate], [oBeforeVariant], iCalculationVersionId);
        await deleteNotMatchingVariantIems(oPersistency, oRequestVariant, iCalculationVersionId, iVariantId);
 ...
 */
    this.updateVariantsOrder = async function (aRequestVariantIds, mParameters, oServiceOutput, oPersistency) {
        const iCalculationVersionId = mParameters.calculation_version_id;
        ...
        const aDatabaseVariants = await oPersistency.Variant.getVariants(iCalculationVersionId, aVariantIds);
 ...
 */
    this.remove =async function (oRequestVariant, mParameters, oServiceOutput, oPersistency) {
        ...
        const bIsLastVariantForVersion = (await oPersistency.Variant.getVariants(iCalculationVersionId)).length === 0;
...
export default {MessageLibrary,Constants,ExpandParameterItems,PlcException,_,MessageCode,VariantService,SQLMaximumInteger};

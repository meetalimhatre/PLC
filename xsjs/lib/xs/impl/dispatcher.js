...
const metadata = async function ($) {
    return new (require('./metadata')).Metadata($);
};
const ping = function ($) {
...
const transportation = async function ($) {
    return new (require('./transportation')).Transportation($);
};
            ...
            if (sInstancePrivilege !== undefined) {
                ...
                case AuthorizationManager.BusinessObjectTypes.Project:
                    if (Helpers.isNullOrUndefined(oRequestContent.parameters.id) || _.includes(CalculationServiceParameters, oRequestContent.parameters.action)) {
                        sBusinessObjectId = helpers.isNullOrUndefined(_.result(oRequestContent.data, 'PROJECT_ID')) ? _.result(_.find(oRequestContent.data, 'PROJECT_ID'), 'PROJECT_ID') : _.result(oRequestContent.data, 'PROJECT_ID');
                    } else {
                ...
                case AuthorizationManager.BusinessObjectTypes.Privilege:
                    sBusinessObjectId = helpers.isNullOrUndefined(oRequestContent.parameters.entity_id) ? helpers.getValueOnKey(oRequestContent.data, 'ENTITY_ID') : oRequestContent.parameters.entity_id;
                    sBusinessObjectType = BusinessObjectTypes.Project;
                    break;
                default: {
                ...
                await AuthorizationManager.checkPrivilege(sBusinessObjectType, sBusinessObjectId, sInstancePrivilege, persistency.getConnection(), $.getPlcUsername());
            }
        }

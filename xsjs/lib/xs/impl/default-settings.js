...
function throwLockingUserException(sLockUser) {
...
this.get = function(aBodyMeta, oParameters, oServiceOutput, oPersistency) {	
...
this.create = function(oDefaultSettings, oParameters, oServiceOutput, oPersistency) {
	var oResult = {};
	var sType = oParameters.type;
	var sUserId = $.getPlcUsername();
	...
	if(sType === "global") {
		...
		if(aLockObject.length === 0) {
			oResult.DEFAULT_SETTINGS = oPersistency.DefaultSettings.create(oDefaultSettings, sType, sUserId);
			oServiceOutput.setBody(oResult).setStatus($.net.http.CREATED);
		} else {	    
		    throwLockingUserException(aLockObject[0].USER_ID);
		}
	} else {
		oResult.DEFAULT_SETTINGS = oPersistency.DefaultSettings.create(oDefaultSettings, sType, sUserId);
		oServiceOutput.setBody(oResult).setStatus($.net.http.CREATED);
...
this.update = function(oDefaultSettings, oParameters, oServiceOutput, oPersistency) {
	var oResult = {};
	var sType = oParameters.type;
	var sUserId = $.getPlcUsername();
	...
	if(sType === "global") {	
		...
		if(aLockObject.length === 0) {
			oResult.DEFAULT_SETTINGS = oPersistency.DefaultSettings.update(oDefaultSettings, sType, sUserId);
...
this.remove = function(oDefaultSettings, oParameters, oServiceOutput, oPersistency) {
	var oResult = {};
	var sType = oParameters.type;
	var sUserId = $.getPlcUsername();
...
}; // end of module.exports.DefaultSettings
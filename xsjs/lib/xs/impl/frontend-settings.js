 ...
 *
 */
this.get = function(aBodyItems, oParameters, oServiceOutput, oPersistency) {
	...
	oServiceOutput.setBody({
		"SETTINGS": _.values(oFrontendSettingsInformation)
	});
}
...
this.create = function(aBodyItems, aParameters, oServiceOutput, oPersistency) {
	...
	oServiceOutput.setBody({
		"SETTINGS": _.values(oResult.SETTINGS)
	}).setStatus($.net.http.CREATED);
...
this.update = function(aBodyItems, aParameters, oServiceOutput, oPersistency) {
	var aMappingResults = mapSettingsUpdates(aBodyItems, getExistentSettings(aBodyItems, aParameters.is_corporate, oPersistency));
	var oResult = oPersistency.FrontendSettings.updateFrontendSettings(aMappingResults);
	if (oResult.ERRORS.length > 0) {
		namingConflictError(oResult.ERRORS);
	}
	oServiceOutput.setBody({
		"SETTINGS": _.values(oResult.SETTINGS)
	}).setStatus($.net.http.OK);
	return oServiceOutput;
}
...
this.remove = function(aBodyItems, aParameters, oServiceOutput, oPersistency) {
	var aFrontendSettingsNotDeleted = oPersistency.FrontendSettings.deleteFrontendSettings(getExistentSettings(aBodyItems, aParameters.is_corporate,
		oPersistency));
	if (aFrontendSettingsNotDeleted.length > 0) {
		...
		_.each(aFrontendSettingsNotDeleted, function(oFrontendSettingsNotDeleted) {
			...
			oMessageDetails.addSettingsObj({
				SETTING_ID: oFrontendSettingsNotDeleted.SETTING_ID
			});
		});
 ...
 */
function getExistentSettings(aBodyItems, isCorporate, oPersistency) {
...
}; // end of module.exports.FrontendSettings
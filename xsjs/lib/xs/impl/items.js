 ...
 */
    ...
    async function setQuantityUomIdByMaterialBaseUomId(aItems, oPersistency) {
        ...
        let aMaterialIds = [];
        aItems.forEach(oItem => {
            if (oItem.ITEM_CATEGORY_ID === ItemCategory.InternalActivity && helpers.isNullOrUndefined(oItem.QUANTITY_UOM_ID)) {
        ...
        aItems.forEach(oItem => {
            ...
            if (!helpers.isNullOrUndefined(oItem.MATERIAL_ID) && !helpers.isNullOrUndefined(aMaterialBaseUomIds) && aMaterialBaseUomIds.length > 0 && helpers.isNullOrUndefined(oItem.QUANTITY_UOM_ID)) {

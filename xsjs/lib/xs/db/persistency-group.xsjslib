...
function Group(dbConnection) {
	 ...
	 *
	 */
	this.getGroups = function(aGroupId, sFilter) {
	 ...
	 */	
	this.insertUsergroups = function(aUsergroups) {
		var aUsergroupsColumn = [];
		_.each(aUsergroups, function (oUsergroup) {		
			var oUsergroupColumn = [oUsergroup.GROUP_ID.toUpperCase(), oUsergroup.DESCRIPTION];
		...
		return helpers.unsuccessfulItemsDbOperation(aUsergroups, aInsertResult);
	 ...
	 * @returns {array} - an array of objects that could not be inserted into the table
	 */
	this.insertUserMembership = function(aUserMembers) {	
		var touchedGroups = new Set();
		var aUserMembersColumn = [];
		_.each(aUserMembers, function (oUserMember) {		
			var oUserMembersColumn = [oUserMember.GROUP_ID.toUpperCase(), oUserMember.USER_ID.toUpperCase()];
		...
		try {
			...
			{
				authorizationUnroller.unrollPrivilegesOnGroupUpdate(dbConnection, groupId);
			}
		} catch(e) {
		...
		//return an array of object that could not be inserted -> 1 means successful operation
		return helpers.unsuccessfulItemsDbOperation(aUserMembers, aInsertResult);
	};
	 ...
	 * @returns {array} - an array of objects that could not be inserted into the table
	 */
	this.insertGroupMembership = function(aGroupMembers) {	
		var touchedGroups = new Set();
		var aGroupMembersColumn = [];
		_.each(aGroupMembers, function (oGroupMember) {		
			var oUserMembersColumn = [oGroupMember.GROUP_ID.toUpperCase(), oGroupMember.SUBGROUP_ID.toUpperCase()];
		...
		//return an array of object that could not be inserted -> 1 means successful operation
		return helpers.unsuccessfulItemsDbOperation(aGroupMembers, aInsertResult);
	};
	...
	this.deleteUsergroups = function(aGroups) {	
		var touchedGroups = new Set();
		var aGroupColumn = [];
		...
		var  aGroupsNotDeleted = helpers.unsuccessfulItemsDbOperation(aGroups, aDeleteResult);
		if (aGroupsNotDeleted.length === 0) {
			...
			authorizationUnroller.unrollPrivileges(dbConnection, Array.from(objectsFromGroupPrivileges));
			
			//delete users and subgroups memberships
	...
	/**
	 * Deletes user membership from the input array
	 * @param {array}
	 *            aUserMembers - the array of user members that will be deleted
	 * @returns {array} - an array of objects that could not be deleted from the table
	 */

	this.deleteUserMembership = function(aUserMembers) {	
		var touchedGroups = new Set();
		var aUserMembersColumn = [];
		_.each(aUserMembers, function (oUserMember) {		
			var oUserMembersColumn = [oUserMember.GROUP_ID.toUpperCase(), oUserMember.USER_ID.toUpperCase()];
		...
		//returns an array with the objects that could not be deleted, 1 means deletion was successful and 0 not successful
		return helpers.unsuccessfulItemsDbOperation(aUserMembers, aDeleteResult);
	};

	/**
	 * Deletes group membership from the input array
	 * @param {array}
	 *           aGroupMembers - the array of group members (subgroups) that will be deleted
	 * @returns {array} - an array of objects that could not be deleted from the table
	 */

	this.deleteGroupMembership = function(aGroupMembers) {	
		var touchedGroups = new Set();
		var aGroupMembersColumn = [];
		_.each(aGroupMembers, function (oGroupMember) {		
			var oUserMembersColumn = [oGroupMember.GROUP_ID.toUpperCase(), oGroupMember.SUBGROUP_ID.toUpperCase()];
		...
		//returns an array with the objects that could not be deleted, 1 means deletion was successful and 0 not successful
		return helpers.unsuccessfulItemsDbOperation(aGroupMembers, aDeleteResult);
	};

	/**
	 * Updates user groups from the input array
	 * @param {object}
	 *            oGroup - user group object that will be updated
	 * @returns {array} - an array of objects that could not be updated
	 */

	this.updateUsergroups = function(aGroups) {	
		var aGroupColumn = [];
		_.each(aGroups, function (oGroup) {		
			var oGroupColumn = [oGroup.DESCRIPTION, oGroup.GROUP_ID.toUpperCase()];
			aGroupColumn.push(oGroupColumn);
		});
		...
		return helpers.unsuccessfulItemsDbOperation(aGroups, aUpdateResult);
	};
}

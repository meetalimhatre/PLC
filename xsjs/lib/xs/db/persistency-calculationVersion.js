...
async function CalculationVersion($, dbConnection, hQuery, sUserId) {
    ...
    var metadata = await new Metadata($, hQuery, null, sUserId);
    ...
    this.getOrDetermineNewCalculationVersionName = async function (oCalculationVersion) {
        ...
        if (this.isNameUniqueInBothTables(oCalculationVersion.CALCULATION_VERSION_NAME, oCalculationVersion.CALCULATION_ID)) {
			 ...
			 */
            var oSplitedCalculationVersionName = await helpers.splitIncrementalString(oCalculationVersion.CALCULATION_VERSION_NAME);
            ...
            var iSuffix = await helpers.findFirstUnusedSuffixInStringArray(oSplitedCalculationVersionName.Prefix, oSplitedCalculationVersionName.StartSuffix, aNamesWithPrefix);
 ...
 // there should be exactly one result row
        ...
        if (bCompressedResult) {
            let output = await helpers.transposeResultArray(result.ITEMS);
    ...
    this.copy = async function (iCalculationVersionId, sSessionId, sUserId, sLanguage, bCompressedResult) {
        ...
        if (bCompressedResult) {
            oReturnObject.itemsCompressed = await helpers.transposeResultArray(result.OT_NEW_ITEMS, false);
        } else {
    ...
    this.addCurrencyUnitsToCalculationResults = async (iCalculationVersionId, oItemCalc, oPersistency, bCompressed) => {
        if (oPersistency.Metadata.getAllCustomFieldsNamesAsArray().length > 0) {
            ...
            const aItemCFsCurrency = await helpers.transposeResultArrayOfObjects(oResult.OT_ITEMS_CUSTOM_FIELDS_CURRENCY);

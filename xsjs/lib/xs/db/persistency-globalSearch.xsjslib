 ...
 */
function GlobalSearch(dbConnection, hQueryPlc) {
	var that = this;
	this.helper = new Helper($, hQueryPlc, dbConnection);
	 ...
	 *  
	 */
	this.get = function(sSortedColumnId, sSortedDirection, sFilter, sType, iTop, sUserId) {
		...
		try {
			...
			let aReturnObject = oGlobalSearchStatement.execute(iTop, sFilter, sFilter, sFilter, sFilter, sUserId, sFilter, sFilter, sFilter, sFilter, sFilter, sFilter, sUserId, sFilter, sFilter, sFilter, sFilter, sFilter, sUserId);
	...
	function getFilter(sFilter){
		...
		sFilter = helpers.replaceSpecialCharsForSQLLikeRegexpr(sFilter);

		let aSplittedFilter = sFilter.split(' ');
		let iSplittedFilterLength = aSplittedFilter.length; 
		let aSplittedFilterToBeReturned = [];
		
		// check for special case when there is only one element in array ( in case somebody is searching for and or or ) do display
		// only data that contains and or or
		if (iSplittedFilterLength === 1) {
			...
			_.each(aSplittedFilter, function(oSplittedFilter, iIndex){
				...
				// OR means append a PIPE ( | ) to the result
				if (oSplittedFilterUpper === 'OR'){
				    // when OR is the last word, do not replace it with a pipe
				    if (iIndex + 1 !== iSplittedFilterLength) {
				        aSplittedFilterToBeReturned.push('|');
				    }
				}
				else if(oSplittedFilterUpper !=='AND'){
					// check if the previous item is an OR; 
		            // if it is, simply append the word without transforming it to a valid regex
	 ...
	 * Function parse the filter selection and prepare output for sql contains function
	 *
	 * @param sSortedDirection {string} - sorted direction 'Ascending' or 'Descending'
	 * 
	 * @returns {string} return coresponding value for sql type
	 */
	 ...
	 * Function parse the filter selection and prepare output for sql contains function
	 *
	 * @param sType {string} - type of the entity that user wants to search for
	 * 
	 * @returns {string} returns computed where clause with filtering entity type by entity searched
	 */

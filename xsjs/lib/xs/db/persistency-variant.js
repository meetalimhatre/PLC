 ...
 */
function Variant($, dbConnection, hQuery) {
     ...
     */
    this.getVariantsInternal = function getVariantsInternal(iCalculationVersionId, aVariantIds, bCopiedVariantsOnly) {
    ...
    this.getVariants = function getVariants(iCalculationVersionId, aVariantIds) {
     ...
     */
    this.getVariant = function getVariant(iCalculationVersionId, iVariantId) {
        const aResult = this.getVariants(iCalculationVersionId, [iVariantId]);
     ...
     */
    this.getCopiedVariants = function getCopiedVariants(iCalculationVersionId) {
        let aVariantIds;
    ...
    */
     ...
     *
     */
    this.createVariant = function createVariant(oVariantHeader, iCalculationVersionId) {
        // remove generated columns from the given object; only as a precaution
        const aRemoveFields = ["VARIANT_ID", "LAST_GENERATED_VERSION_ID", "LAST_GENERATED_CALCULATION_ID"];
    ...
    this.upsertVariantItems = (iVariantId, aVariantItemsToUpsert, iCalculationVersionId) => {
        ...
        const aVariantItemKeys = ["VARIANT_ID", "ITEM_ID", "IS_INCLUDED", "QUANTITY", "QUANTITY_STATE", "QUANTITY_UOM_ID", "TOTAL_QUANTITY", "TOTAL_COST"];
    ...
    this.copyToTemporaryTables = (iCalculationVersionId) => {
        const aVariantKeys = ["VARIANT_ID", "CALCULATION_VERSION_ID", "VARIANT_NAME", "COMMENT", "EXCHANGE_RATE_TYPE_ID", "TOTAL_COST", "REPORT_CURRENCY_ID", "SALES_PRICE", "SALES_PRICE_CURRENCY_ID", "VARIANT_ORDER", "IS_SELECTED", "VARIANT_TYPE", "LAST_REMOVED_MARKINGS_ON", "LAST_REMOVED_MARKINGS_BY", "LAST_MODIFIED_ON", "LAST_MODIFIED_BY", "LAST_CALCULATED_ON", "LAST_CALCULATED_BY"];
        const aVariantItemKeys = ["VARIANT_ID", "ITEM_ID", "IS_INCLUDED", "QUANTITY", "QUANTITY_CALCULATED", "QUANTITY_STATE", "QUANTITY_UOM_ID", "TOTAL_QUANTITY", "TOTAL_COST"];
        ...
        aVariantsToUpdate.forEach((oVariant) => {
            ...
            aSortedColumns.forEach((sColumn) => {
                if (_.has(oVariant, sColumn)) {
                    ...
                    aVariantValues.push(_.find(aVariantsFromDatabase, oExistingVariant =>
                        oExistingVariant.VARIANT_ID === oVariant.VARIANT_ID)[sColumn]);
                }
            });
    ...
    this.updateParentsIsIncludedState = function (iVariantId, aListOfItemsToUpdate) {
        dbConnection.executeUpdate(
            `UPDATE "${Tables.variant_item}" set IS_INCLUDED = 1

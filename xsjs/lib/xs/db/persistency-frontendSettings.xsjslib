...
const Sequences = Object.freeze({
	frontend_settings: 'sap.plc.db.sequence::s_frontend_settings'
});

/**
 * Provides persistency operations with frontend settings.
 */
function FrontendSettings(dbConnection, hQuery) {
	 ...
	 */
	this.getFrontendSettings = function(sType, sUserId) {
		try {
			var stmtFrontendSettings =
				`select SETTING_ID, SETTING_NAME, SETTING_TYPE, USER_ID, SETTING_CONTENT from 
	 ...
	 */
	this.getFrontendSettingsMassChangeIds = function(sIds) {
		var sStmtFrontendSettings = 
	 ...
	 */
	this.insertFrontendSettings = function(aSettings, sUserId) {
		var aSettingsInsert = [];
		...
		_.each(aSettings, function(oSetting) {
			...
			var oSettingsColumn = [iSettingId, oSetting.SETTING_NAME, oSetting.SETTING_TYPE.toUpperCase(), sUserId, oSetting.SETTING_CONTENT];
		...
		try {
			var aInsertResult = dbConnection.executeUpdate(
				`INSERT INTO "${Tables.frontend_settings}" 
			                    (SETTING_ID, SETTING_NAME, SETTING_TYPE, USER_ID, SETTING_CONTENT) VALUES (?,?,?,?,?)`,
				aSettingsInsert);
		} catch (e) {
		...
		return {
			ERRORS: helpers.unsuccessfulItemsDbOperation(aSettings, aInsertResult),
	 ...
	 */
	this.updateFrontendSettings = function(aSettings) {
		var aSettingsUpdate = [];

		_.each(aSettings, function(oSetting) {
			var oSettingsColumn = [oSetting.SETTING_NAME, oSetting.SETTING_CONTENT, oSetting.SETTING_ID];
			aSettingsUpdate.push(oSettingsColumn);
		});

		try {
			var aUpdateResult = dbConnection.executeUpdate(
				`update "${Tables.frontend_settings}"
		...
		return {
			ERRORS: helpers.unsuccessfulItemsDbOperation(aSettings, aUpdateResult),
			SETTINGS: aSettings
		};
	};
	 ...
	 */
	this.deleteFrontendSettings = function(aSettings) {
		var aSettingsIds = [];
		_.each(aSettings, function(oSetting) {
		...
		var aDeleteResult = dbConnection.executeUpdate(`delete from "${Tables.frontend_settings}" where SETTING_ID = ?`,
			aSettingsIds);

		//returns an array with the objects that could not be deleted, 1 means delete was successful and 0 not successful
		return helpers.unsuccessfulItemsDbOperation(aSettings, aDeleteResult);
	};
}

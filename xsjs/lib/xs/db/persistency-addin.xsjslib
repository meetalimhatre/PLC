...
function Addin(dbConnection, hQuery) {
	 ...
	 */
	this.getAddin = function(sGuid, aVersions) {
		
		var sAddinStmt = ['select a.addin_guid addin_guid, a.addin_major_version addin_major_version, a.addin_minor_version addin_minor_version, a.addin_revision_number addin_revision_number,',
	...
	this.getAddinsByStatus = function(sStatus) {
		...
		var sAddinsStmt = ['select a.addin_guid addin_guid, a.addin_major_version addin_major_version, a.addin_minor_version addin_minor_version, a.addin_revision_number addin_revision_number,',
					'a.addin_build_number addin_build_number, a.name name,',
                    'a.full_qualified_name full_qualified_name, a.description description, a.publisher publisher,',
                    'a.status status,',
                    'a.certificate_issuer certificate_issuer, a.certificate_subject certificate_subject, a.certificate_valid_from certificate_valid_from, a.certificate_valid_to certificate_valid_to,',
	 ...
	 *
	 */
	this.getAddinVersion = function(sGuid, aVersions) {

		var sAddinStmt = ['select addin_guid, addin_major_version, addin_minor_version, addin_revision_number,addin_build_number,',
	 ...
	 */
	this.getAddinConfiguration = function(sGuid, aVersions,	bUsePreviousVersion) {		
		var oResultAddinConfiguration = getAddinConfigurationForVersion(sGuid, aVersions);
		...
		if (oResultAddinConfiguration === undefined && bUsePreviousVersion === true) {
			...
			var stmt = [	'select addin_guid, addin_major_version, addin_minor_version, addin_revision_number, addin_build_number',
			...
			while (i < oPreviousVersions.length && oResultAddinConfiguration === undefined) {
				var aVersion = [oPreviousVersions[i].ADDIN_MAJOR_VERSION, oPreviousVersions[i].ADDIN_MINOR_VERSION, oPreviousVersions[i].ADDIN_REVISION_NUMBER, oPreviousVersions[i].ADDIN_BUILD_NUMBER ];
				oResultAddinConfiguration = getAddinConfigurationForVersion(sGuid, aVersion);
				i++;
			}
		}
	...
	this.versionExists = function(sGuid, aVersions) {
		var sCountStmt = ['select count(*) as rowcount',
		            'from "' + Tables.version + '"',
		            'where addin_guid = ? and addin_major_version = ? and addin_minor_version = ? and addin_revision_number = ? and addin_build_number = ?',
		            ].join(" ");
	...
	this.register = function(oAddinVersion) {
		...
		var aPropertiesToExclude = ['ADDIN_VERSION', 'STATUS'];
	...
	this.unregister = function(sGuid, aVersions) {
		...
		var stmtVersion = ['delete from "' + Tables.version + '"', sWhere].join(" ");
		dbConnection.executeUpdate(stmtVersion, sGuid, aVersions[0], aVersions[1], aVersions[2], aVersions[3]);

		var stmtConfigHeader = ['delete from "' + Tables.configuration_header + '"', sWhere].join(" ");
		dbConnection.executeUpdate(stmtConfigHeader, sGuid, aVersions[0], aVersions[1], aVersions[2], aVersions[3]);

		var stmtConfigItems = ['delete from "' + Tables.configuration_items + '"', sWhere].join(" ");
	 ...
	 * @returns {object} oConfigurationHeaderToUpdate - updated configuration header entity
	 */
	this.updateConfigurationItems = function(aConfigItems, sGuid, aVersions) {
		// Remove old configuration items
		var sDeleteStmt = [	'delete from "' + Tables.configuration_items + '"',
		                    'where addin_guid = ? and addin_major_version = ? and addin_minor_version = ? and addin_revision_number = ? and addin_build_number = ?'
		                 ].join(" ");
		...
		_.each(aConfigItems, function(oConfig) {
			...
			aUpdatedConfigurationItems.push({CONFIG_KEY : oConfig.CONFIG_KEY, CONFIG_VALUE : oConfig.CONFIG_VALUE});
		});
	 ...
	 */
	function getAddinConfigurationHeader (sGuid, aVersions) {
		
		// Get addin configuration header
		var sConfigHeaderStmt = [	'select 	addin_guid, addin_major_version, addin_minor_version, addin_revision_number, addin_build_number, ',
									'			created_on, created_by, last_modified_on, last_modified_by',
									'from "' + Tables.configuration_header + '"',
									'where 		addin_guid=? and addin_major_version=? and addin_minor_version=? and addin_revision_number=? and addin_build_number=?;'
								].join(" ");
	...
	function getAddinConfigurationItems(sGuid, aVersions) {
		...
		var sConfigItemsStmt = [	'select config_key, config_value',
	 ...
	 */
	function getAddinConfigurationForVersion(sGuid, aVersions) {
		
		var oConfigHeader = getAddinConfigurationHeader(sGuid, aVersions);		
		if(oConfigHeader === undefined) {
			return undefined;
		}

		// Get configuration items if the header exists
		var oConfigItems = getAddinConfigurationItems(sGuid, aVersions);

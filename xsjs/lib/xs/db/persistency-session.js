...
function Session($, dbConnection, hQuery) {
    ...
    this.getSessionDetails = function(sSessionId, sUserId) {
        ...
        const oSessionDetailsStmt = hQuery.statement(
                `select user_id, session_id, "LANGUAGE", "LAST_ACTIVITY_TIME",
                    seconds_between(LAST_ACTIVITY_TIME, CURRENT_UTCTIMESTAMP) as LIFETIME
                    from "${Tables.session}"
                    where user_id = ? and session_id = ?
        `);
        var aSessionDetailsResult = oSessionDetailsStmt.execute(sUserId, sSessionId);
     ...
     */
    this.isSessionOpened = function(sSessionId, sUserId) {
    	oMessageDetails.addUserObj({id: sUserId});
        ...
        var oCheckSessionStatement = hQuery.statement(['select count(*)  as rowcount from  "' + Tables.session + '"', '  where session_id = ? and user_id = ?'].join(" "));
        var aCount = oCheckSessionStatement.execute(sSessionId, sUserId);
     ...
     */
    this.updateSessionForOpenCalculationVersion = function(sSessionId, sUserId) {
    	oMessageDetails.addUserObj({id: sUserId});
        ...
        oUpdateStatement.execute(sSessionId, sUserId.toUpperCase());
     ...
     */
    this.upsertSession = function(sSessionId, sUserId, sIsoLanguage) {
    	oMessageDetails.addUserObj({id: sUserId});
        if (!_.isString(sSessionId)) {
        	const sLogMessage = "sSessionId must be a valid string.";
        	$.trace.error(sLogMessage);
        ...
        try{
            oUpsertStatement.execute(sSessionId, sUserId, sIsoLanguage.toUpperCase(), new Date(), sUserId);
        } catch(e) {
     ...
     */
    this.deleteSession = function(sSessionId, sUserId) {
    	if (!_.isString(sSessionId)) {
         	const sLogMessage = "sSessionId must be a valid string.";
        	$.trace.error(sLogMessage);
    		throw new PlcException(Code.GENERAL_UNEXPECTED_EXCEPTION, sLogMessage, oMessageDetails);
    	...
    	var oCheckCalVersionStatement = hQuery.statement('select session_id, calculation_version_id, is_writeable from "' 
    			+ Tables.open_calculation_versions + '" where session_id=?');
    	var aOpenCalculation = oCheckCalVersionStatement.execute(sSessionId);
        ...
        var iSessionDeleteResult = sessionDeleteStmt.execute(sSessionId);
        
        //delete locks
        hQuery.statement('delete  from  "' + Tables.lock + '" where user_id = ?').execute(sUserId);
     ...
     */    
    this.updateLastActivity = function(sSessionId, sUserId) {
    	oMessageDetails.addUserObj({id: sUserId});
        if (!_.isString(sSessionId)) {
        	const sLogMessage = "sSessionId must be a valid string.";
        	$.trace.error(sLogMessage);
        ...
        const iAffectedRows = dbConnection.executeUpdate(
            `update "${Tables.session}" set LAST_ACTIVITY_TIME = CURRENT_UTCTIMESTAMP where SESSION_ID = ? and USER_ID = ?`,
            sSessionId, sUserId
        );
        if (iAffectedRows === 0) { 
     ...
     *             whenever sSessionId or sUserId is not a string
     */    
    this.updateLastUserActivity = function( sUserId, sCurrentDate) {
    	oMessageDetails.addUserObj({id: sUserId});
        if (!_.isString(sUserId)) {
        	const sLogMessage = "sUserId must be a valid string.";
        	$.trace.error(sLogMessage);
        ...
        const iAffectedRows = dbConnection.executeUpdate(
          `upsert "${Tables.user_activity}" (USER_ID, LAST_ACTIVITY_TIME)
            values (?, ?) 
   ...
   this.deleteOutdatedEntries = function() {
		...
		dbConnection.executeUpdate(
		        `delete from "${Tables.calculation}" 
	   	...
	   	var bItemExtExists = dbConnection.executeQuery(
	   		`select table_name from "SYS"."TABLES" where schema_name=CURRENT_SCHEMA and table_name=? and is_user_defined_type=?`,
	   		Tables.item_temporary_ext, "FALSE").length === 1;
	   	if (bItemExtExists) {
     ...
     *             whenever sSessionId is not a string
     */ 
  this.clearTemporaryTables = function(sSessionId) {
      ...
      try{
          ...
          deleteGttItemTemporaryStatement.execute(sSessionId);
          ...
          deleteGttItemTemporaryWithMasterDataCustomFieldsStatement.execute(sSessionId);

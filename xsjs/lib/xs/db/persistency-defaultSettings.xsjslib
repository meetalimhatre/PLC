...
var Tables = Object.freeze({
	defaultSettings: "sap.plc.db::basis.t_default_settings"
});

const Procedures = Object.freeze({
	default_settings_read : 'sap.plc.db.defaultsettings.procedures::p_default_settings_read'
});

function DefaultSettings(dbConnection, hQuery, hQueryRepl) {
	var that = this;
	this.helper = new Helper($, hQuery, dbConnection);
	
    function throwWrongSettingException(){
     ...
     */
    function checkDefaultSettingsEntity(oDefaultSettings){
	 ...
	 */
	this.get = function(sType, sUserId, sLanguage, sMasterDataDate) {
		var oReturnObject = {};	
		var iEntryExists = checkEntryExists(sType, sUserId);
	 ...
	 */
	this.create = function(oDefaultSettings, sType, sUserId) {	
			
		var oResult = {};
		checkDefaultSettingsEntity(oDefaultSettings);
		...
		checkAllRelevantMasterdataEntriesExist(oEntity);
 	 ...
 	 */
 	this.removeDefaultSettings = function(sType, sUserId){

		switch(sType) {
			case "global": {
				const oDeleteStatement = hQuery.statement('delete from "' + Tables.defaultSettings + '" where USER_ID = \'\'');
				oDeleteStatement.execute();
				break;
			}
			case "user": {
				const oDeleteStatement = hQuery.statement('delete from "' + Tables.defaultSettings + '" where USER_ID = ?');
				oDeleteStatement.execute(sUserId);
				break;
			}
			default: {
			    throwWrongSettingException();
	 ...
	 */
	this.update = function(oDefaultSettings, sType, sUserId) {		
		...
		switch(sType) {
			case "global": {
				...
				iAffectedRows = oUpdateStatement.execute(oDefaultSettings.CONTROLLING_AREA_ID, oDefaultSettings.COMPANY_CODE_ID, oDefaultSettings.PLANT_ID, oDefaultSettings.REPORT_CURRENCY_ID, oDefaultSettings.COMPONENT_SPLIT_ID, oDefaultSettings.COSTING_SHEET_ID);
				break;
			}
			case "user": {
				const oUpdateStatement = hQuery.statement('update "' + Tables.defaultSettings + '" set CONTROLLING_AREA_ID = ?, COMPANY_CODE_ID = ?, PLANT_ID = ?, REPORT_CURRENCY_ID = ?, COMPONENT_SPLIT_ID = ?, COSTING_SHEET_ID = ? where USER_ID = ?');
				iAffectedRows = oUpdateStatement.execute(oDefaultSettings.CONTROLLING_AREA_ID, oDefaultSettings.COMPANY_CODE_ID, oDefaultSettings.PLANT_ID, oDefaultSettings.REPORT_CURRENCY_ID, oDefaultSettings.COMPONENT_SPLIT_ID, oDefaultSettings.COSTING_SHEET_ID, sUserId);
				break;
			}
			default: {
	 ...
	 */
	function checkEntryExists(sType, sUserId) {
		var aEntryExists = null;
		
		switch(sType){
			case 'global':
				aEntryExists = hQuery.statement('select count(*) as COUNT from "' + Tables.defaultSettings + '" where USER_ID = \'\'').execute();
				break;
			case 'user':
				aEntryExists = hQuery.statement('select count(*) as COUNT from "' + Tables.defaultSettings + '" where USER_ID = ?').execute(sUserId);
				break;
			default: {
			    throwWrongSettingException();
			}
	 ...
	 * 
	 */
	function checkAllRelevantMasterdataEntriesExist(oDefaultSettings){
		...
		if(!helpers.isNullOrUndefined(oDefaultSettings.CONTROLLING_AREA_ID) && oDefaultSettings.CONTROLLING_AREA_ID !== '') {
			...
			const aFieldsValuesMainPlcTable = apiHelpers.getColumnKeyValues(aFieldsMainPlcTable, oDefaultSettings);

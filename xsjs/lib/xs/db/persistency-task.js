...
var Sequences = Object.freeze({
	task_id : 'sap.plc.db.sequence::s_task_id'
});
...
function Task(dbConnection) {
	 ...
	 *
	 */
	this.get = function(sUserId, sTaskType, iTaskId, sTaskStatus) {				
	...
	this.create = function(oTask) {				
		...
		var iTaskId = _.max([sequenceTaskId, maxTaskId]);
	 ...
	 */
	this.update = (oTask) => {	
		var oUpdateSet = _.omit(oTask, ["TASK_ID", "SESSION_ID"]);
		...
		aValues.push(oTask.TASK_ID); aValues.push(oTask.SESSION_ID);
		try {
			dbConnection.executeUpdate(stmt, [aValues]);
		} catch(e) {
	...
	this.updateInactiveTasksOnTimeout = (sUserId, oProjectParameters)  => {
		...
		try{
			...
			if(!helpers.isNullOrUndefined(oProjectParameters)){
				...
				var sStmt = `UPDATE "` + Tables.task + 
						 `" SET STATUS='CANCELED', STARTED = CURRENT_UTCTIMESTAMP
				...
				dbConnection.executeUpdate(
							sStmt,
							oProjectParameters.TASK_TYPE, 
							oProjectParameters.PARAMETERS,
							iTimeoutSeconds);
			} 
			if(!helpers.isNullOrUndefined(sUserId)){
				var sStmt = `UPDATE "` + Tables.task +  
	...
	this.cancelTasksWithStatusAndLastUpdatedOlderThan = (sStatus, sTaskType, iMinutes) => {	
		...
		_.each(aInactiveTasks, function(oInactiveTask){
			oInactiveTask.STATUS = TaskStatus.CANCELED;
			oInactiveTask.PROGRESS_STEP = 4
			this.update(oInactiveTask);
		}.bind(this));
	};
}

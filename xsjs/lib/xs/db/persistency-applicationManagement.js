...
var Tables = Object.freeze({
	initialization_state : 'sap.plc.db::basis.t_initialization_state'
});
...
function ApplicationManagement($, dbConnection) {
     ...
     */
    this.getApplicationVersion = function(mtaMetadata) {
     ...
     */
    this.isPlcInitialized = function(sVersion) {
    ...
    this.writePlcInitializationState = function(sVersion) {
        ...
        // add new row
        dbConnection.executeUpdate('insert into "' + Tables.initialization_state + '" (plc_version, generation_time) values (?, ?)',
            sVersion, new Date());
    };
}

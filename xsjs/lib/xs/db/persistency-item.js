...
function Item($, dbConnection, hQuery, sUserId) {
	 ...
	 */
	this.get = function (aItemIds, sSessionId, iCvId, bCompressedResult) {
		try {
			...
			if(bCompressedResult === true){
			    return {
			        ITEMS: [],
			        ITEMS_COMPRESSED : helpers.transposeResultArray(oResult.OT_ITEMS)
			    };
			}
				else{
				return {
	 ...
	 *             if any exceptional state during the communication with the database occurs
	 */
	this.getSaveRelevantFields = function (aItemIds, sSessionId, iCvId, iMaxLimit) {
		...
		const aAuditFields = ["ITEM_ID", "CALCULATION_VERSION_ID", "CREATED_ON", "CREATED_BY", "LAST_MODIFIED_ON",
		 	"LAST_MODIFIED_BY", "IS_DIRTY", "PRICE_SOURCE_ID", "PRICE_SOURCE_TYPE_ID" ];
		
		let sStmt =
			 `select ${aAuditFields.join(", ")}
	 ...
	 *
	 */
	function updateSuccessor(oItem, sSessionId) {
		var iCalcVersionId = helpers.getValueOnKey(oItem, 'CALCULATION_VERSION_ID');
		var parentItemId = helpers.getValueOnKey(oItem, 'PARENT_ITEM_ID');
		var predecessorItemId = helpers.getValueOnKey(oItem, 'PREDECESSOR_ITEM_ID');
		var iItemId = helpers.getValueOnKey(oItem, 'ITEM_ID');
		var iIsDirty = 1;

		var sUpdatePredecessorStatement = 'update "sap.plc.db::basis.t_item_temporary" set predecessor_item_id = ?, is_dirty = ? ' +
			'where session_id = ? and calculation_version_id = ? and parent_item_id = ? ' + 'and item_id<>?';
		...
		if (predecessorItemId === null) {
			oUpdatePredecessorStatement.execute(iItemId, iIsDirty, sSessionId, iCalcVersionId, parentItemId, iItemId);
		} else {
			oUpdatePredecessorStatement.execute(iItemId, iIsDirty, sSessionId, iCalcVersionId, parentItemId, iItemId, predecessorItemId);
		}
	}
	 ...
	 */
	this.update = function (oItem, sSessionId, bUpdateOnlyStandardFields, iOldCategoryId, iNewCategoryId) {
		...
		if (!_.has(oItem, "ITEM_ID") || !_.has(oItem, "CALCULATION_VERSION_ID")) {
			const sLogMessage = "Missing mandatory properties ITEM_ID and CALCULATION_VERSION_ID for given item object. " +
				"These properties are primary keys of t_item_temporary and MUST be contained in the given object.";	
			$.trace.error(sLogMessage);
			throw new PlcException(Code.GENERAL_UNEXPECTED_EXCEPTION, sLogMessage);
		}
		...
		const oUpdateSet = _.omit(oItem, ["ITEM_ID", "CALCULATION_VERSION_ID", "SESSION_ID"]);
		...
		if (_.has(oItem, "PREDECESSOR_ITEM_ID")) {
			...
			updateSuccessor(oItem, sSessionId);
		}
	...
	function executeItemMassUpdate(aItems, sTableName, sSessionId, aUpdateFields) {
		...
		aItems.forEach(item => {
			...
			row.push(item.ITEM_ID),
			row.push(item.CALCULATION_VERSION_ID);
			row.push(sSessionId);
	 ...
	 */
	this.massUpdate = function (aItems, sSessionId) {
		...
		const oUpdateSet = _.omit(aItems[0], ["ITEM_ID", "CALCULATION_VERSION_ID", "SESSION_ID"]);
		const aColumnNames = _.keys(oUpdateSet);
		...
		// update standard fields
		if (aStandardFields.length > 0) {
			executeItemMassUpdate(aItems, Tables.item_temporary, sSessionId, aStandardFields);
		}

		// update custom fields
		if (aCustomFields.length > 0) {
			executeItemMassUpdate(aItems, Tables.item_temporary_ext, sSessionId, aCustomFields);
		}
	};
	 ...
	 */
	function updateItemTable(aStandardFields, oItem, sSessionId) {
	 ...
	 */
	function upsertItemExtensionTable(aCustomFields, oItem, sSessionId) {
		...
		var iAffectedRows = upsertStmt.execute(aValuesCust);
	 ...
	 *            sSessionId - The session id with which the item to update is associated
	 */
	this.updateReferencedCalculationVersionID = function (aItemToUpdateReferencedVersion, sSessionId) {
	 ...
	 */
	this.automaticValueDetermination = function (aItems, iCvId, sSessionId, bImport, bReevaluate) {
	 ...
	 *             if any exceptional state during the communication with the database occurs
	 */
	this.create = function (aItems, sSessionId, iCvId, iImport, iSetDefaultValues, iUpdateMasterDataAndPrices, bCompressedResult) {
				...
				_.each(aTableColumns, function (sColumnName) {
					...
					if (sColumnName === "SESSION_ID") {
						...
						if(iIndex!==-1){
					        ...
					        aCustomFieldItems[sColumnName].push({ITEM_ID : oItem.ITEM_ID, VALUE: oItem[sColumnName]});
						}
						aItemValues.push(oItem[sColumnName]);
					} else {
		...
		if(Object.keys(aCustomFieldItems).length > 0){
		    aItems = _.filter(aItems, item => { return _.some(aMasterdataCustomFields, sProperty => _.has(item, sProperty)) === true; });//this is done in order to avoid unnecessary updates for items with no custom field set in the request
	 ...
	 */
	this.deleteItems = function (sSessionId, iCalculationVersionID) {
		oMessageDetails.addCalculationVersionObjs({
			id: iCalculationVersionID
		});
	 ...
	 *
	 */
	this.hasItemChildren = function (oItem, sSessionId) {
		oMessageDetails.addCalculationVersionObjs({
			id: oItem.CALCULATION_VERSION_ID
		});
		oMessageDetails.addItemObjs({
			id: oItem.ITEM_ID
		});
		...
		const sStmt = `select top 1 item_id 
		                ...
		                where parent_item_id = ? 
		                    ...
		                    and is_deleted = 0
		`;
		var aDirectChildrenIds = hQuery.statement(sStmt).execute(oItem.ITEM_ID, oItem.CALCULATION_VERSION_ID, sSessionId);
	 ...
	 *             if any exceptional state during the communication with the database occurs
	 */
	this.markItemForDeletion = function (sSessionId, oBodyItem, bDeleteRoot) {
		var result = {};
	...
	this.setActiveStates = function(aItems, iCvId, sSessionId) {
		...
		var aValues = _.map(aItems, function(oItem){
			return [oItem.ITEM_ID, oItem.PARENT_ITEM_ID, oItem.IS_ACTIVE];
		});
	 ...
	 *
	 */
	this.insertChangedItemIdForAFL = function(aItems) {

		var aValues = _.map(aItems, function(oItem){
			return [oItem.ITEM_ID, null];
		});
	 ...
	 *
	 */
	this.getPriceSourceBySourceType = function (iPriceSourceType) {
		var sStmt = ['select top 1 price_source_id from "' + Tables.price_source + '"  where price_source_type_id = ?'].join(" ");
		var aPriceSourceIds = hQuery.statement(sStmt).execute(iPriceSourceType);
	...
	this.getFormulasAndRollupsForStandardAndCustomFields = function() {
		...
		const oAllFields = dbConnection.executeQuery(
			`select distinct meta.column_id, attr.item_category_id, rollup_type_id, is_formula_used 
	...
	*/
	this.getParentItemIds = function(iCvId, sSessionId) {
		const oResult = dbConnection.executeQuery(
	...
	*/
	this.getValidItemIds = function(aItemIds, iCalculationVersionId, sSessionId) {
		if (aItemIds.length === 0) return aItemIds; // nothing to to if aItemIds is empty
		...
		const oResult = dbConnection.executeQuery(
			`select distinct item_id from "${Tables.item_temporary}"
			 where calculation_version_id = ? and session_id = ? and is_deleted = 0
			 and item_id in (${aItemIds.join(",")})`,
			iCalculationVersionId, sSessionId);

...
async function Metadata($, hQuery, dbConnection, sUserId) {
    ...
    this.getDisplayOrder = async function (oMeta, oOldMeta) {
        ...
        var aMaxValue = await oCheckStatement.execute(oMeta.PATH, oMeta.BUSINESS_OBJECT, helpers.isNullOrUndefined(oOldMeta) ? oMeta.SIDE_PANEL_GROUP_ID : oOldMeta.SIDE_PANEL_GROUP_ID);
    ...
    this.updateFormulas = function (oMeta) {
        ...
        _.each(oMeta.FORMULAS, async oFormula => {
            checkIfFormulaStringIsEmpty(oFormula);
    ...
    this.removeLayoutData = async function () {
    ...
    this.updateManualFieldForStandardFields = async function (oMetaStandardField) {
    ...
    this.updateFieldWithDefaultValue = async oMetaTriggerChange => {
    ...
    this.copyItemsToItemExt = async function () {
    ...
    this.copyMasterdataToMasterdataExt = async sBusinessObject => {
    ...
    this.deleteCustomFieldEntriesFromReplicationTool = async function (aCustomFieldsToDelete) {

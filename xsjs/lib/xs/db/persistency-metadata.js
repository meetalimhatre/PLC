...
const Procedures = Object.freeze({
	checkFormula: 'sap.plc.db.calcengine.procedures::p_check_formulas'
});

const BusinessObjectTables = new Map([
	[MasterDataObjectTypes.ActivityPrice, 't_activity_price'],
	[MasterDataObjectTypes.CostCenter, 't_cost_center'],
	[MasterDataObjectTypes.Material, 't_material'],
	[MasterDataObjectTypes.MaterialPrice, 't_material_price'],
	[MasterDataObjectTypes.MaterialPlant, 't_material_plant'],
	[MasterDataObjectTypes.WorkCenter, 't_work_center']
]);

const aMetadataReadOnlyProperties = [ "PATH", "BUSINESS_OBJECT", "COLUMN_ID", "IS_CUSTOM", "UOM_CURRENCY_FLAG", "SEMANTIC_DATA_TYPE", "SEMANTIC_DATA_TYPE_ATTRIBUTES",
	"IS_IMMUTABLE_AFTER_SAVE", "IS_REQUIRED_IN_MASTERDATA", "IS_WILDCARD_ALLOWED", "IS_USABLE_IN_FORMULA",
	"REF_UOM_CURRENCY_PATH", "REF_UOM_CURRENCY_BUSINESS_OBJECT", "REF_UOM_CURRENCY_COLUMN_ID" ];
const aMetadataItemAttributeReadOnlyProperties = ["PATH", "BUSINESS_OBJECT", "SUBITEM_STATE", "IS_MANDATORY",
	"IS_READ_ONLY", "IS_TRANSFERABLE", "COLUMN_ID", "LANGUAGE", "CREATED_ON", "CREATED_BY"];
const aFormulaReadOnlyProperties = [ "FORMULA_ID", "PATH", "BUSINESS_OBJECT", "COLUMN_ID", "ITEM_CATEGORY_ID" ];


function Metadata($, hQuery, dbConnection, sUserId) {
	var that = this;
	...
	this.getMetadataFields = function(sPath, sBusinessObject, sColumnId, bIsCustom) {
		...
		var aStmtBuilder = [ 'SELECT a.PATH, a.BUSINESS_OBJECT, a.COLUMN_ID, a.IS_CUSTOM, a.ROLLUP_TYPE_ID, a.SIDE_PANEL_GROUP_ID,'
		...
		+ ' b.VALIDATION_REGEX_VALUE FROM "' + Tables.metadata + '" as a left outer join "sap.plc.db::basis.t_regex" as b on a.VALIDATION_REGEX_ID = b.VALIDATION_REGEX_ID' ];
	...
	this.getMetadataText = function(sPath, sColumnId) {
		...
		var aStmtBuilder = [ 'select PATH, COLUMN_ID, LANGUAGE, DISPLAY_NAME, DISPLAY_DESCRIPTION, CREATED_ON, CREATED_BY,'
		+ ' LAST_MODIFIED_ON, LAST_MODIFIED_BY from "' + Tables.metadataText + '" ' ];
	...
	this.getMetadataItemAttributes = function(sPath, sBusinessObject, sColumnId) {
		...
		var aStmtBuilder = [ 'select PATH, BUSINESS_OBJECT, COLUMN_ID, ITEM_CATEGORY_ID, SUBITEM_STATE, IS_MANDATORY,'
		+'IS_READ_ONLY, IS_TRANSFERABLE, DEFAULT_VALUE, CREATED_ON, CREATED_BY,'
		+ ' LAST_MODIFIED_ON, LAST_MODIFIED_BY from "' + Tables.metadataItemAttributes + '" ' ];
	 ...
	 */
	this.getColumnsForCategories = function(sPath, sBusinessObject) {
		...
		var result = stmt.execute(sPath, sBusinessObject);
		return result;
	};
	...
	this.getMetadataFormulas = function(sPath, sBusinessObject, sColumnId) {
		...
		var aStmtBuilder = [ 'select FORMULA_ID, PATH, BUSINESS_OBJECT, COLUMN_ID, ITEM_CATEGORY_ID, IS_FORMULA_USED,'
		+ ' FORMULA_STRING, FORMULA_DESCRIPTION from "' + Tables.formula + '" ' ];
	...
	this.getMetadataSelectionFilter = function(sPath, sBusinessObject, sColumnId) {
		...
		var aStmtBuilder = [ 'select PATH, BUSINESS_OBJECT, COLUMN_ID, FILTER_PATH, FILTER_BUSINESS_OBJECT, FILTER_COLUMN_ID from "'
		+ Tables.metadataSelectionFilter + '" ' ];
	...
	this.getMetadataSelectionDisplayed = function(sPath, sBusinessObject, sColumnId) {
		...
		var aStmtBuilder = [ 'select PATH, BUSINESS_OBJECT, COLUMN_ID, DISPLAY_ORDER, DISPLAYED_PATH, DISPLAYED_BUSINESS_OBJECT, DISPLAYED_COLUMN_ID from "'
		+ Tables.metadataSelectionDisplayed + '" ' ];
	 ...
	 */
	this.buildWhereClauseAndExecuteStatement = function(sPath, sBusinessObject, sColumnId, bIsCustom, aStmtBuilder, sWhereClause) {
		var aMetadataResult = [];
		...
		if(sBusinessObject === 'Masterdata' && bIsCustom) {
			...
			var aMasterdataBusinessObjects = ['Material', 'Material_Price', 'Material_Plant', 'Cost_Center', 'Work_Center', "Activity_Price"];
		...
		if (!helpers.isNullOrUndefined(bIsCustom)) {
			...
			whereClauseParams.push(helpers.boolToInt(bIsCustom));
		}
		aStmtBuilder.push(' ORDER BY COLUMN_ID ');
		oGetMetadataStatement = hQuery.statement(aStmtBuilder.join(""));
		aMetadataResult = oGetMetadataStatement.execute(whereClauseParams);
	...
	this.create = function(oMeta) {
		...
		if (oMeta.UOM_CURRENCY_FLAG === 0 && !helpers.isNullOrUndefinedOrEmpty(oMeta.REF_UOM_CURRENCY_PATH) &&
			!helpers.isNullOrUndefinedOrEmpty(oMeta.REF_UOM_CURRENCY_BUSINESS_OBJECT) && !helpers.isNullOrUndefinedOrEmpty(oMeta.REF_UOM_CURRENCY_COLUMN_ID)) {
	...
	this.createMetadata = function(oMeta, oRefMeta) {
		...
		var oSettings = {
			...
			PROPERTIES_TO_EXCLUDE : [ "TEXT", "ATTRIBUTES", "FORMULAS" ],
	 ...
	 * @returns {object} object that contains all generated values
	 */
	this.getDisplayOrder = function(oMeta, oOldMeta) {
		var bIsCustom = helpers.isNullOrUndefined(oOldMeta) ? 1 : oOldMeta.IS_CUSTOM;
		var displayOrder = null;
		var oCheckStatement = hQuery.statement('select top 1 MAX(DISPLAY_ORDER) as MAX_DISPLAY_ORDER, IS_CUSTOM from "' + Tables.metadata
			+ '" where path = ? and business_object = ? and side_panel_group_id = ? ' +
			' group by IS_CUSTOM order by MAX(DISPLAY_ORDER) desc ');
		var aMaxValue = oCheckStatement.execute(oMeta.PATH, oMeta.BUSINESS_OBJECT, helpers.isNullOrUndefined(oOldMeta) ? oMeta.SIDE_PANEL_GROUP_ID : oOldMeta.SIDE_PANEL_GROUP_ID);

		var iMaxValue = (helpers.isNullOrUndefined(aMaxValue[0]) || helpers.isNullOrUndefined(aMaxValue[0].MAX_DISPLAY_ORDER)) ? null : parseInt(aMaxValue[0].MAX_DISPLAY_ORDER, 10);
		var bIsCustomPrevious = (helpers.isNullOrUndefined(aMaxValue[0]) || helpers.isNullOrUndefined(aMaxValue[0].IS_CUSTOM)) ? null : aMaxValue[0].IS_CUSTOM;
	 ...
	 * @returns {object} object that contains all generated values
	 */
	this.getTableDisplayOrder = function(oMeta) {
		var tableDisplayOrder = null;
		var oCheckStatement = hQuery.statement('select top 1 MAX(TABLE_DISPLAY_ORDER) as MAX_TABLE_DISPLAY_ORDER, IS_CUSTOM from "' + Tables.metadata
			+ '" where path = ? and business_object = ? '
			+ ' group by IS_CUSTOM order by MAX(TABLE_DISPLAY_ORDER) desc ');
		var aMaxValue = oCheckStatement.execute(oMeta.PATH, oMeta.BUSINESS_OBJECT);

		var iMaxValue = (helpers.isNullOrUndefined(aMaxValue[0]) || helpers.isNullOrUndefined(aMaxValue[0].MAX_TABLE_DISPLAY_ORDER)) ? null : parseInt(aMaxValue[0].MAX_TABLE_DISPLAY_ORDER, 10);
		var bIsCustomPrevious = (helpers.isNullOrUndefined(aMaxValue[0]) || helpers.isNullOrUndefined(aMaxValue[0].IS_CUSTOM)) ? null : aMaxValue[0].IS_CUSTOM;
	 ...
	 *             if the given arguments are of wrong type
	 */
	this.checkMetadataExists = function(oMeta) {
		var sPath = oMeta.PATH;
		var sObject = oMeta.BUSINESS_OBJECT;
		var sColumn = oMeta.COLUMN_ID;

		var oCheckStatement = hQuery.statement('select count(*) as rowcount from "' + Tables.metadata
	...
	this.update = function(oMeta) {
		...
		// check if referenced UOM exists
		if (oMeta.UOM_CURRENCY_FLAG === 0 && !helpers.isNullOrUndefined(oMeta.REF_UOM_CURRENCY_PATH) &&
			!helpers.isNullOrUndefined(oMeta.REF_UOM_CURRENCY_BUSINESS_OBJECT) && !helpers.isNullOrUndefined(oMeta.REF_UOM_CURRENCY_COLUMN_ID) &&
			oMeta.REF_UOM_CURRENCY_PATH !== "" && oMeta.REF_UOM_CURRENCY_BUSINESS_OBJECT !== "" && oMeta.REF_UOM_CURRENCY_COLUMN_ID !== "") {
			refMetadata = that.getRefMetadata(oMeta);
			if (!helpers.isNullOrUndefined(refMetadata)) {
				var oMessageDetails = new MessageDetails();
				oMessageDetails.addMetadataObjs(oMeta);
		...
		var oCheckStatement = hQuery.statement('select PATH, BUSINESS_OBJECT, COLUMN_ID, IS_CUSTOM, ROLLUP_TYPE_ID, SIDE_PANEL_GROUP_ID,'
			+ ' DISPLAY_ORDER, TABLE_DISPLAY_ORDER, REF_UOM_CURRENCY_PATH, REF_UOM_CURRENCY_BUSINESS_OBJECT,'
			+ ' REF_UOM_CURRENCY_COLUMN_ID, UOM_CURRENCY_FLAG, SEMANTIC_DATA_TYPE, SEMANTIC_DATA_TYPE_ATTRIBUTES,'
			+ ' PROPERTY_TYPE, IS_IMMUTABLE_AFTER_SAVE, IS_REQUIRED_IN_MASTERDATA, IS_WILDCARD_ALLOWED,'
			+ ' IS_USABLE_IN_FORMULA, RESOURCE_KEY_DISPLAY_NAME, RESOURCE_KEY_DISPLAY_DESCRIPTION,'
			+ ' CREATED_ON, CREATED_BY, LAST_MODIFIED_ON, LAST_MODIFIED_BY, VALIDATION_REGEX_ID'
			+ ' from "' + Tables.metadata
			+ '" where path = ? and business_object = ? and column_id = ?');
		var aMetadata = oCheckStatement.execute(sPath, sObject, sColumn);
	 ...
	 *             if the given arguments are of wrong type
	 */
	this.updateMetadata = function(oMeta, oMetadataOld) {
		var oSettings = {
			TABLE : Tables.metadata,
			PROPERTIES_TO_EXCLUDE : aMetadataReadOnlyProperties.concat(["TEXT", "ATTRIBUTES", "FORMULAS"]),
			GENERATED_PROPERTIES : _.omit(oAuditValues, ['CREATED_ON', 'CREATED_BY'])
		};
		...
		updateStmt.execute(aMetaValues);
	...
	this.upsertMetadataText = function(oMeta) {
		...
		_.each(oMeta.TEXT, function(oTextMeta) {
			var aKeyNames = [ "PATH", "COLUMN_ID", "LANGUAGE" ];
			var aKeyValues = [ oTextMeta.PATH, oTextMeta.COLUMN_ID, oTextMeta.LANGUAGE ];
			var aMetaNames = aKeyNames.concat([ "DISPLAY_NAME", "DISPLAY_DESCRIPTION" ], (_.keys(oSettingsText.GENERATED_PROPERTIES)));
			var aMetaValues = aKeyValues.concat([ oTextMeta.DISPLAY_NAME || "", oTextMeta.DISPLAY_DESCRIPTION || ""], (_.values(oSettingsText.GENERATED_PROPERTIES)));
			...
			checkCFTextIsValid(oMeta, aMetaNames, aMetaValues, ["DISPLAY_NAME", "DISPLAY_DESCRIPTION"]);

			//check if the entry already exited and get the creation date and user
			var oExistingValues = hQuery.statement(
				'SELECT created_on, created_by ' +
				' FROM "' + oSettingsText.TABLE + '"' +
				' WHERE path = ? and column_id = ? and language = ?'
			).execute(aKeyValues);
			...
			oUpsertStatement.execute(aMetaValues.concat(aKeyValues));
	...
	this.updateMetadataAttributes = function(oMeta, oMetadataOld) {
		...
		if (oMetadataOld.IS_CUSTOM === 1 && oMeta.ROLLUP_TYPE_ID !== oMetadataOld.ROLLUP_TYPE_ID) {
			_.each(oMeta.ATTRIBUTES, function(oAttrMeta) {
				var oCheckStatement = hQuery.statement('delete from "' + oSettingsAttributes.TABLE
					+ '" where path = ? and business_object = ? and column_id = ? and item_category_id = ?');
				oCheckStatement.execute(oAttrMeta.PATH, oAttrMeta.BUSINESS_OBJECT, oAttrMeta.COLUMN_ID, oAttrMeta.ITEM_CATEGORY_ID);
			});
		...
		_.each(oMeta.ATTRIBUTES, function(oAttrMeta) {
			...
			var oCheckStatement = hQuery.statement('select count(*) as rowcount from "' + oSettingsAttributes.TABLE
				+ '" where path = ? and business_object = ? and column_id = ? and item_category_id = ?');
			var aAttributesCount = oCheckStatement.execute(oAttrMeta.PATH, oAttrMeta.BUSINESS_OBJECT, oAttrMeta.COLUMN_ID, oAttrMeta.ITEM_CATEGORY_ID);
			...
			if (iAttributesCount > 0) {
				...
				var updateStmt = hQuery.statement(aStmtBuilder.join(" "));
				var iAffectedRows = updateStmt.execute(aMetaValues);
				if (iAffectedRows === 0) {
					var oMessageDetails = new MessageDetails();
					oMessageDetails.addMetadataObjs(oMeta);
				...
				var oSelectStatement = hQuery.statement('select PATH, BUSINESS_OBJECT, COLUMN_ID, ITEM_CATEGORY_ID, SUBITEM_STATE, IS_MANDATORY,'
					+ ' IS_READ_ONLY, IS_TRANSFERABLE, DEFAULT_VALUE, CREATED_ON, CREATED_BY,'
					+ ' LAST_MODIFIED_ON, LAST_MODIFIED_BY from "' + oSettingsAttributes.TABLE
					+ '" where path = ? and business_object = ? and column_id = ? and item_category_id = ?');
				var aAttributesUpdated = oSelectStatement.execute(oAttrMeta.PATH, oAttrMeta.BUSINESS_OBJECT, oAttrMeta.COLUMN_ID, oAttrMeta.ITEM_CATEGORY_ID);
	...
	checkIfFormulaStringIsEmpty = (oFormula) => {
		if(_.isEmpty(oFormula.FORMULA_STRING) && oFormula.IS_FORMULA_USED === 1){
			...
			throw new PlcException(FormulaInterpreterError[8], sServerMsg, oMessageDetails); //mapped to CALCULATIONENGINE_SYNTAX_ERROR_WARNING(see message.xsjslib: FormulaInterpreterError)
		}

		/** Add missing properties to oFormula required for procedure check */
	...
	/**
	 * Use this function to check if the text of a custom fields contains
	 * illegal characters (only & for now). If that's the case throw an error.
	 * @params {object}		oMeta
	 * 			- custom field to be created/updated
	 * @params {array}		aMetaNames
	 * 			- column names from t_metadata__text
	 * @params {array}		aMetaValues
	 * 			- corresponding values for aMetaNames
	 * @parms  {array}		aKeys
	 * 			- the columns from aMetaNames to be checked
	 * 
	 * @throws {PlcException} if the text contains illegal chars
	 * 			- CUSTOM_FIELDS_TEXT_ERROR (400 Bad Request)
	 */
	...
	/**
	 * update formulas for a given metadata object in in t_formula table
	 *
	 * @param {object}
	 *            oMeta - each object of the array from request with the properties of the new metadata
	 *
	 * @returns {array} - updated formulas
	 * @throws {PlcException}
	 *             if the given arguments are of wrong type
	 */
	this.updateFormulas = function(oMeta) {
		...
		var aFormulas = [];
		/*
		 * this array will be used to trigger change of IS_MANUAL generated by the DbController field
		 * it is only filled for newly created formulas or updated one that change IS_FORMULA_USED field
		*/
		...
		_.each(oMeta.FORMULAS, (oFormula) => {
			checkIfFormulaStringIsEmpty(oFormula);
			var oCheckStatement = hQuery.statement('select FORMULA_ID, PATH, BUSINESS_OBJECT, COLUMN_ID, ITEM_CATEGORY_ID,'
				+ ' IS_FORMULA_USED, FORMULA_STRING, FORMULA_DESCRIPTION from "' + oSettingsFormulas.TABLE
				+ '" where path = ? and business_object = ? and column_id = ? and item_category_id = ?');
			var aFormulaResults = oCheckStatement.execute(oFormula.PATH, oFormula.BUSINESS_OBJECT, oFormula.COLUMN_ID, oFormula.ITEM_CATEGORY_ID);
			if (aFormulaResults.length > 0) {
				...
				// add in array that triggers IS_MANUAL field update this entry if IS_FORMULA_USED has changed
				...
				// add in array that triggers IS_MANUAL field update this entry for new formulas
				aFormulasTriggerIsManualChange.push(oFormula);
			}
		});
	...
	/**
	 * Delete an existing metadata in database, also the UOM custom field attached
	 *
	 * @param {object}
	 * 			 oMeta - object that must be deleted
	 * @returns {int} rowCount - returns 1 in case of deletion
	 * @throws {PlcException}
	 *             if the given arguments are of wrong type
	 * @throws {PlcException}
	 *             if errors occur during deletion from the tables
	 */
	this.remove = function(oMeta) {
		var sPath = oMeta.PATH;
		var sObject = oMeta.BUSINESS_OBJECT;
		var sColumnId = oMeta.COLUMN_ID;

		var stmt = hQuery.statement('select PATH, BUSINESS_OBJECT, COLUMN_ID, IS_CUSTOM, ROLLUP_TYPE_ID, SIDE_PANEL_GROUP_ID,'
			+ ' DISPLAY_ORDER, TABLE_DISPLAY_ORDER, REF_UOM_CURRENCY_PATH, REF_UOM_CURRENCY_BUSINESS_OBJECT,'
			+ ' REF_UOM_CURRENCY_COLUMN_ID, UOM_CURRENCY_FLAG, SEMANTIC_DATA_TYPE, SEMANTIC_DATA_TYPE_ATTRIBUTES,'
			+ ' PROPERTY_TYPE, IS_IMMUTABLE_AFTER_SAVE, IS_REQUIRED_IN_MASTERDATA, IS_WILDCARD_ALLOWED,'
			+ ' IS_USABLE_IN_FORMULA, RESOURCE_KEY_DISPLAY_NAME, RESOURCE_KEY_DISPLAY_DESCRIPTION,'
			+ ' CREATED_ON, CREATED_BY, LAST_MODIFIED_ON, LAST_MODIFIED_BY, VALIDATION_REGEX_ID'
			+ ' from "' + Tables.metadata
			+ '" where PATH = ? and BUSINESS_OBJECT = ? and COLUMN_ID = ? and IS_CUSTOM = 1');
		var aMetadata = stmt.execute(sPath, sObject, sColumnId);
		var iRowCount = aMetadata.length;

		if (iRowCount > 0) {
			try {
				// remove related UOM field in case oMeta is not a UOM already
				if (!helpers.isNullOrUndefined(aMetadata[0].REF_UOM_CURRENCY_PATH) && !helpers.isNullOrUndefined(aMetadata[0].REF_UOM_CURRENCY_BUSINESS_OBJECT) &&
					!helpers.isNullOrUndefined(aMetadata[0].REF_UOM_CURRENCY_COLUMN_ID)) {
				...
				stmt.execute(sPath, sObject, sColumnId);
				//delete texts
				stmt = hQuery.statement('delete from "' + Tables.metadataText + '" where PATH = ? and COLUMN_ID = ?');
				stmt.execute(sPath, sColumnId);
				//delete attributes
				stmt = hQuery.statement('delete from "' + Tables.metadataItemAttributes
					+ '" where PATH = ? and BUSINESS_OBJECT = ? and COLUMN_ID = ?');
				stmt.execute(sPath, sObject, sColumnId);
				// delete metadata custom field
	...
	/**
	 * Delete all entries from layout tables that are not found in metadata
	 * It is triggered when some custom fields are deleted,
	 * they don't exist anymore in t_metadata but exist in layout tables
	 */
	...
	/**
	 * deletes related UOM custom field.
	 *
	 * 	 @param {object}
	 * 			 oMeta - object that must be deleted
	 *   @throws {PlcException}
	 *             if deletion failed
	 */
	this.removeUOMRelated = function(oMeta) {
	...
	/**
	 * check if a field is used in costing sheet overhead row formula
	 * 
	 * @param {oMeta} - object that must be checked if it is used in formula
	 * @returns true if field is used in one or more formulas, false if it is not
	 */
	...
	/**
	 * check if a field is used in costing sheet overhead row overhead custom 
	 * 
	 * @param @param {oMeta} -object that must be checked if it is used as overhead custom
	 * @returns -array containing name of custom field if it is used as overhead custom in valid costing sheet,else an empty one
	 */
	...
	/**
	 * check if a field is used in formula
	 *
	 * @param {oMeta} - object that must be checked if it is used in formula
	 * @returns true if field is used in one or more formulas, false if it is not
	 */
	this.checkIsUsedInFormula = function(oMeta) {
		...
		var aResults = oCheckStatement.execute(sRegularExpression, oMeta.PATH, oMeta.BUSINESS_OBJECT);
		return aResults;
	};

	/**
	 * updates CUTOM_FIELD_NAME_IS_MANUAL generated by DBController field, with IS_FORMULA_USED value
	 *
	 * @param {oFormula} - object that must be used to update CUTOM_FIELD_NAME_IS_MANUAL field
	 * @returns
	 */
	this.updateIsManualField = function(oFormula) {
		var oUpdateStatement = hQuery.statement("update itemExt set itemExt."
			+ oFormula.COLUMN_ID + "_IS_MANUAL = ? "
			+ " from \"" + Tables.itemExt + "\" itemExt, \"" + Tables.item + "\" item "
			+ " where item.ITEM_ID = itemExt.ITEM_ID and item.CALCULATION_VERSION_ID = itemExt.CALCULATION_VERSION_ID "
			+ " and item.ITEM_CATEGORY_ID = ?");
		var aResults = oUpdateStatement.execute( oFormula.IS_FORMULA_USED, oFormula.ITEM_CATEGORY_ID);
		return aResults;
	};

	/**
	 * updates CUSTOM_FIELD_NAME_UNIT generated by DBController field, with value
	 * taken from calculation version or from metadata_attributes if the custom field
	 * is either currency(in case if it's a currency for item custom field) or unit of measure(in all other cases)
	 * @param {oMetaTriggerUnitChange} - object that must be used to update CUTOM_FIELD_NAME_UNIT field
	 * @returns
	 */
	this.updateUnitField = function(oMetaTriggerUnitChange) {
		/***
		 * get unit related to this custom field
		 */
		let oRefMeta, oUpdateStatement, aResults;
		if (oMetaTriggerUnitChange.UOM_CURRENCY_FLAG === 0 && !helpers.isNullOrUndefinedOrEmpty(oMetaTriggerUnitChange.REF_UOM_CURRENCY_PATH) &&
			!helpers.isNullOrUndefinedOrEmpty(oMetaTriggerUnitChange.REF_UOM_CURRENCY_BUSINESS_OBJECT) && !helpers.isNullOrUndefinedOrEmpty(oMetaTriggerUnitChange.REF_UOM_CURRENCY_COLUMN_ID)) {
		...
		if (!helpers.isNullOrUndefinedOrEmpty(oRefMeta)) {
			...
			if(oMetaTriggerUnitChange.PATH === constants.BusinessObjectTypes.Item && oMetaTriggerUnitChange.BUSINESS_OBJECT === constants.BusinessObjectTypes.Item && oRefMeta.PROPERTY_TYPE === 7){
				//if it's a custom field for item, of type currency, default value is reporting currency of calculation version
	...
	/**
	 * updates CUTOM_FIELD_NAME_UNIT generated by DBController field, with value
	 * taken from calculation version or from metadata_attributes if the custom field
	 * is either currency or unit of measure
	 * @param {oMetaTriggerUnitChange} - object that must be used to update CUTOM_FIELD_NAME_UNIT field
	 * @returns
	 */
	this.updateManualField = function(oMetaTriggerManualChange) {
		var oUpdateStatement, oUpdateStatementParents, oUpdateStatementChildren, aResults;
		/***
		 * in case of currency must set _UNIT to be calculation version REPORT_CURRENCY_ID
		 */
		if (oMetaTriggerManualChange.ROLLUP_TYPE_ID === 0) {
			oUpdateStatement = hQuery.statement("update itemExt set itemExt."
				+ oMetaTriggerManualChange.COLUMN_ID + "_IS_MANUAL = "
				+ " CASE WHEN (formula.IS_FORMULA_USED IS NULL OR formula.IS_FORMULA_USED = 0 ) THEN 1 ELSE 0 END,  itemExt."
				+ oMetaTriggerManualChange.COLUMN_ID + "_CALCULATED = "
				+ " CASE WHEN (formula.IS_FORMULA_USED IS NULL OR formula.IS_FORMULA_USED = 0 ) THEN NULL ELSE itemExt." + oMetaTriggerManualChange.COLUMN_ID + "_CALCULATED END"
				+ " from \"" + Tables.itemExt + "\" itemExt inner join \"" + Tables.item + "\" item "
				+ " on item.CALCULATION_VERSION_ID = itemExt.CALCULATION_VERSION_ID and item.ITEM_ID = itemExt.ITEM_ID left outer join "
				+ "(select DISTINCT IS_FORMULA_USED, ITEM_CATEGORY_ID, PATH,BUSINESS_OBJECT,COLUMN_ID from \"" + Tables.formula + "\" ) formula on item.ITEM_CATEGORY_ID = formula.ITEM_CATEGORY_ID "
				+ " and formula.PATH = ? and formula.BUSINESS_OBJECT = ? and formula.COLUMN_ID = ? where item.ITEM_CATEGORY_ID in (select distinct ITEM_CATEGORY_ID from \""
				+ Tables.metadataItemAttributes + "\" where PATH=? and BUSINESS_OBJECT=? and COLUMN_ID=? ) ");
			aResults = oUpdateStatement.execute(oMetaTriggerManualChange.PATH, oMetaTriggerManualChange.BUSINESS_OBJECT, oMetaTriggerManualChange.COLUMN_ID, oMetaTriggerManualChange.PATH, oMetaTriggerManualChange.BUSINESS_OBJECT, oMetaTriggerManualChange.COLUMN_ID);
			return aResults;
		} else {
			// set _IS_MANUAL = 0 for all the parents
			oUpdateStatementParents = hQuery.statement("update itemExt set itemExt."
				+ oMetaTriggerManualChange.COLUMN_ID + "_MANUAL = null,  itemExt."
				+ oMetaTriggerManualChange.COLUMN_ID + "_IS_MANUAL = 0 "
				+ " from \"" + Tables.itemExt + "\" itemExt, \"" + Tables.item + "\" item "
				+ " where item.CALCULATION_VERSION_ID = itemExt.CALCULATION_VERSION_ID "
				+ " and item.ITEM_ID = itemExt.ITEM_ID "
				+ " and item.ITEM_CATEGORY_ID in (select distinct ITEM_CATEGORY_ID from \""
				+ Tables.metadataItemAttributes + "\" where PATH=? and BUSINESS_OBJECT=? and COLUMN_ID=? ) "
				+ " and (item.ITEM_ID, item.calculation_version_id) in (select distinct PARENT_ITEM_ID, calculation_version_id from \"" + Tables.item + "\" where PARENT_ITEM_ID is not null ) ");
			var aResultsParents = oUpdateStatementParents.execute(oMetaTriggerManualChange.PATH, oMetaTriggerManualChange.BUSINESS_OBJECT, oMetaTriggerManualChange.COLUMN_ID);

			/* don't update _MANUAL field anymore
			 * - BCP bug 1680278435, Fix in V2.0.2: Manual values after changing the formula
			 * We should never change existing values in the table because this would lead to the fact that not only
			 * working versions but also frozen versions are adapted.
			 * The default value should really only be used if new items added – either manually, via Add-In or via Import.
			 * */
			oUpdateStatementChildren = hQuery.statement("update itemExt set itemExt."
				+ oMetaTriggerManualChange.COLUMN_ID + "_IS_MANUAL =  CASE WHEN (formula.IS_FORMULA_USED IS NULL OR formula.IS_FORMULA_USED = 0 ) THEN 1 ELSE 0 END "
				+ " from \"" + Tables.itemExt + "\" itemExt inner join \"" + Tables.item + "\" item "
				+ " on item.CALCULATION_VERSION_ID = itemExt.CALCULATION_VERSION_ID and item.ITEM_ID = itemExt.ITEM_ID left outer join "
				+ "(select DISTINCT IS_FORMULA_USED, ITEM_CATEGORY_ID, PATH,BUSINESS_OBJECT,COLUMN_ID from \"" + Tables.formula + "\" ) formula on item.ITEM_CATEGORY_ID = formula.ITEM_CATEGORY_ID "
				+ " and formula.PATH = ? and formula.BUSINESS_OBJECT = ? and formula.COLUMN_ID = ? where item.ITEM_CATEGORY_ID in (select distinct ITEM_CATEGORY_ID from \""
				+ Tables.metadataItemAttributes + "\" where PATH=? and BUSINESS_OBJECT=? and COLUMN_ID=? ) "
				+ " and (item.ITEM_ID, item.calculation_version_id) not in (select distinct PARENT_ITEM_ID, calculation_version_id from \"" + Tables.item + "\" where PARENT_ITEM_ID is not null ) ");

			var aResultsChildren = oUpdateStatementChildren.execute(oMetaTriggerManualChange.PATH, oMetaTriggerManualChange.BUSINESS_OBJECT, oMetaTriggerManualChange.COLUMN_ID, oMetaTriggerManualChange.PATH, oMetaTriggerManualChange.BUSINESS_OBJECT, oMetaTriggerManualChange.COLUMN_ID);
	...
	/**
	 * updates STANDARD_FIELD_NAME_IS_MANUAL for all items from t_item,
	 * with 1 in case of formula is disabled for that item category for all leaf items
	 * let the same value manual or calculated in case of formula is enabled for that item category (maybe value was set to manual before) for all leaf items
	 * with 0 for all parent items
	 * @param {oMetaStandardField} - object that contains information about standard field that
	 * @returns
	 */
	this.updateManualFieldForStandardFields = function(oMetaStandardField) {
		// set _IS_MANUAL = 1 for standard fields that can have formulas, with no formula defined or formula not used
		// except the price fields (ROLLUP_TYPE_ID = 100)
		const sUpdateStatement =
			`update item 
		...
		// set _IS_MANUAL = 0 for fields PRICE_FIXED_PORTION, PRICE_VARIABLE_PORTION and PRICE_UNIT for the parent items
		const sUpdateStatementParents =
			`update item 
		...
		// set _IS_MANUAL = 1 for prices that are on leaf items and have no formula
		const sUpdateStatementChildren =
			`update item 
			set item.${MapStandardFieldsWithFormulas.get(oMetaStandardField.COLUMN_ID)} = 1
			from "sap.plc.db::basis.t_item" item
	...
	/**
	 * Updates CUSTOM_FIELD_NAME_MANUAL generated by DBController field, with value
	 * taken from default value of the custom field from metadata_attributes,
	 * CUSTOM_FIELD_NAME_IS_MANUAL is set to 1 (for business_object = item)
	 * @param {oMetaTriggerChange} - object that must be used to update CUSTOM_FIELD_NAME_MANUAL / CUSTOM_FIELD_NAME_IS_MANUAL field
	 * @returns
	 */
	...
	/**
	 * inserts all items that are in t_item and not in item_ext
	 *
	 * @returns
	 */
	...
	/**
	 * Inserts all masterdata(e.g. cost center, material) that are in t_<masterdata> and not in t_<masterdata>_ext
	 * @param {sBusinessObject} - business object name
	 * @returns
	 */
	...
	/**
	 * validates formula strings, throw PLC error with specific code if string is not correct
	 * see FormulaInterpreterErrorMapping from message.xsjslib for error codes possible
	 * @param {oFormula} - object that must be used to update CUTOM_FIELD_NAME_IS_MANUAL field
	 * @returns
	 */
	this.validateFormulaString = function(oFormula) {

		var dbConnection = that.getConnection();
		var checkFormulaProcedure = dbConnection.loadProcedure(Procedures.checkFormula);
		
		var result = checkFormulaProcedure();
		//NOTE: result.ERRORS is an array in XSA (but is an object and with length property in XSC)
		if (_.isArray(result.ERRORS) && result.ERRORS.length > 0 ) {
			...
			//this code is used to transform formulaUsed.FORMULA_ERROR from array of objects to string and use it in log message
	...
	this.getConnection = function() {
		return hQuery.getConnection();
	};

	/**
	 * creates DBArtefactController and call generateAllFilesExt
	 */
	this.createDeleteAndGenerate = function(){
		var oDBController = new DbArtefactController($, that.getConnection());
		oDBController.generateAllFilesExt();
	};

	/**
	 * set transaction autocommit off for the current connection
	 */
	this.setTransactionAutocommitDDLOff = function() {
		var connection = this.getConnection();
		connection.executeUpdate("SET TRANSACTION AUTOCOMMIT DDL off");
	};

	/**
	 * Get all custom fields that have rollup and do not have formula for an item category
	 *
	 * @param sPath
	 *            {string} - path is used to navigate through the client side models (business objects)
	 * @param sBusinessObject
	 ...
	 *
	 * @returns {array} aFieldsWithRollup - an array containing all custom fields that have rollup and do not have formula for an item category
	 */
	this.getRollupCustomFieldsWithoutFormulas = function(sPath, sBusinessObject, iItemCategory) {

		var aFieldsWithRollup = [];
		var oStatement = hQuery.statement('select distinct meta.column_id '
			+ ' from "' + Tables.metadata + '" as meta '
			+ ' inner join "' + Tables.metadataItemAttributes + '" as attr '
			+ ' on meta.business_object = attr.business_object '
			+ ' and meta.path = attr.path '
			+ ' and meta.column_id = attr.column_id '
			+ ' where meta.path = ? and meta.business_object = ? and meta.is_custom=1 '
			+ ' and (meta.uom_currency_flag IS NULL or meta.uom_currency_flag <> 1) '
			+ ' and meta.rollup_type_id <> 0 and attr.item_category_id = ? '
			+ ' minus '
			+ ' select distinct meta.column_id '
			+ ' from "' + Tables.metadata + '" as meta '
			+ ' inner join "' + Tables.formula + '" as formula '
			+ ' on meta.path=formula.path '
			+ ' and meta.business_object=formula.business_object '
			+ ' and meta.column_id=formula.column_id '
			+ ' where meta.path = ? and meta.business_object = ? and meta.is_custom=1 '
			+ ' and (meta.uom_currency_flag IS NULL or meta.uom_currency_flag <> 1 ) '
			+ ' and formula.is_formula_used = 1 and meta.rollup_type_id <> 0 and formula.item_category_id = ? ' );

		aFieldsWithRollup = oStatement.execute(sPath, sBusinessObject, iItemCategory, sPath, sBusinessObject, iItemCategory);

		return aFieldsWithRollup;
	};

	/**
	 * creates DBArtefactController and call generateAllFiles
	 */
	...
	/**
	 * Function to get metadata for an array.
	 *
	 * @param aBody
	 *            {array} - array which contain metadata ids which should be retrieved from the table
	 * @returns {array} aMetadataResult - an array containing data from t_metadata
	 */
	...
	/**
	 * Function to get all custom fields names as an array.
	 * @returns {array} aAllCustomFields - an array containing all custom fields names
	 */
	...
	/**
	 * Function to returns true if a formula with the specified formula string exists.
	 * @returns {boolean}  - a formula using the provided string exists
	 */
	...
	/**
	 * Function which created the required entries in the `t_field_mapping` table when a new custom field is created.
	 * This is required for the replication tool in order to match all the fields of the business object
	 * 		- the entry `CF_NAME_MANUAL` is created for all custom fields
	 * 		- the entry `CF_NAME_UNIT` is created only for those custom fields which also have a unit, e.g. Currency, UOM
	 * @param {array} aCustomFieldsToCreate - the array containing all the successfuly create custom fields which 
	 * 										  needs to be also added in the `t_field_mapping` table
	 */
	...
	/**
	 * Function which deletes the entries of the deleted custom fields from the `t_field_mapping` table.
	 * @param {array} aCustomFieldsToDelete - the array containing the successfuly deleted custom fields
	 */
	this.deleteCustomFieldEntriesFromReplicationTool = function(aCustomFieldsToDelete) {
		if (aCustomFieldsToDelete.length > 0) {
			...
			/**
			 * In order to avoid doing separated queries for each custom fields which needs to be deleted, 
			 * it's better to create a bigger `where` clause and put all the conditions for the deletion process.
			 */

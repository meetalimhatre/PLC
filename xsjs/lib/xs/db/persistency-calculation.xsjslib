 ...
 */
function Calculation(dbConnection, hQuery) {
			...
			get: () => {
				return (() => {
					if (undefined === this._calculationVersion) {
						...
						this._calculationVersion = new CalculationVersion($, dbConnection, hQuery, $.getPlcUsername());
					}
			...
			get: () => {
				return (() => {
					if (undefined === this._project) {
						...
						this._project = new Project(dbConnection, hQuery);
					}
	...
	function checkCalculationIdIsPositiveInteger(iCalculationId){
	 ...
	 */
	this.get = function(aProjects, sUserId, aCalculationIds, itopPerProject, sSearchCriteria, iTopCalculations) {
		...
		var sStmt = 
		    `select top ? CALCULATION_ID, PROJECT_ID, CALCULATION_NAME, CURRENT_CALCULATION_VERSION_ID, CREATED_ON, CREATED_BY, LAST_MODIFIED_ON, LAST_MODIFIED_BY, CALCULATION_VERSION_NO from 
		...
		var aResult = [];
		try {
			aResult = selectStmt.execute(aValues);
		} catch (e) {
	 ...
	 * @returns {object} output - An object that contains all found calculations
	 */
	this.getSaved = function(aProjects, sUserId, aCalculationIds, itopPerProject, sSearchCriteria, iTopCalculations) {
		if(helpers.isNullOrUndefined(itopPerProject)){
			itopPerProject = 100;
        }
	 ...
	 */
	this.remove = function(iCalculationId) {
	    checkCalculationIdIsPositiveInteger(iCalculationId);
	 ...
	 */
	this.create = function(oCalculation, sSessionId, sUserId) {
		...
		_.each(oCalculation.CALCULATION_VERSIONS, function(oCalculationVersion, iIndex) {
			...
			if (oCalculationVersion.CALCULATION_VERSION_ID < 0) {
				...
				result = that.calculationVersion.create(oCalculationVersion, sSessionId, sUserId);
			}
		});
	 ...
	 */
	this.update = function(oCalculation) {
		...
		var iAffectedRows = 0;
		try {
			iAffectedRows = updateStmt.execute(aValues);
		} catch (e) {
	 ...
	 */
	this.getOpeningUsersForVersions = function(iCalculationId) {
	    checkCalculationIdIsPositiveInteger(iCalculationId);
		const sStmt = `
	 ...
	 */
	this.isNameUnique = function(oCalculation) {
        ...
        if (_.isString(oCalculation.PROJECT_ID)) {
			...
			aCount = oCheckNameStatement.execute(oCalculation.CALCULATION_NAME, oCalculation.CALCULATION_ID, oCalculation.PROJECT_ID);
		}else{
			aStmtBuilder.push('project_id is null');
			oCheckNameStatement = hQuery.statement(aStmtBuilder.join(" "));
			aCount = oCheckNameStatement.execute(oCalculation.CALCULATION_NAME, oCalculation.CALCULATION_ID);
		}
	 ...
	 */
	this.getCurrentCalculationData = function(iCalculationId){
	    checkCalculationIdIsPositiveInteger(iCalculationId);
		
		var aStmtBuilder = [ 'select calculation_id, project_id, calculation_name, current_calculation_version_id, created_on,' 
        ...
        var aCalculation = getCalculationStatement.execute(iCalculationId);
	 ...
	 */
	this.createCalculationAsCopy = function(iCalculationVersionId, sTargetProjectId, sSessionId, sUserId, sLanguage) {
	 ...
	 */
	this.getUniqueNameForCalculation = function(sCalculationName, iCalculationId) {
		...
		var oSplitedCalculationName = helpers.splitIncrementalString(sCalculationName);
	 ...
	 * @returns {integer} - the number of affected rows
	 */
	this.isCurrentVersion = function(iCalculationVersionId) {
	 ...
	 * @returns {integer} - the number of affected rows
	 */
	this.getFrozenVersions = function(iCalculationId) {
	    checkCalculationIdIsPositiveInteger(iCalculationId);
	    
		try {

			var oStatement = hQuery.statement('select calculation_version_id from "' + Tables.calculation_version + '"'
												 + ' where is_frozen = 1 and calculation_id = ? ');
			var aResult = oStatement.execute(iCalculationId);	
	 ...
	 */
	this.getSourceVersionsWithMasterVersionsFromDifferentCalculations = function(iCalculationId) {
	    checkCalculationIdIsPositiveInteger(iCalculationId);
		...
		try {
			...
			var oStatement = hQuery.statement('select distinct calcVers.CALCULATION_VERSION_ID from "' + Tables.calculation_version + '" calcVers inner join ' +
					'(select REFERENCED_CALCULATION_VERSION_ID, CALCULATION_VERSION_ID from "' + Tables.item + 
					'" union select REFERENCED_CALCULATION_VERSION_ID, CALCULATION_VERSION_ID from "' + Tables.item_temporary + 
					'") item on item.REFERENCED_CALCULATION_VERSION_ID = calcVers.CALCULATION_VERSION_ID and calcVers.CALCULATION_ID = ? inner join "' + Tables.calculation_version + 
					'" calcVers1 on item.CALCULATION_VERSION_ID = calcVers1.CALCULATION_VERSION_ID and calcVers1.CALCULATION_ID != ?');
        	var aCalculationVersions = oStatement.execute(iCalculationId, iCalculationId);

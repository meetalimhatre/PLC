...
var Procedures = Object.freeze({
	costing_sheet_read : 'sap.plc.db.administration.procedures::p_costing_sheet_read'
});

function CostingSheet(dbConnection, hQuery, hQueryRepl) {
	...
	var costingSheetRow = new CostingSheetRow(dbConnection, hQuery, hQueryRepl);
	 ...
	 */
	this.removeCostingSheetRow = function(oCostingSheet,sMasterDataDate){
	    ...
	    var oResult = apiHelpers.removeRow(oCostingSheet,sMasterDataDate,oConfiguration,hQuery);

		//delete entries from costing sheet row
		var aCostingSheetRowRecords = apiHelpers.findValidEntriesInTable(Resources[sCostingSheetRow].dbobjects.plcTable, [oConfiguration.aPartialKeyPlcTableColumns[0]], [oCostingSheet.COSTING_SHEET_ID], sMasterDataDate, hQuery);
		...
		var aCostingSheetDependenciesRecords = apiHelpers.findValidEntriesInTable(Resources[sCostingSheetRowDependencies].dbobjects.plcTable, [oConfiguration.aPartialKeyPlcTableColumns[0]], [oCostingSheet.COSTING_SHEET_ID], sMasterDataDate, hQuery);
		if (aCostingSheetDependenciesRecords.length > 0) {
		    var iDeletedTextRecords = apiHelpers.updateEntriesWithValidToInTable(Resources[sCostingSheetRowDependencies].dbobjects.plcTable, [oConfiguration.aPartialKeyPlcTableColumns[0]], [oCostingSheet.COSTING_SHEET_ID], sMasterDataDate, hQuery);
	 ...
	 * @returns {object} oResult          - deleted entry
	 */
	this.removeCostingSheetTextRow = function(oCostingSheetText,sMasterDataDate){
		...
		return apiHelpers.removeTextRow(oCostingSheetText,sMasterDataDate,oConfiguration,hQuery);	
	...
	this.update = function(oBatchItems, sMasterDataDate){
        ...
        var oResult = costingSheetRow.update(oBatchItems, sMasterDataDate);
     ...
     */ 
    this.updateCostingSheetRow = function(oCostingSheet,sMasterDataDate){
    	var oConfiguration = {
				sObjectName                : sCostingSheet,
				aPartialKeyPlcTableColumns : ["COSTING_SHEET_ID"]
		};

  	  var oResult = apiHelpers.updateRow(oCostingSheet,sMasterDataDate,oConfiguration,hQuery,this.helper);
	 ...
	 * @returns {object} oResult          - updated entry
	 */
	this.updateCostingSheetTextRow = function(oCostingSheetText,sMasterDataDate){
		...
		return apiHelpers.updateTextRow(oCostingSheetText,sMasterDataDate,oConfiguration,hQuery,this.helper);
	 ...
	 */
	this.insertCostingSheetRow = function(oCostingSheet,sMasterDataDate){
		...
		var oResult = apiHelpers.insertRow(oCostingSheet,sMasterDataDate,oConfiguration,hQuery,hQueryRepl,this.helper);
	 ...
	 * @returns {object} oResult          - inserted entry
	 */
	this.insertCostingSheetTextRow = function(oCostingSheetText,sMasterDataDate){
		...
		return apiHelpers.insertTextRow(oCostingSheetText,sMasterDataDate,oConfiguration,hQuery,this.helper);

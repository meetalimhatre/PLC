...
var Procedures = Object.freeze({
	account_group_read : 'sap.plc.db.administration.procedures::p_account_group_read'
});
...
function AccountGroup(dbConnection, hQuery, hQueryRepl) {
	...
	var oConfigurationRanges = {
			...
			aPartialKeyPlcTableColumns : ["FROM_ACCOUNT_ID", "ACCOUNT_GROUP_ID"]
	};
	...
	this.remove = function(oBatchItems, sMasterDataDate){
		...
		_.each(aBatchAccGroupsItems, function(oRecord){ 
			try {
				apiHelpers.checkColumns(oRecord, aMetadataGroupFields);
	...
	this.removeAllAccountRangesforAccountGroup = function(oRecord, sMasterDataDate){
      ...
      */
	this.removeAccountGroupsRow = function(oAccountGroups, sMasterDataDate){
		
        var oResult = apiHelpers.removeRow(oAccountGroups, sMasterDataDate, oConfigurationGroups, hQuery);

    	//delete entries from table t_account_account_group (Account Ranges)
        var aRangeRecords = apiHelpers.findValidEntriesInTable(Resources[sBusinessObjectNameRanges].dbobjects.plcTable, [oConfigurationGroups.aPartialKeyPlcTableColumns[0]], [oAccountGroups.ACCOUNT_GROUP_ID], sMasterDataDate, hQuery);
        if (aRangeRecords.length !== 0){
        	var iDeletedRecords = apiHelpers.updateEntriesWithValidToInTable(Resources[sBusinessObjectNameRanges].dbobjects.plcTable, [oConfigurationGroups.aPartialKeyPlcTableColumns[0]], [oAccountGroups.ACCOUNT_GROUP_ID], sMasterDataDate, hQuery);
	 ...
	 * @returns {object} oResult          - deleted entry
	 */
	this.removeAccountGroupTextRow = function(oAccountGroupText,sMasterDataDate){

		return apiHelpers.removeTextRow(oAccountGroupText,sMasterDataDate,oConfigurationGroups,hQuery);
	...
	this.insert = function(oBatchItems, sMasterDataDate){
		...
		oResult.entities[BusinessObjectsEntities.ACCOUNT_GROUP_ENTITIES] = [];
		_.each(aBatchAccountGroupItems, function(oRecord){ 
			try {
				apiHelpers.checkColumns(oRecord, aMetadataGroupFields);
		...
		_.each(aBatchAccRangesItems, function(oRecord){ 
			try {
     ...
     */
	this.insertAccountGroupRow = function(oAccountGroup, sMasterDataDate) {

		var oResult = apiHelpers.insertRow(oAccountGroup, sMasterDataDate, oConfigurationGroups, hQuery, hQueryRepl, this.helper);
	...
	this.insertAccountGroupTextRow = function(oAccountGroupText, sMasterDataDate) {

		return apiHelpers.insertTextRow(oAccountGroupText, sMasterDataDate, oConfigurationGroups, hQuery, this.helper);

	};

	this.insertAccountRangeRow = function(oAccountRange, sMasterDataDate) {

		var exceptionInfo = "Inconsistent or missing data. See log for details";
		var aFieldsValuesAccountGroupTable = apiHelpers.getColumnKeyValues(oConfigurationRanges.aPartialKeyPlcTableColumns, oAccountRange);
		var aFoundAccountGroupRecords = apiHelpers.findValidEntriesInTable(Resources[sBusinessObjectNameGroups].dbobjects.plcTable, [oConfigurationRanges.aPartialKeyPlcTableColumns[1]], [aFieldsValuesAccountGroupTable[1]], sMasterDataDate, hQuery);
		...
		var oResult = apiHelpers.insertRow(oAccountRange, sMasterDataDate, oConfigurationRanges, hQuery, hQueryRepl, this.helper);		
	 ...
	 * @param   {string} sMasterDataDate  - master data timestamp
	 */
	this.checkCreateReferenceObjects = function(oAccountGroup, sMasterDataDate) {
		...
		var aFieldsValuesMainPlcTable = apiHelpers.getColumnKeyValues(aFieldsMainPlcTable, oAccountGroup);
	...
	this.update = function(oBatchItems, sMasterDataDate){
		...
		oResult.entities[BusinessObjectsEntities.ACCOUNT_RANGES_ENTITIES] = [];
		_.each(aBatchAccountRangeItems, function(oRecord){ 
			try {
		...
		if((oResult.hasErrors==false)&&(aChangedObjectsKeys.length > 0)){
 			...
 			var oCopiedObjects = apiHelpers.copyUnchangedRows(aChangedObjectsKeys, oConfigurationGroups, sMasterDataDate, hQuery, this.helper);
     ...
     */ 
	this.updateAccountGroupRow = function(oAccountGroup, sMasterDataDate) {

		var oResult = apiHelpers.updateRow(oAccountGroup, sMasterDataDate, oConfigurationGroups, hQuery, this.helper);
	...
	this.updateAccountGroupTextRow = function(oAccountGroupText, sMasterDataDate) {

		return apiHelpers.updateTextRow(oAccountGroupText, sMasterDataDate, oConfigurationGroups, hQuery, this.helper);

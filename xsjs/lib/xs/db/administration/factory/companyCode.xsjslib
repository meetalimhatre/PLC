...
function CompanyCode(dbConnection, hQuery, sObjectName) {
	...
	CompanyCode.prototype.getDataUsingSqlProcedure = function(fnProcedure, oProcedureParameters){
    	var oReturnObject = {};
    	var oFilters = [
    	    ["COMPANY_CODE_CURRENCY_ID", "plcTable.COMPANY_CODE_CURRENCY_ID"],
    	    ["COMPANY_CODE_DESCRIPTION", "plcTextTable.COMPANY_CODE_DESCRIPTION"],
    	...
    	if(oProcedureParameters.bAutocompleteIsNullOrUndefined === true) {
            ...
            oReturnObject[BusinessObjectsEntities.CONTROLLING_AREA_ENTITIES] = ConstrollingArea.getControllingAreaEntities(dependentBusinessObject.CONTROLLING_AREA_ID, masterdataTimestamp, sLanguage, dbConnection);
    	}
...
*/
function getCompanyCodeEntitiesForControllingArea(sControllingAreaId, masterdataTimestamp, dbConnection) {
...
/**
     ...
     * @param   {object} dbConnection - the db connection used
     */    
function getCompanyCodeEntities(aCompanyCodesIds, masterdataTimestamp, sLanguage, dbConnection) {

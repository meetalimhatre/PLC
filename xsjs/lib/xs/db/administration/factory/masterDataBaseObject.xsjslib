...
function MasterDataBaseObject(dbConnection, hQuery, sObjectName, sIgnoreBadData) {
	 ...
	 */
	MasterDataBaseObject.prototype.get = function(oGetParameters, sLanguage, sMasterDataDate) {
	...
	MasterDataBaseObject.prototype.getDataUsingSqlProcedure = function(fnProcedure, oProcedureParameters){
		return {}; //to be redefined in the objects that will inherit this base object, if needed
	};
     ...
     */
	MasterDataBaseObject.prototype.determineCurrentOperations = function(oBatchItems){
	...
	MasterDataBaseObject.prototype.registerValidationTemplates = function(){
		...
		if(_.includes(this.currentMainOperations, Operation.CREATE) || _.includes(this.currentMainOperations, Operation.UPDATE)
				|| _.includes(this.currentMainOperations, Operation.UPSERT)){
	 ...
	 */
	MasterDataBaseObject.prototype.registerProcessTemplates = function(){
		...
		if(_.includes(this.currentMainOperations, Operation.DELETE)&&(hasBusinessObjectText(sObjectName))){
		...
		if(_.includes(this.currentTextOperations, Operation.CREATE) || _.includes(this.currentTextOperations, Operation.UPDATE)
				|| _.includes(this.currentTextOperations, Operation.UPSERT)){
	 ...
	 */
	MasterDataBaseObject.prototype.prepareContext = function(){
		this.context.MasterdataBusinessObject = {
			...
			fieldsReadOnly: fillFieldsReadOnlyContextProperty(Resources[sObjectName].configuration.aReadOnlyColumns),
			...
			usedInBusinessObjects: fillUsedInBusinessObjectsContextProperty(Resources[sObjectName].configuration.UsedInBusinessObjects),
			referencedObjects: fillReferencedObjectsContextProperty(Resources[sObjectName].configuration.ReferencedObjects),
	 ...
	 */
	function fillFieldsReadOnlyContextProperty(aReadOnlyColumns){
	 ...
	 */
	function fillUsedInBusinessObjectsContextProperty(aUsedInBusinessObjects){
		...
		_.each(aUsedInBusinessObjects, function(oBusinessObject){
	 ...
	 */
	function fillReferencedObjectsContextProperty(aReferencedObjects){
		var aReferencedObjectsProperty = [];
		
		if(helpers.isNullOrUndefined(aReferencedObjects))
			return aReferencedObjectsProperty;
			
		_.each(aReferencedObjects, function(oBusinessObject){
			var oRefObj = {};
	 ...
	 * @param   {object} oBatchItems - object that comes from the request containing all items that should be created/updated/deleted/upserted
	 */
	MasterDataBaseObject.prototype.validate = function(oBatchItems,sMasterDataDate){
		
		this.determineCurrentOperations(oBatchItems);
		this.registerValidationTemplates();
		this.prepareContext();
		deleteTemporaryTableContent(sObjectName);
		 ...
		 * In case that there is no error, the temporary table will be used as source in order to insert/update the data in the main/text tables.
		 */
        ...
        //Use template engine to generate the sqls needed for validation & execute the queries
		_.each(this.validationTemplates, function(sValidationTemplate,iIndex) {
			generateAndExecuteSql(that.validationTemplatesPackageName, sValidationTemplate, that.context);    
		});
		
		this.validateAfter(oBatchItems);
		
		//If there is at least one error, the response will be prepared.
		var oResultObject = {
			entities:{},
			hasErrors: false,
			errors:[]    	    
		};
		createResponseWithErrors(oResultObject, sObjectName, AdministrationObjType.MAIN_OBJ);
		if(hasBusinessObjectText(sObjectName)){
			createResponseWithErrors(oResultObject, sObjectName, AdministrationObjType.TEXT_OBJ);
		}
		if(sObjectName === BusinessObjectTypes.WorkCenter){
		    if(!helpers.isNullOrUndefined(this.currentProcessOperations)){
		        createResponseWithErrors(oResultObject, BusinessObjectTypes.WorkCenterProcess, AdministrationObjType.MAIN_OBJ);
		    }
		    if(!helpers.isNullOrUndefined(this.currentActivityOperations)){
		        createResponseWithErrors(oResultObject, BusinessObjectTypes.WorkCenterActivity, AdministrationObjType.MAIN_OBJ);
		    }
		}
	...
	/**
	 * General logic in order to modify the data and return the response.
	 */
	MasterDataBaseObject.prototype.process = function(){
		...
		this.processBefore();
         //Use template engine to generate the sqls needed for processing & execute the queries
		_.each(this.processingTemplates, function(sProcessingTemplate,iIndex) {
		...
		createResponseAfterProcessingEntries(oResultObject, sObjectName, AdministrationObjType.MAIN_OBJ, this.context.MasterdataBusinessObject.currentTimestamp, 
				this.currentMainOperations, this.context.MasterdataBusinessObject.fieldsCustom);
		if(hasBusinessObjectText(sObjectName)){
			createResponseAfterProcessingEntries(oResultObject, sObjectName, AdministrationObjType.TEXT_OBJ, this.context.MasterdataBusinessObject.currentTimestamp, 
				this.currentTextOperations, []);
		}
		if(sObjectName === BusinessObjectTypes.WorkCenter){
		    if(!helpers.isNullOrUndefined(this.currentProcessOperations)){
    		    createResponseAfterProcessingEntries(oResultObject, BusinessObjectTypes.WorkCenterProcess, AdministrationObjType.MAIN_OBJ, this.context.MasterdataBusinessObject.currentTimestamp, 
    				    this.currentProcessOperations,[]);	
		    }
		    if(!helpers.isNullOrUndefined(this.currentActivityOperations)){
    		    createResponseAfterProcessingEntries(oResultObject, BusinessObjectTypes.WorkCenterActivity, AdministrationObjType.MAIN_OBJ, this.context.MasterdataBusinessObject.currentTimestamp, 
    				    this.currentActivityOperations,[]);	
		    }
		} 
		return oResultObject;
	};
	...
	/**
	 * For some objects (Material_Price, Activity_Price we should initialize some fields
	 * 
	 *  @param   {object} oRecord - entry that is processed
	 *  @param   {string} sOperation - operation (Create/Update/Delete/Upsert)
	 *  @param   {string} sObjectType - object type (main/text)
	 *  
	 */
	...
	/**
	 * Generate a DB artefact identified by package and template name using the template engine and the context object.
	 * 
	 *  @param   {string} packageName - path for validation/processing templates
	 *  @param   {string} templateName - template name
	 *  @param   {object} oContext - context (used in order to obtain the sql query, by merging the template with context)
	 */
	...
	function checkAndInsertIntoTemporaryTable(oBatchItems, sOperation, sObjectType, oContext){
		...
		_.each(aBatchItems, function(oRecord){ 
			var oEntry = {};
			try {
				checkColumnsUsingMetadata(oRecord, that.aMetadataFields, sOperation, sObjectType);
				that.initializeColumns(oRecord, sOperation, sObjectType);
				that.checkMandatoryProperties(oRecord, sOperation, sObjectType);
				oEntry = oRecord;
	...
	/**
	 * Deletes the content of temporary tables
	 * 
	 * @param   {string} sObjectName - object name
	 */
	function deleteTemporaryTableContent(sObjectName){
	 ...
	 */
	function checkColumnsUsingMetadata(oEntry, aMetadataFields, sOperation, sObjectType){
	 ...
	 *  @param   {string} sObjectType - object type (main/text)
	 */
	 MasterDataBaseObject.prototype.checkMandatoryProperties = function(oEntry, sOperation, sObjectType){
		...
		if(( sOperation === Operation.CREATE || sOperation === Operation.UPDATE || sOperation === Operation.UPSERT) 
				&& (sObjectType === AdministrationObjType.MAIN_OBJ) 
	...
	MasterDataBaseObject.prototype.checkMandatoryNotNullProperties = function(oEntry, aMandatoryProperties, aObjMissingProperties){
		...
		if (aObjMissingProperties.length !== 0 || aObjNullMandatoryProperties.length !== 0){
    		...
    		if (aObjMissingProperties.length !== 0) {
    			...
    			oMsgDetails.validationObj = {"columnIds": aObjMissingProperties, "validationInfoCode": ValidationInfoCode.MISSING_MANDATORY_ENTRY};
    		}
    		else {
    		    sLogMessage = `Null or empty is not allowed for: ${_.map(aObjNullMandatoryProperties,"columnId").join(", ")}`;
    			oMsgDetails.validationObj = {"columnIds": aObjNullMandatoryProperties, "validationInfoCode": ValidationInfoCode.VALUE_ERROR};
    		}
    		$.trace.error(sLogMessage);
	...
	/**
	 * Create a response after processing the entries
	 * 
	 * 	@param   {object} oResultObject - record that is processed
	 *  @param   {string} sOperation - operation (Create/Update/Delete/Upsert)
	 *  @param   {string} sObjectType - object type (main/text)
	 *  @param   {string} sMasterDataDate - masterdata date
	 *  @param   {array} aOperations - array with requested operations
	 *  @param   {array} aCustomFields - array of custom fields
	 */
	function createResponseAfterProcessingEntries(oResultObject, sObjectName, sObjectType, sMasterDataDate, aOperations, aCustomFields) {
	    ...
	    if(_.includes(aOperations, Operation.CREATE)){
	    	oResultObject[BatchOperation.CREATE][sSection] = selectCreatedUpdatedDeletedEntries(sPlcTable, sTempTable, aKeys, Operation.CREATE, sMasterDataDate, aCustomFields);
	    }
	    if(_.includes(aOperations, Operation.UPSERT)){
	    	oResultObject[BatchOperation.UPSERT][sSection] = selectCreatedUpdatedDeletedEntries(sPlcTable, sTempTable, aKeys, Operation.UPSERT, sMasterDataDate, aCustomFields);
	    }
	    if(_.includes(aOperations, Operation.UPDATE)){
	    	oResultObject[BatchOperation.UPDATE][sSection] = selectCreatedUpdatedDeletedEntries(sPlcTable, sTempTable, aKeys, Operation.UPDATE, sMasterDataDate, aCustomFields);
	    }
	    if(_.includes(aOperations, Operation.DELETE)){
	    	oResultObject[BatchOperation.DELETE][sSection] = selectCreatedUpdatedDeletedEntries(sPlcTable, sTempTable, aKeys, Operation.DELETE, sMasterDataDate, aCustomFields);
	    }
		
	}
	
	/**
	 * Select created/updated/deleted items
	 * 
	 * 	@param   {string} sPlcTable - PLC  table
	 *  @param   {string} sOperation - operation (Create/Update/Delete/Upsert)
	 *  @param   {string} sObjectType - object type (main/text)
	 *  @param   {string} sMasterDataDate - masterdata date
	 *  @param   {string} sTempTable - temporary table
	 *  @param   {string} sTempTable - array of custom fields
	 */
	function selectCreatedUpdatedDeletedEntries(sPlcTable, sTempTable, aKeys, sOperation, sMasterDataDate, aCustomFields){		
	 ...
	 *  @param   {string} sOperation - operation (Create/Update/Delete/Upsert)
	 *  @param   {string} sObjectType - object type (main/text)
	 *  @param   {object} oResultObject - result object with errors
	 */
	function createResponseWithErrors(oResultObject, sObjectName, sObjectType) {
		...
		_.each(aErrorEntries, function(oErrorEntry,iIndex){
    		var oResult = {} ;
        	var aEntity = [];
        	var entitySection = {};
        	var oMessageDetails = new MessageDetails(); 
            ...
            if(oErrorEntry.ERROR_DETAILS !== ''){
        	    setErrorDetails(oMessageDetails, oErrorEntry.ERROR_DETAILS);  
            }
		...
		//Setting an error message in trace
		//TODO: In the trace file it is logged only a message that the request has errors. We need to clarify what should be logged and for how many objects(think of creations of 100+ objects,
		//the effect on performance if we log all errors, etc)
	 ...
	 * Set error details
	 * 
	 * @param   {object} oMessageDetails
	 * @param   {object} oErrorDetails 
	 */
	function setErrorDetails(oMessageDetails, oErrorDetails){
	    try{
	...
	/**
	 * Check if a business object is configured to have text (text entitiy/table)
	 * 
	 * @param   {string} sObjectName - object name
	 */
	function hasBusinessObjectText(sObjectName){
	...
	/**
	 * Check if a operation for a business object must be done 
	 * 
	 * 	@param   {object} oBatchItems - object that comes from the request containing all items that should be created/updated/deleted/upserted
	 *  @param   {string} sOperation - operation (Create/Update/Delete/Upsert)
	 *  @param   {string} sObjectType - object type (main/text)
	 *  @param   {string} sObjectName - object name
	 */
	function isOperationRequested(oBatchItems, sObjectName, sOperation, sObjectType){
		...
		if(!helpers.isNullOrUndefined(oBatchItems[sOperation]) && !helpers.isNullOrUndefined(oBatchItems[sOperation][sSection])
		   && (oBatchItems[sOperation][sSection].length > 0))
			return true;

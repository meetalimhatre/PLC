...
async function MasterDataBaseObject(dbConnection, hQuery, sObjectName, sIgnoreBadData) {
    ...
    this.aMetadataFields = await getMetadataForAllFields(this.metadata, sObjectName);
    ...
    MasterDataBaseObject.prototype.validate = async function (oBatchItems, sMasterDataDate) {
        ...
        _.each(oBatchItems, async function (value, key) {
            ...
            if (_.includes(that.currentMainOperations, sOperation))
                await checkAndInsertIntoTemporaryTable(oBatchItems[key], sOperation, AdministrationObjType.MAIN_OBJ, that.context);
            if (_.includes(that.currentTextOperations, sOperation))
                await checkAndInsertIntoTemporaryTable(oBatchItems[key], sOperation, AdministrationObjType.TEXT_OBJ, that.context);
        });
        ...
        _.each(this.validationTemplates, async function (sValidationTemplate, iIndex) {
            await generateAndExecuteSql(that.validationTemplatesPackageName, sValidationTemplate, that.context);
        });
    ...
    MasterDataBaseObject.prototype.process = async function () {
        ...
        _.each(this.processingTemplates, async function (sProcessingTemplate, iIndex) {
            await generateAndExecuteSql(that.processingTemplatesPackageName, sProcessingTemplate, that.context);
        });
        this.processAfter();

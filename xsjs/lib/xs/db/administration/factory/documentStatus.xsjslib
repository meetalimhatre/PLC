...
function DocumentStatus(dbConnection, hQuery, sObjectName) {
	...
	DocumentStatus.prototype.getDataUsingSqlProcedure = function(fnProcedure, oProcedureParameters){
    	var oReturnObject = {};
    	
    	var oFilters = [
    	    ["DOCUMENT_TYPE_ID", "plcTable.DOCUMENT_TYPE_ID"],
    	    ["DOCUMENT_STATUS_DESCRIPTION", "plcTextTable.DOCUMENT_STATUS_DESCRIPTION"],
    	    ["DOCUMENT_STATUS_ID", "plcTable.DOCUMENT_STATUS_ID"],
    	...
    	if(oProcedureParameters.bAutocompleteIsNullOrUndefined === true){
            ...
            oReturnObject[BusinessObjectsEntities.DOCUMENT_TYPE_ENTITIES] = DocumentType.getDocumentTypeEntities(dependentBusinessObject.DOCUMENT_TYPE_ID, masterdataTimestamp, oProcedureParameters, dbConnection);
    	}

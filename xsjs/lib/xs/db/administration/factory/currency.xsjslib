...
function Currency(dbConnection, hQuery, sObjectName) {
    ...
    Currency.prototype.getDataUsingSqlProcedure = async function (fnProcedure, oProcedureParameters) {
        var oReturnObject = {};
    ...
    Currency.prototype.checkMandatoryProperties = async function (oEntry, sOperation, sObjectType) {
        ...
        if ((sOperation === Operation.CREATE || sOperation === Operation.UPDATE) && sObjectType === AdministrationObjType.MAIN_OBJ && !helpers.isNullOrUndefined(Resources[sObjectName].configuration.aOtherMandatoryColumns)) {
...
export default {_,helpers,Resources,BusinessObjectsEntities,MasterDataBaseObject,HelperObjectTypes,MessageLibrary,PlcException,MessageCode,ValidationInfoCode,AdministrationObjType,Operation,Currency};

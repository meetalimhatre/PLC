...
function Plant(dbConnection, hQuery, sObjectName) {
	...
	Plant.prototype.getDataUsingSqlProcedure = function(fnProcedure, oProcedureParameters){
    	var oReturnObject = {}; 	
		var sControllingAreaId = '';
		var sSqlFilter = '';
		
			var oFilters = [
    	    ["PLANT_ID", "plcTable.PLANT_ID"],
    	    ["PLANT_DESCRIPTION", "plcTextTable.PLANT_DESCRIPTION"],
    	    ["COMPANY_CODE_ID", "plcTable.COMPANY_CODE_ID"],
    	    ["COUNTRY", "plcTable.COUNTRY"],
    	    ["POSTAL_CODE", "plcTable.POSTAL_CODE"],
    	    ["REGION", "plcTable.REGION"],
    	    ["CITY", "plcTable.CITY"],
    	    ["STREET_NUMBER_OR_PO_BOX", "plcTable.STREET_NUMBER_OR_PO_BOX"],
    	    ["_SOURCE", "plcTable._SOURCE"]
    	];
    	
    	if(oProcedureParameters.sUrlFilter !== ''){
				var filterObject = apiHelpers.getFilterObjectFromFilterString(oProcedureParameters.sUrlFilter,this.aMetadataFields);
		...
		if(oProcedureParameters.bAutocompleteIsNullOrUndefined === true){
		    var result = fnProcedure(oProcedureParameters.sLanguage, oProcedureParameters.sMasterDataDate,
    				           sSqlFilter,oProcedureParameters.iTopRecords, oProcedureParameters.iSkipRecords);
			...
			if(sControllingAreaId !== ''){
    	        aCompanyCodesIds = CompanyCode.getCompanyCodeEntitiesForControllingArea( sControllingAreaId, masterdataTimestamp, dbConnection);
			}
    			let stmt = `select  
    	                    	plcTable.PLANT_ID,
			...
			oReturnObject[BusinessObjectsEntities.COMPANY_CODE_ENTITIES] = CompanyCode.getCompanyCodeEntities(dependentPlants.COMPANY_CODE_ID, masterdataTimestamp, oProcedureParameters.sLanguage, dbConnection);
		    
		    const dependentCompanyCodes = helpers.transposeResultArrayOfObjects(oReturnObject[BusinessObjectsEntities.COMPANY_CODE_ENTITIES], true);
		    oReturnObject[BusinessObjectsEntities.CONTROLLING_AREA_ENTITIES] = ControllingArea.getControllingAreaEntities(dependentCompanyCodes.CONTROLLING_AREA_ID, masterdataTimestamp, oProcedureParameters.sLanguage, dbConnection);
		}

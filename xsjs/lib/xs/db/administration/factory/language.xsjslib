...
function Language(dbConnection, hQuery, sObjectName) {
	...
	Language.prototype.getDataUsingSqlProcedure = function(fnProcedure, oProcedureParameters){
    	var oReturnObject = {}; 	
    	var result = fnProcedure(oProcedureParameters.sMasterDataDate, 
	...
	Language.prototype.validateBefore = function(){
		
		if(oSession===null){
			oSession = new Session($, dbConnection, hQuery);
		}
		...
		var sqlMainNotSupported = "update temp_table" + 
	    ...
	    "    where temp_table.operation in ('" + Operation.CREATE + "', '" + Operation.UPSERT + "', '" + Operation.DELETE + "')" + 
	    "    and (temp_table.error_code = '' or  temp_table.error_code is null)";

		dbConnection.executeUpdate(sqlMainNotSupported);
		
		var sqlMain = "update temp_table" + 
					" set temp_table.ERROR_CODE = '" + MessageCode.GENERAL_UNEXPECTED_EXCEPTION.code + "'" +  
					" from \"" + Resources[sObjectName].dbobjects.tempTable + "\" as temp_table" + 
				    "    where temp_table.LANGUAGE in ('" + oSessionDetails.language + "','" + sDefaultLanguage + "')" + 
				    "    and temp_table.operation in ('" + Operation.UPDATE + "')" + 
				    "    and (temp_table.error_code = '' or  temp_table.error_code is null)";

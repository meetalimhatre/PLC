...
function Customer(dbConnection, hQuery, sObjectName) {
	...
	Customer.prototype.getDataUsingSqlProcedure = function(fnProcedure, oProcedureParameters){
    	var oReturnObject = {}; 
    	
    		
    	var oFilters = [
    	    ["CUSTOMER_ID", "plcTable.CUSTOMER_ID"],
    	    ["CUSTOMER_NAME", "plcTable.CUSTOMER_NAME"],
    	...
    	if(oProcedureParameters.bAutocompleteIsNullOrUndefined === true) {
        	var result = fnProcedure(oProcedureParameters.sMasterDataDate, oProcedureParameters.sSqlFilter,
	...
	Customer.prototype.validateBefore = function(){
		...
		var sqlMain = "update temp_table" + 
					...
					" temp_table.ERROR_DETAILS = '{\"validationObj\": { \"columnIds\": [{\"columnId\":\"CUSTOMER_ID"+
					"\"}],\"validationInfoCode\": \"" + ValidationInfoCode.VALUE_ERROR + "\"}}'" + 
					" from \"" + Resources[sObjectName].dbobjects.tempTable + "\" as temp_table" + 
				    "    where temp_table.CUSTOMER_ID in ('DELETED')" + 
				    "    and temp_table.operation in ('" + Operation.CREATE + "')" + 

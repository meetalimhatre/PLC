...
function ActivityType(dbConnection, hQuery, sObjectName) {
	...
	ActivityType.prototype.getDataUsingSqlProcedure = function(fnProcedure, oProcedureParameters){
    	var oReturnObject = {}; 	
    	
    	let oFilters = [
    	    ["ACTIVITY_TYPE_ID" , "plcTable.ACTIVITY_TYPE_ID"],
    	    ["CONTROLLING_AREA_ID" , "plcTable.CONTROLLING_AREA_ID"],
			["ACCOUNT_ID" , "plcTable.ACCOUNT_ID"],
			["ACTIVITY_TYPE_DESCRIPTION" , "plcTextTable.ACTIVITY_TYPE_DESCRIPTION"],
	...
	ActivityType.prototype.validateAfter = function(){

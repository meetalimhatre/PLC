...
const Sequences = Object.freeze({
	layout : 'sap.plc.db.sequence::s_layout'
});
...
function Layout(dbConnection, hQuery) {
	...
	this.getLayouts = function(sUserId, iLayoutType) {
		...
		var stmtLayout = 'select layout.LAYOUT_ID, layout.LAYOUT_NAME, layout.IS_CORPORATE, layout.LAYOUT_TYPE, layoutPersonal.IS_CURRENT from "' +
							Tables.layout + '" layout left outer join "' + Tables.layout_personal + '" layoutPersonal on layoutPersonal.LAYOUT_ID = layout.LAYOUT_ID ' +
							'where (layoutPersonal.USER_ID in (select USER_ID from "' + Tables.layout_personal +'" where USER_ID = ?) or layout.IS_CORPORATE = 1) and layout_type = ?';
		var stmtLayoutColumn = 'select column.LAYOUT_ID, column.DISPLAY_ORDER, column.PATH, column.BUSINESS_OBJECT, column.COLUMN_ID, column.COSTING_SHEET_ROW_ID, ' +
							'column.COST_COMPONENT_ID, column.COLUMN_WIDTH from "' + Tables.layout_columns + 
							'" column inner join "' + Tables.layout + '" layout ON column.layout_id=layout.layout_id ' +
							'left outer join "' + Tables.layout_personal + '" personal on column.layout_id=personal.layout_id ' +
							'where personal.user_id=? or layout.is_corporate=1';
		var stmtLayoutHiddenFields = 'select hiddenFields.LAYOUT_ID, hiddenFields.PATH, hiddenFields.BUSINESS_OBJECT, hiddenFields.COLUMN_ID from "' + Tables.layout_hidden_fields + '" hiddenFields '+
								'inner join "' + Tables.layout + '" layout ON hiddenFields.layout_id=layout.layout_id ' +
								'left outer join "' + Tables.layout_personal + '" personal on hiddenFields.layout_id=personal.layout_id ' +
								'where personal.user_id=? or layout.is_corporate=1';
	 ...
	 */
	this.create = function(oLayoutData, sUserId, iIsCorporate) {
	...
	this.createLayoutAdditionalData = function(oLayoutData, iLayoutId) {
		...
		_.each(oLayoutData.LAYOUT_COLUMNS, function (oLayoutColumns) {		
			var oLayoutColumn = [
					iLayoutId, oLayoutColumns.DISPLAY_ORDER, oLayoutColumns.PATH || null,
					oLayoutColumns.BUSINESS_OBJECT || null, oLayoutColumns.COLUMN_ID || null,
					oLayoutColumns.COSTING_SHEET_ROW_ID || null, oLayoutColumns.COST_COMPONENT_ID === undefined ? null: oLayoutColumns.COST_COMPONENT_ID,
		...
		_.each(oLayoutData.HIDDEN_FIELDS, function (oLayoutHiddenField) {
			var oHiddenField = [
			 					iLayoutId, oLayoutHiddenField.PATH, 
			 					oLayoutHiddenField.BUSINESS_OBJECT, oLayoutHiddenField.COLUMN_ID
			 			];
	 ...
	 *             if any exceptional state during the communication with the database occurs
	 */
	this.update = function(oLayoutData, sUserId, iIsCorporate) {
	...
	this.isNameUnique = function(iLayoutId, sLayoutName, sUserId) {
		var sSelectStmt = 'select count(*) as rowcount from "' + Tables.layout +'" where (IS_CORPORATE = 1 and upper(LAYOUT_NAME) = upper(?)) and LAYOUT_ID != ?' +
						  'or (IS_CORPORATE = 0 and upper(LAYOUT_NAME) = upper(?) and layout_id in ' +
						  '(select layout_id from "' + Tables.layout_personal + '" where USER_ID = ?) and LAYOUT_ID != ?)';
	 ...
	 */
	this.deleteLayout = function(iLayoutId, sUserId, iIsCorporate) {
		if(iIsCorporate === 0){  	

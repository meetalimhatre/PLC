...
function registerHelpers(Handlebars) {
	...
	Handlebars.registerHelper('ddl_primaryKeyDefinition', function ddl_primaryKeyDefinition(oBusinessObject) {
		return 'PRIMARY KEY (' +
			_.map(oBusinessObject.primaryKeys, function(value, key) {
				return '"' + key + '"';
			}).join(", ") + ')';
	});
	...
	Handlebars.registerHelper('customFieldsWithTablePrefix', function customFieldsWithTablePrefix(oBusinessObject) {
		var sPlcFields = '';
		_.each(oBusinessObject.customFields, function(value, key){
			sPlcFields += ', plcExtTable.' + key + '_MANUAL' +
				', plcExtTable.' + key + '_UNIT';
			if (!value.isMasterdataField){
	...
	Handlebars.registerHelper('customFieldsDefaultValuesSelect', function defaultValuesCustomFieldsSelect(oBusinessObject) {
		...
		_.each(oBusinessObject.customFields, function(value, key){
			...
			if((value.propertyType === 6 || value.propertyType === 7)){
				...
				sCustomDefaultValues += ' AND :iv_setDefaultValues = 1 ';
				if(value.propertyType === 7   && !value.isMasterdataField )//Currency
	...
	Handlebars.registerHelper('customFieldsOneTimeCost', function defaultValuesCustomFieldsSelect(oBusinessObject) {
		...
		_.each(oBusinessObject.customFields, function(value, key){		
			...
			if (!helpers.isNullOrUndefinedOrEmpty(value.itemCategories) && value.itemCategories.includes(8) && !helpers.isNullOrUndefined(value.defaultValue))	// item category 8 - Variable Item used for One Time Cost			 
			...
			if (!helpers.isNullOrUndefinedOrEmpty(value.itemCategories) && value.itemCategories.includes(8)) {	// item category 8 - Variable Item used for One Time Cost
				if((value.propertyType === 6 || value.propertyType === 7)){				
			...
			if (!value.isMasterdataField) {
				if (!helpers.isNullOrUndefinedOrEmpty(value.itemCategories) && value.itemCategories.includes(8)){ 
					if (!helpers.isNullOrUndefinedOrEmpty(value.itemCategoriesFormula) && value.itemCategoriesFormula.includes(8)) // item category 8 - Variable Item used for One Time Cost
	...
	Handlebars.registerHelper('updateAllCustomFields_referencedCvId', function condCustomFields(oBusinessObject) {
		...
		_.each(oBusinessObject.customFields, function(value, key){
			if (value.isMasterdataField){
				sCustomFieldsCalculated += ' ' + key + '_MANUAL = source_root_item_ext.' + key + '_MANUAL,' + 
				                           ' ' + key + '_UNIT = source_root_item_ext.' + key + '_UNIT';
			}else{
				sCustomFieldsCalculated += ' ' + key + '_CALCULATED = source_root_item_ext.' + key + '_CALCULATED,' + 
										   ' ' + key + '_MANUAL = CASE  WHEN  source_root_item_ext.' + key + '_IS_MANUAL = 1 THEN source_root_item_ext.' + key + '_MANUAL ' +
										   ' ' + 'ELSE source_root_item_ext.' + key + '_CALCULATED END,' +
										   ' ' + key + '_UNIT = source_root_item_ext.' + key + '_UNIT,' + 
										   ' ' + key + '_IS_MANUAL = 1';
			}
			iIndex++;

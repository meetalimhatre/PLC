...
const Procedures = Object.freeze({
    data_protection_update: "sap.plc.db.management.procedures::p_data_protection_update_user_ids"
});

const Views = Object.freeze({
    data_protection_display_info: "sap.plc.db.views::V_DATA_PROTECTION_DISPLAY_INFO",
});
...
function DataProtection(dbConnection) {
    ...
    *
    */
    this.removeReferencesToUserIds = function(sUserId) {
        const procedure = dbConnection.loadProcedure(Procedures.data_protection_update);
        var oResult = procedure(sUserId);
        return oResult.OV_AFFECTED_ROWS;
    };

    this.deleteInstanceBasedUserIds = function(sUserId, bJobAutomation) {
        function checkIfUserIsTheOnlyAdministratorOfProjects() {
        ...
        if(!bJobAutomation) { checkIfUserIsTheOnlyAdministratorOfProjects(); }
    ...
    */
    this.deleteUserIds = function(sUserId) {
        function removeUserFromPLCTables() {
            function createDeleteStatement(sTable) {
                return `delete from "${sTable}" where user_id = ?; `;
            }

            dbConnection.executeUpdate(createDeleteStatement(Tables.usergroup_user), sUserId);
            dbConnection.executeUpdate(createDeleteStatement(Tables.layout_personal), sUserId);
            dbConnection.executeUpdate(createDeleteStatement(Tables.default_settings), sUserId);
            dbConnection.executeUpdate(createDeleteStatement(Tables.lock), sUserId);
            dbConnection.executeUpdate(createDeleteStatement(Tables.recent_calculation_versions), sUserId);
            dbConnection.executeUpdate(createDeleteStatement(Tables.frontend_settings), sUserId);
            dbConnection.executeUpdate(createDeleteStatement(Tables.auto_complete_user), sUserId);
        ...
        function unrollPrivilegesAfterUserDeletion() {            
            const aResultProjects = findProjectsTheUserHasPrivilegesFor(sUserId);
            _.each(aResultProjects, (oProject) => {
                authorizationUnroller.unrollPrivilegesOnObjectUpdate(dbConnection, "PROJECT", oProject.PROJECT_ID);
            });            
        }

        removeUserFromPLCTables(sUserId);
        unrollPrivilegesAfterUserDeletion(sUserId);
    };
	 ...
	 */
    this.deleteCustomerId = function(sCustomerId) {
        let iAffectedRows = 0;
        ...
        const aTablesToModify = [Tables.activity_price, Tables.material_price, Tables.customer];
	 ...
	 * @returns {number} - Number of affected rows
	 */
    this.deleteVendorId = function(sVendorId) {
    ...
    this.removePersonalDataFromProject = function(sProjectId) {
        ...
        const sStmtUpdateTempVariants =
                    `update "${Tables.variant_temporary}"
                    set last_calculated_by = ?,
                        last_modified_by = ?,
                        last_removed_markings_by = ?
                    where variant_id in (
        ...
        const sStmtUpdateEntityTags =
                            `update "${Tables.entity_tags}"
                            set created_by = ?
        ...
        const sStmtUpdateCalculationVersions =
            `update "${Tables.calculation_version}"
            set customer_id = ?,
                last_modified_by = ?
        ...
        const sStmtUpdateOneTimeProjectCost =
            `update "${Tables.one_time_project_cost}"
            set last_modified_by = ?
            where project_id = ?`;
        iAffectedRows += dbConnection.executeUpdate(sStmtUpdateOneTimeProjectCost, this.sPlaceholder, sProjectId);

        const sStmtUpdateOneTimeProductCost =
            `update "${Tables.one_time_product_cost}"
            set last_modified_by = ?
        ...
        const sStmtUpdateProjectLifecycleConfig =
            `update "${Tables.project_lifecycle_configuration}"
            set last_modified_by = ?
            where project_id = ?`;
        iAffectedRows += dbConnection.executeUpdate(sStmtUpdateProjectLifecycleConfig, this.sPlaceholder, sProjectId);

        const sStmtUpdateProjectLifecyclePeriodType =
            `update "${Tables.project_lifecycle_period_type}"
            set last_modified_by = ?
            where project_id = ?`;
        iAffectedRows += dbConnection.executeUpdate(sStmtUpdateProjectLifecyclePeriodType, this.sPlaceholder, sProjectId);

        const sStmtUpdateProjectMonthlyLifecyclePeriod =
            `update "${Tables.project_monthly_lifecycle_period}"
            set last_modified_by = ?
            where project_id = ?`;
        iAffectedRows += dbConnection.executeUpdate(sStmtUpdateProjectMonthlyLifecyclePeriod, this.sPlaceholder, sProjectId);

        const sStmtUpdateProjectLifecyclePeriodQuatityValue =
            `update "${Tables.project_lifecycle_period_quantity_value}"
            set last_modified_by = ?
            where project_id = ?`;
        iAffectedRows += dbConnection.executeUpdate(sStmtUpdateProjectLifecyclePeriodQuatityValue, this.sPlaceholder, sProjectId);

        const sStmtUpdateProjectTotalQuantites =
            `update "${Tables.project_total_quantities}"
            set last_modified_by = ?
            where calculation_id in (
            ${sStmtSelectCalculations})`;
    ...
    this.findFormulasThatContainPersonalData = function(sParam, sType) {   
        ...
        const aSearchParam = [`%'${sParam}'%`, "%CREATED_BY%", "%LAST_MODIFIED_BY%", "%CUSTOMER_ID%", "%VENDOR_ID%"];
	 ...
	 */
    this.erasePersonalDataAfterEndOfValidity = function() {
            const sStmtSelect =
                `select * from "${Tables.personal_data_validity}" where subject <> '*' and VALID_TO < CURRENT_UTCTIMESTAMP`;
            ...
            _.each(oResultSetVendor, (oResult) => { this.deleteVendorId(oResult.VENDOR_ID)});
            ...
            _.each(oResultSetCustomer, (oResult) => { this.deleteCustomerId(oResult.CUSTOMER_ID)});
            ...
            _.each(oResultSetProject, (oResult) =>  { this.removePersonalDataFromProject(oResult.PROJECT_ID)});
           
           
            dbConnection.commit(); //we need an explicit commit here, as this function is triggered outside dispatcher by the xsjob process    	
    };

    /**
     * Function to retrieve all the data for a given entity from the data protection view.
     */
	 ...
	 * Function to check if there are any formulas which contain personal data
	 *
	 * @param sEntityID {string} - the id of the entity for which the retention data is displayed
	 *
	 * @param sEntityType {string} - the type of the entity (CUSTOMER, VENDOR, etc..)
	 *
	 * @returns {object} - contains the metadata (column names) and the data from the tables
	 */
    this.getRetentionData = function (sEntityID, sEntityType) {
        var aRetentionData = {
            metadata : '',
            data : []
        };

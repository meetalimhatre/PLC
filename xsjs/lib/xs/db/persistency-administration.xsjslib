...
async function Administration(dbConnection, hQuery, hQueryRepl) {
    ...
    this.misc = await new imps.Misc($, hQuery, sUserId, dbConnection);
    var that = this;
	 ...
	 */
    this.getAdministration = async function (oGetParameters, sLanguage, sMasterDataDate) {
        ...
        case BusinessObjectTypes.ExchangeRateType:
            return (await new imps.ExchangeRateType(dbConnection, hQuery, hQueryRepl, imps.ExchangeRateTypeImport.oConfiguration)).get(oGetParameters, sLanguage, sMasterDataDate);
        case BusinessObjectTypes.AccountGroup:
            oReturnObjects = (await new imps.AccountGroup(dbConnection, hQuery, hQueryRepl)).get(oGetParameters, sLanguage, sMasterDataDate);
            await setLockStatus(oGetParameters.business_object);
            return oReturnObjects;
        case BusinessObjectTypes.ComponentSplit:
            oReturnObjects = (await new imps.ComponentSplit(dbConnection, hQuery, hQueryRepl)).get(oGetParameters, sLanguage, sMasterDataDate);
            await setLockStatus(oGetParameters.business_object);
            return oReturnObjects;
        case BusinessObjectTypes.CostingSheet:
            oReturnObjects = (await new imps.CostingSheet(dbConnection, hQuery, hQueryRepl)).get(oGetParameters, sLanguage, sMasterDataDate);
            await setLockStatus(oGetParameters.business_object);
            return oReturnObjects;
        case BusinessObjectTypes.CostingSheetRow:
            oReturnObjects = (await new imps.CostingSheetRow(dbConnection, hQuery, hQueryRepl)).get(oGetParameters, sLanguage, sMasterDataDate);
            await setLockStatus(BusinessObjectTypes.CostingSheet);
            await setLockStatus(BusinessObjectTypes.CostingSheetRow);
            return oReturnObjects;
        default:
            return (await new imps.MasterDataObjectHandlerProxy(dbConnection, hQuery, oGetParameters.business_object)).get(oGetParameters, sLanguage, sMasterDataDate);
        }
    ...
    this.deleteAdministration = async function (sObjectName, oBatchItems, sMasterDataDate) {
        ...
        case BusinessObjectTypes.AccountGroup:
            if (this.getLock(BusinessObjectTypes.AccountGroup).length > 0) {
                ...
                return (await new imps.AccountGroup(dbConnection, hQuery, hQueryRepl)).remove(oBatchItems, sMasterDataDate);
            }
            break;
        ...
        case BusinessObjectTypes.ComponentSplit:
            if (this.getLock(BusinessObjectTypes.ComponentSplit).length > 0) {
                ...
                return (await new imps.ComponentSplit(dbConnection, hQuery, hQueryRepl)).remove(oBatchItems, sMasterDataDate);
            }
            break;
         // to avoid eslint warning
        case BusinessObjectTypes.CostingSheet:
            if (this.getLock(BusinessObjectTypes.CostingSheet).length > 0) {
                ...
                this.setLock([
                    BusinessObjectTypes.CostingSheet,
                    BusinessObjectTypes.CostingSheetRow
                ]);
                return (await new imps.CostingSheet(dbConnection, hQuery, hQueryRepl)).remove(oBatchItems, sMasterDataDate);
            }
            break;
         // to avoid eslint warning
        case BusinessObjectTypes.CostingSheetRow:
            if (this.getLock([
                ...
                return (await new imps.CostingSheetRow(dbConnection, hQuery, hQueryRepl)).remove(oBatchItems, sMasterDataDate);
            }
            break;
         // to avoid eslint warning
        case BusinessObjectTypes.ExchangeRateType:
            return (await new imps.ExchangeRateType(dbConnection, hQuery, hQueryRepl, imps.ExchangeRateTypeImport.oConfiguration)).remove(oBatchItems, sMasterDataDate);
        default: {
	 ...
	 */
    //I055799 - If Costing Sheet, Component Split, Account Group, Price Source will be refactored, the method can be removed
    this.insertAdministration = async function (sObjectName, oBatchItems, sMasterDataDate) {
        ...
        case BusinessObjectTypes.AccountGroup:
            if (this.getLock(BusinessObjectTypes.AccountGroup).length > 0) {
                ...
                this.setLock([BusinessObjectTypes.AccountGroup]);
                return (await new imps.AccountGroup(dbConnection, hQuery, hQueryRepl)).insert(oBatchItems, sMasterDataDate);
            }
            break;
         // to avoid eslint warning
        case BusinessObjectTypes.ComponentSplit:
            if (this.getLock(BusinessObjectTypes.ComponentSplit).length > 0) {
                ...
                this.setLock([BusinessObjectTypes.ComponentSplit]);
                return (await new imps.ComponentSplit(dbConnection, hQuery, hQueryRepl)).insert(oBatchItems, sMasterDataDate);
            }
            break;
         // to avoid eslint warning
        case BusinessObjectTypes.CostingSheet:
            if (this.getLock(BusinessObjectTypes.CostingSheet).length > 0) {
                ...
                return (await new imps.CostingSheet(dbConnection, hQuery, hQueryRepl)).insert(oBatchItems, sMasterDataDate);
            }
            break;
         // to avoid eslint warning
        case BusinessObjectTypes.CostingSheetRow:
            if (this.getLock(BusinessObjectTypes.CostingSheetRow).length > 0) {
                ...
                return (await new imps.CostingSheetRow(dbConnection, hQuery, hQueryRepl)).insert(oBatchItems, sMasterDataDate);
            }
            break;
         // to avoid eslint warning
        case BusinessObjectTypes.ExchangeRateType:
            return (await new imps.ExchangeRateType(dbConnection, hQuery, hQueryRepl, imps.ExchangeRateTypeImport.oConfiguration)).insert(oBatchItems, sMasterDataDate);
        default: {
	 ...
	 */
    //I055799 - If Costing Sheet, Component Split, Account Group, Price Source will be refactored, the method can be removed
    this.updateAdministration = async function (sObjectName, oBatchItems, sMasterDataDate) {
        ...
        case BusinessObjectTypes.AccountGroup:
            if (this.getLock(sObjectName).length > 0) {
                ...
                this.setLock([BusinessObjectTypes.AccountGroup]);
                return await (await new imps.AccountGroup(dbConnection, hQuery, hQueryRepl)).update(oBatchItems, sMasterDataDate);
            }
            break;
         // to avoid eslint warning
        case BusinessObjectTypes.ComponentSplit:
            if (this.getLock(sObjectName).length > 0) {
                ...
                this.setLock([BusinessObjectTypes.ComponentSplit]);
                return await (await new imps.ComponentSplit(dbConnection, hQuery, hQueryRepl)).update(oBatchItems, sMasterDataDate);
            }
            break;
         // to avoid eslint warning
        case BusinessObjectTypes.CostingSheet:
            if (this.getLock(sObjectName).length > 0) {
                ...
                return await (await new imps.CostingSheet(dbConnection, hQuery, hQueryRepl)).update(oBatchItems, sMasterDataDate);
            }
            break;
         // to avoid eslint warning
        case BusinessObjectTypes.CostingSheetRow:
            if (this.getLock(sObjectName).length > 0) {
                ...
                return await (await new imps.CostingSheetRow(dbConnection, hQuery, hQueryRepl)).update(oBatchItems, sMasterDataDate);
            }
            break;
         // to avoid eslint warning
        case BusinessObjectTypes.ExchangeRateType:
            return await (await new imps.ExchangeRateType(dbConnection, hQuery, hQueryRepl, imps.ExchangeRateTypeImport.oConfiguration)).update(oBatchItems, sMasterDataDate);
        default: {
                const sLogMessage = `Business Object '${ sObjectName }' not maintained.`;
                $.trace.error(sLogMessage);
                throw new PlcException(Code.GENERAL_UNEXPECTED_EXCEPTION, sLogMessage);
	...
	*/
    this.getMasterdataForCalculationVersion = async function (iCvId, sLanguage, sSessionId, aBusinessObjectsEntities) {
	...
	*/
    this.getMasterdataOnItemLevel = async function (iCvId, sLanguage, sSessionId, aItems) {
        ...
        try {
            ...
            var iAffectedRowsDelete = await deleteStmt.execute();
            if (aItems.length > 0) {
	 ...
	 */
    this.checkIfTotalFieldsAreValidForCostingSheetRows = async function (sObjectName, oBatchItemsInsert, oBatchItemsUpdate, sMasterDataDate) {
        ...
        return (await new imps.CostingSheetRow(dbConnection, hQuery, hQueryRepl)).checkIfTotalFieldsAreValidForCostingSheetRows(oBatchItemsInsert, oBatchItemsUpdate, sMasterDataDate);
    };

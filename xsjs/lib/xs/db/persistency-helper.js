...
function Helper($, hQuery, dbConnection) {
	 ...
	 */
    this.getNextSequenceID = async function (sSequenceName) {
        var oNewEntityIdStatement = hQuery.statement('select "' + sSequenceName + '".nextval as newid from dummy');
        var aEntityId = await oNewEntityIdStatement.execute();
        ...
        var iEntityId;
        try {
            iEntityId = await helpers.toPositiveInteger(aEntityId[0].NEWID);
        } catch (e) {
    ...
    this.insertNewEntity = async function (oEntity, oSettings, customHQuery) {
        ...
        try {
            await oInsertStmt.execute(aInsertValues);
        } catch (e) {
	 ...
	 *             if any exceptional state during the communication with the database occurs
	 */
    this.selectEntities = async function (oSettings, customHQuery) {
        var hQueryToUse = customHQuery || hQuery;
        var sTableName = oSettings.TABLE;
        ...
        try {
            var oResult = await stmt.execute(aStmtValues);
            return oResult;
        } catch (e) {
    ...
    this.updateEntity = async function (oEntity, oSettings, customHQuery) {
        // check the optional customHQuery parameter; if it is defined it should be used if not use the default hQuery from
        ...
        var iAffectedRows = 0;
        try {
            iAffectedRows = await stmt.execute(aQueryValues);
        } catch (e) {
	 ...
	 */
    this.getColumnsForTable = async function (sTableName) {
        var sDetermineColumnsStmt = hQuery.statement('select column_name from "SYS"."TABLE_COLUMNS" where schema_name = CURRENT_SCHEMA' + " and table_name = '" + sTableName + "' order by position");
        var aColumns = _.map(await sDetermineColumnsStmt.execute(), function (oColumnResult, iIndex) {
	 ...
	 */
    this.getKeyColumnsForTable = async function (sSchema, sTableName) {
        var sSchemaStr = sSchema ? `'${ sSchema }'` : 'CURRENT_SCHEMA';
        var sColumnsStmt = hQuery.statement('select column_name from "SYS"."CONSTRAINTS" where schema_name = ' + sSchemaStr + " and table_name = '" + sTableName + "' and IS_PRIMARY_KEY = 'TRUE' order by POSITION ASC");
        var aKeyTableColumns = _.map(await sColumnsStmt.execute(), function (oColumnResult, iIndex) {
            return oColumnResult.COLUMN_NAME;
        });
	 ...
	 */
    this.exists = async function (aIds, sTableName, sColumnName) {

        var aStmtBuilder = [];
        var aValues = [];
        var aCount = [];

        if (aIds.length <= 0 || await helpers.isNullOrUndefined(sTableName) || await helpers.isNullOrUndefined(sColumnName)) {
        ...
        try {
            aCount = await selectStmt.execute(aValues);
        } catch (e) {
	...
	*/
    this.updateEntityRelation = function (iEntityId, iParentEntityId) {
        const oEntityParent = { 'PARENT_ENTITY_ID': iParentEntityId };
        const oSettings = {
            TABLE: Tables.entity_relation,
            WHERE_PROPERTIES: { ENTITY_ID: iEntityId }
        };
	...
	*/
    this.getPath = function (iEntityId) {
        const sPath = this.selectEntities({
            ...
            COLUMNS: ['PATH'],
            WHERE_PROPERTIES: { 'QUERY_NODE': iEntityId }
        });
...
export default {helpers,_,MessageLibrary,PlcException,Code,constants,Tables,Views,Helper};

...
function Misc($, hQuery, sUserId, dbConnection) {
	 ...
	 */
	this.determineAvailableLanguage = function(sTableName, sPreferredLanguage) {
		...
		if (sTableName.indexOf('"') == -1) {
			allLanguagesStmt = hQuery.statement([ 'select distinct '
					+ sLanguageFieldname + ' from "' + sTableName + '"' ]
					.join(" "));
		} else {
			// when the integration tests are done, the table has already "
			// (e.g: "I055799"."table__text")
			allLanguagesStmt = hQuery.statement([ 'select distinct '
					+ sLanguageFieldname + ' from ' + sTableName ].join(" "));
		}
		...
		var aAllLanguages = _.map(aAllLanguagesWithProperies,
				sLanguageFieldname);
	...
	this.determineLanguageField = function(sTableName) {
		...
		if ((_.endsWith(sTableName, '__text'))
				|| (_.endsWith(sTableName, '__text"'))) {
			return aLanguageFields[1];// LANGUAGE
		}
		...
		if (_.includes(aTables, sTableName)) {
			return aLanguageFields[0];// LANGU
		}
	 ...
	 *          systemMessage
	 */
	this.getSystemMessages = function(sIsoLanguage) {
		...
		var stmt = hQuery.statement('select top 1 message from "'
				+ Tables.system_message
				+ '" where language = (select ifnull(max(language),?)'
				+ ' from "' + Tables.system_message + '" where language = ?)');
	 ...
	 *          groups
	 */
	this.getSidePanelGroups = function() {
		
		var stmt = hQuery.statement(['select SIDE_PANEL_GROUP_ID, SIDE_PANEL_GROUP_DISPLAY_ORDER, RESOURCE_KEY_GROUP_DESCRIPTION',
						'  from "' + Tables.group + '"'].join(" "));
	 ...
	 */
	this.getLock = function(sObject, sUserId, bIncludingCurrentUser) {
	...
	/**
	 * Returns the users that are active and locked specific object in t_lock table
	 * specified one
	 * 
	 * @param {string}
	 *            sObject - the object that needs to be verified if it's locked
	 ...
	 *            sUserId - the current user
	 * 
	 * @returns {array} - locking users that are active in system and have the object locked
	 */
	this.getLockingUsers = function(sObject, sUserId) {
		
		var oSelectStatement = hQuery.statement([ 'select SESSION_ID, USER_ID, LANGUAGE, LAST_ACTIVITY_TIME from  "' + Tables.session + '"',
				                    				' where user_id in (select "USER_ID" from "' + Tables.lock + '"',
	...
	/**
	 * lock table t_lock in exclusive mode
	 */
	this.lockTableTLockExclusive = function() {
		try {
	...
	/**
	 * Set the lock on a specific object for the specified user
	 * 
	 * @param {string}
	 *            sObject - the object that needs to be locked
	 * @param {string}
	 *            sUserId - the current user
	 */
	...
	/**
	 * Release all the locked objects for the specified user
	 * 
	 * @param {string}
	 *            sUserId - the current user
	 */
	this.releaseLock = function(sUserId) {
		
		var stmt = hQuery.statement('select count(*) as rowcount from "'
				+ Tables.lock + '" where USER_ID = ?');
		var aCount = stmt.execute(sUserId);
	...
	/**
	 * Returns application default settings for user.
	 * 
	 * @param {string}
	 *            sUserId - The userId for which the settings should be
	 *            returned.
	 * @returns {object} - map (key-values) for default settings
	 */
	this.getDefaultSettings = function(sUserId) {
		try {
			// do select * to be agnostic for new columns in default settings table
			var stmt = hQuery.statement('select * from "'
					+ Tables.default_settings + '" where USER_ID = ?'); // get user-specific settings
			var userSettings = stmt.execute(sUserId);

			stmt = hQuery.statement('select * from "' + Tables.default_settings + '" where USER_ID = \'\''); // get global settings
			var globalSettings = stmt.execute();

			var result = {};
			
			// if controlling area is found in user settings take everything from user settings
			// else if controlling area is found in global settings take global settings
			// else take everything from metadata
			if (userSettings.length > 0 && !helpers.isNullOrUndefined(userSettings[0]) &&
					userSettings[0].CONTROLLING_AREA_ID !== '' && !helpers.isNullOrUndefined(userSettings[0].CONTROLLING_AREA_ID)) {
								result = userSettings[0];				
			}else if(globalSettings.length > 0 && !helpers.isNullOrUndefined(globalSettings[0]) &&
					globalSettings[0].CONTROLLING_AREA_ID !== '' && !helpers.isNullOrUndefined(globalSettings[0].CONTROLLING_AREA_ID)){
					result = globalSettings[0];
			}
			
			// get currency if is found in user settings; else get it from global settings; else get it from metadata
	...
	/**
	 * Returns active users from the system.
	 * 
	 * @returns {object} - USER_ID, LANGUAGE AND SECONDS_BETWEEN
	 */
	this.getActiveUsers = function() {
		return hQuery.statement(
				'select a.USER_ID, a.LANGUAGE, TO_DOUBLE(SECONDS_BETWEEN ( a.LAST_ACTIVITY_TIME , CURRENT_UTCTIMESTAMP ) ) as SECONDS_BETWEEN'
				  ...
				  + ' a.LAST_ACTIVITY_TIME < CURRENT_UTCTIMESTAMP AND'
				  + ' a.USER_ID <> ? '
				  )			
				  .execute(sUserId);
	};	

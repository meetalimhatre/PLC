...
var mMasterDataBusinessObjects = {
	"account":          				["t_account", "t_account__text"],
	"account_group":    				["t_account_group", "t_account_group__text", "t_account_account_group"],
	"activity_type":    				["t_activity_type", "t_activity_type__text"],
	"business_area":    				["t_business_area", "t_business_area__text"],
	"company_code":     				["t_company_code", "t_company_code__text"],
	"component_split":  				["t_component_split_account_group", "t_component_split", "t_component_split__text"],
	"controlling_area": 				["t_controlling_area", "t_controlling_area__text"],
	"cost_center":      				["t_cost_center", "t_cost_center__text"],
	"costing_sheet":    				["t_costing_sheet", "t_costing_sheet_base", "t_costing_sheet_base_row", "t_costing_sheet_overhead", "t_costing_sheet_overhead_row", "t_costing_sheet_row", "t_costing_sheet_row__text", "t_costing_sheet_row_dependencies", "t_costing_sheet_row__text"],
	"material_account_determination": 	["t_material_account_determination"],
	"material_group":   				["t_material_group", "t_material_group__text"],
	"material_type":    				["t_material_type", "t_material_type__text"],
	"overhead_group":   				["t_overhead_group", "t_overhead_group__text"],
	"plant":            				["t_plant", "t_plant__text"],
	"profit_center":    				["t_profit_center", "t_profit_center__text"],
	"valuation_class":  				["t_valuation_class", "t_valuation_class__text"]
};
...
const Procedures = {
	"check_formulas_staging" : "sap.plc.db.calcengine.procedures::p_check_formulas_staging"
};
...
function Transportation(dbConnection, oDbArtefactController, oMetadataPersistency) {
	...
	var aFilteredExportColumns = ["_VALID_FROM", "_VALID_TO", "_SOURCE", "CREATED_ON", "CREATED_BY", "LAST_MODIFIED_ON", "LAST_MODIFIED_BY"];

	var aTablesUsingSequence = ["t_costing_sheet_base", "t_costing_sheet_overhead", "t_costing_sheet_overhead_row", "t_formula"];
	 ...
	 */
	function getQueryToInsertStagingTableWithSequence(sTableName, sColumnsInsert, sParametersInsert) {
		var query = `INSERT INTO "${getFullStagingTableName(sTableName)}" (NEW_ID,${sColumnsInsert}) VALUES ("sap.plc.db.sequence::s_${sTableName.substring(2)}".nextval,${sParametersInsert})`;
		return query;
	}
	...
	function isUnlockStatus() {
		var fullTableNameLock = getFullTableName(sTableNameLock);
	 ...
	 */
	 function hasActiveUsers() {
		var sQuery = 'select a.USER_ID'
		+ ' FROM  "sap.plc.db::basis.t_session" AS a,'
		+ ' "sap.plc.db::basis.t_application_timeout" AS b'
		+ ' WHERE   b.APPLICATION_TIMEOUT_ID = \'SessionTimeout\' AND'
		+ ' SECONDS_BETWEEN ( a.LAST_ACTIVITY_TIME , CURRENT_UTCTIMESTAMP ) < b.VALUE_IN_SECONDS AND'
		+ ' a.LAST_ACTIVITY_TIME < CURRENT_UTCTIMESTAMP';
	...
	function deleteAllStagingTables() {
		...
		Object.keys(mDataModel).forEach(function(sTableName) {
			if (mDataModel[sTableName].hasStagingTable) {
				var query = 'delete from "' + getFullStagingTableName(sTableName) + '"';
	 ...
	 */
	function insertIntoStagingTablesWithoutCustomField(mTableData, sTableName) {
		...
		if (aAllRowsDataClient.length === 0) {
			return; // nothing to do if table contains no rows 
		}
		var aAllRowsData = [];
		...
		var query = 'INSERT INTO "' + getFullStagingTableName(sTableName) + '" (' + sColumnsInsert + ')' + ' VALUES (' + sParametersInsert + ')';
	 ...
	 */
	function insertIntoStagingTables(mTableData) {
		try {
			Object.keys(mTableData).forEach(function(sTableName) {
				insertIntoStagingTablesWithoutCustomField(mTableData, sTableName);
			});
		} catch (e) {
	 ...
	 */
	function buildQueryCheckReferenceKey(sTableName, oConstraint) {
		var fromStagingTableName = getFullStagingTableName(sTableName);
		var toStagingTableName = getFullStagingTableName(oConstraint.targetTable);
		var toTableName = getFullTableName(oConstraint.targetTable);
		var aFromFields = [];
		var aToFields = [];
		var aOnCondition = [];
		var aWhereCondition = [];
		...
		if (oConstraint.fields.length > 0) {
			...
			if (!mDataModel[oConstraint.targetTable].hasStagingTable) {
				query += 'SELECT ' + aFromFields.join(",") + ' FROM "' + fromStagingTableName + '" T1 LEFT OUTER JOIN' +
				' (SELECT ' + aToFields.join(",") +	' FROM "' + toTableName;
			} else {
				query += 'SELECT ' + aFromFields.join(",") + ' FROM "' + fromStagingTableName + '" T1 LEFT OUTER JOIN' +
				' (SELECT ' + aToFields.join(",") + ' FROM "' + toStagingTableName + '"' + ' UNION ALL SELECT ' + aToFields.join(",") + ' FROM "' + toTableName;
			}
	 ...
	 */
	function checkCustomFieldsAndFormulas(mTableData, mParameters) {		
		if (mParameters.mode !== "replace") {
			...
			var oModifiedCustomFieldsResult = dbConnection.executeQuery(
				'	select db.column_id as column_id \
					from "sap.plc.db::basis.t_metadata" as db \
					  inner join  "sap.plc.db::basis.t_metadata_staging" as staging \
					  	on 	db.path = staging.path	\
					  	and db.business_object = staging.business_object \
					  	and db.column_id = staging.column_id \
				  	where 		db.is_custom = 1 \
					  		and staging.is_custom = 1 \
							and ( \
									ifnull(db.rollup_type_id, -1) <> ifnull(staging.rollup_type_id, -1) \
								or ifnull(db.side_panel_group_id, -1) <> ifnull(staging.side_panel_group_id, -1) \
								or ifnull(db.display_order, -1) <> ifnull(staging.display_order, -1) \
								or ifnull(db.table_display_order, -1) <> ifnull(staging.table_display_order, -1) \
								or ifnull(db.ref_uom_currency_path, \'\') <> ifnull(staging.ref_uom_currency_path, \'\') \
								or ifnull(db.ref_uom_currency_business_object, \'\') <> ifnull(staging.ref_uom_currency_business_object, \'\') \
								or ifnull(db.ref_uom_currency_column_id, \'\') <> ifnull(staging.ref_uom_currency_column_id, \'\') \
								or ifnull(db.uom_currency_flag, -1) <> ifnull(staging.uom_currency_flag, -1) \
								or ifnull(db.semantic_data_type, \'\') <> ifnull(staging.semantic_data_type, \'\') \
								or ifnull(db.semantic_data_type_attributes, \'\') <> ifnull(staging.semantic_data_type_attributes, \'\') \
								or ifnull(db.property_type, -1) <> ifnull(staging.property_type, -1) \
								or ifnull(db.is_usable_in_formula, -1) <> ifnull(staging.is_usable_in_formula, -1) \
							);'
			);
			...
			var oDeletedCustomFieldsResult = dbConnection.executeQuery(
				'select column_id \
			...
			var oModifiedFormulasResult = dbConnection.executeQuery(
					'	select db.column_id as column_id \
						from "sap.plc.db::basis.t_formula" as db \
						  inner join  "sap.plc.db::basis.t_formula_staging" as staging \
						  	on 	db.path = staging.path	\
						  	and db.business_object = staging.business_object \
						  	and db.column_id = staging.column_id \
							and db.item_category_id = staging.item_category_id \
					  	where  db.is_formula_used <> staging.is_formula_used \
							or HASH_SHA256(to_binary(db.formula_string)) <> HASH_SHA256(to_binary(staging.formula_string)) \
							or ifnull(db.formula_description, \'\') <> ifnull(staging.formula_description, \'\')'
				);
			...
			var oDeletedFormulasResult = dbConnection.executeQuery(
				'select path, business_object, column_id, item_category_id \
			...
			var oModifiedCustomFieldItemAttributes = dbConnection.executeQuery(
				'	( \
							SELECT attributes.path, attributes.business_object, attributes.column_id, attributes.item_category_id, attributes.subitem_state, attributes.is_mandatory, \
								attributes.is_read_only, attributes.is_transferable, attributes.default_value \
							FROM "sap.plc.db::basis.t_metadata" as metadata \
								INNER JOIN "sap.plc.db::basis.t_metadata_item_attributes" as attributes \
									on attributes.path = metadata.path	 \
									and attributes.business_object = metadata.business_object \
									and attributes.column_id = metadata.column_id \
								WHERE metadata.is_custom = 1 \
						EXCEPT \
							SELECT attributes_staging.path, attributes_staging.business_object, attributes_staging.column_id, attributes_staging.item_category_id, attributes_staging.subitem_state, attributes_staging.is_mandatory, \
								attributes_staging.is_read_only, attributes_staging.is_transferable, attributes_staging.default_value \
							FROM "sap.plc.db::basis.t_metadata" as metadata \
								INNER JOIN "sap.plc.db::basis.t_metadata_item_attributes_staging" as attributes_staging \
									on 	attributes_staging.path = metadata.path	 \
									and attributes_staging.business_object = metadata.business_object \
									and attributes_staging.column_id = metadata.column_id \
								WHERE metadata.is_custom = 1 \
					) \
					UNION ALL \
					( \
			...
			var oModifiedCustomFieldTexts = dbConnection.executeQuery(
				'	( \
							SELECT texts.path, texts.column_id, texts.language, texts.display_name, texts.display_description \
							FROM "sap.plc.db::basis.t_metadata__text" as texts \
								INNER JOIN "sap.plc.db::basis.t_metadata" as metadata \
									ON texts.path = metadata.path \
									AND texts.column_id = metadata.column_id \
								WHERE metadata.is_custom = 1 \
						EXCEPT \
							SELECT texts_staging.path, texts_staging.column_id, texts_staging.language, texts_staging.display_name, texts_staging.display_description \
							FROM "sap.plc.db::basis.t_metadata__text_staging" as texts_staging \
								INNER JOIN "sap.plc.db::basis.t_metadata" as metadata \
									ON texts_staging.path = metadata.path \
									AND texts_staging.column_id = metadata.column_id \
								WHERE metadata.is_custom = 1 \
					) \
					UNION ALL \
					( \
		...
		var oChangedDataTypeCustomFieldsResult = dbConnection.executeQuery(
			'	select db.column_id as column_id \
				from "sap.plc.db::basis.t_metadata" as db \
				  inner join  "sap.plc.db::basis.t_metadata_staging" as staging \
					on 	db.path = staging.path	\
					and db.business_object = staging.business_object \
					and db.column_id = staging.column_id \
				where 	(	db.semantic_data_type <> staging.semantic_data_type \
							or	nullif(db.semantic_data_type_attributes, staging.semantic_data_type_attributes) is not null \
						) \
						and staging.is_custom = 1;'
		);
		...
		const oInvalidMetadataResult = dbConnection.executeQuery(
			`SELECT path, business_object, column_id FROM "sap.plc.db::basis.t_metadata_staging"
		...
		var oMissingItemAttributesResult = dbConnection.executeQuery(
			'select path, business_object, column_id from "sap.plc.db::basis.t_metadata_staging" \
			where (path, business_object, column_id) not in ( \
					select path, business_object, column_id from "sap.plc.db::basis.t_metadata_item_attributes_staging" \
			);');
		...
		var oInvalidMetadataAttributesResult = dbConnection.executeQuery(
			'SELECT path, business_object, column_id, item_category_id FROM "sap.plc.db::basis.t_metadata_item_attributes_staging" where  \
			item_category_id < -1 or item_category_id > (SELECT MAX(item_category_id) FROM "sap.plc.db::basis.t_item_category") or  \
			subitem_state not in (-1,0,1)'
		);
		...
		if (oFormulaErrorsResult.ERRORS.length > 0) {
			...
			oMessageDetails.addFormulaObjs({
				FORMULA_ERROR: JSON.parse('{' + oFormulaError.ERROR_DETAILS.replace(/'/g, "\"") + '}')
			});
	...
	function insertCustomFieldsAndFormulasToPersistentTables(mParameters) {
		...
		if(mParameters.mode === "replace"){
			...
			// delete ALL custom fields from t_metadata
			...
			dbConnection.executeUpdate(
				'	delete from "sap.plc.db::basis.t_metadata__text" \
					where  (path, column_id) not in ( \
							select path, column_id from "sap.plc.db::basis.t_metadata" \
						)'
			);
			
			// clean up all attributes where the combination of path, business_object, column_id is not longer in t_metadata; that hopefully clears
			// the table correctly
			dbConnection.executeUpdate(
				'	delete from "sap.plc.db::basis.t_metadata_item_attributes" \
					where  (path, business_object, column_id) not in ( \
							select path, business_object, column_id from "sap.plc.db::basis.t_metadata" \
						)'
			);			

			// remove formulas that are not in staging; in order to maintain formula_id, the modified formulas must kept in t_formula; an upsert stmt 
			// takes care of the update
			dbConnection.executeUpdate('delete from "sap.plc.db::basis.t_formula" \
										where (path, business_object, column_id, item_category_id) not in ( \
											select path, business_object, column_id, item_category_id from "sap.plc.db::basis.t_formula_staging" \
										)');

			// Update existing formulas with new values from the staging table but ignore formula_id because it might be different at the other system. 
			dbConnection.executeUpdate(
					'update "sap.plc.db::basis.t_formula" formula set \
						is_formula_used = staging.is_formula_used, formula_string = staging.formula_string, formula_description = staging.formula_description \
					from "sap.plc.db::basis.t_formula_staging" as staging inner join "sap.plc.db::basis.t_formula" as formula \
						on formula.path = staging.path AND formula.business_object = staging.business_object AND \
						formula.column_id = staging.column_id AND formula.item_category_id = staging.item_category_id'
			);
		}
		
		// inserting data from metadata staging tables into persistent tables; since only custom fields can be transported and imported data can be manipulated
		// it is important to make sure that only rows with custom fields are imported; for t_metadata_staging this is simple since the colunn is_custom can be 
		// used in where clause; upserting data to other tables uses a join over t_metadata_staging to limit to custom field rows
		// audit fields shall be ignored completely during the insert/upsert
		dbConnection.executeUpdate(
			'upsert "sap.plc.db::basis.t_metadata" ' +
			'(path, business_object, column_id, rollup_type_id, side_panel_group_id, display_order, table_display_order, ' +
			'ref_uom_currency_path, ref_uom_currency_business_object, ref_uom_currency_column_id, uom_currency_flag, ' +
			'semantic_data_type, semantic_data_type_attributes, property_type, is_usable_in_formula, is_custom) ' +
			'select path, business_object, column_id, rollup_type_id, side_panel_group_id, display_order, table_display_order, ' + 
			'ref_uom_currency_path, ref_uom_currency_business_object, ref_uom_currency_column_id, uom_currency_flag, ' +
			'semantic_data_type, semantic_data_type_attributes, property_type, 1 as is_usable_in_formula, is_custom ' +
			'from "sap.plc.db::basis.t_metadata_staging" where is_custom = 1;'
		);

		dbConnection.executeUpdate(
				'upsert "sap.plc.db::basis.t_metadata_item_attributes" \
					(path, business_object, column_id, item_category_id, subitem_state, is_mandatory, \
					is_read_only, is_transferable, default_value) \
				select attributes.path, attributes.business_object, attributes.column_id, attributes.item_category_id, attributes.subitem_state, attributes.is_mandatory, \
				attributes.is_read_only, attributes.is_transferable, attributes.default_value \
				from "sap.plc.db::basis.t_metadata_item_attributes_staging" as attributes\
					inner join "sap.plc.db::basis.t_metadata_staging" as metadata \
						on 	metadata.path = attributes.path \
						and metadata.business_object = attributes.business_object \
						and metadata.column_id = attributes.column_id \
				where metadata.is_custom = 1;'
		);
		
		dbConnection.executeUpdate(
			'upsert "sap.plc.db::basis.t_metadata__text" \
				(path, column_id, language, display_name, display_description) \
			select text.path, text.column_id, text.language, text.display_name, text.display_description \
			from "sap.plc.db::basis.t_metadata__text_staging" as text \
				inner join "sap.plc.db::basis.t_metadata_staging" as metadata \
					on 	metadata.path = text.path \
					and metadata.column_id = text.column_id \
					where metadata.is_custom = 1;'
		);
		
		// Insert new formulas and use newly assigned formula_id from staging tables.
		// This is custom logic and, thus, it cannot be handled by function insertIntoTableUsingSequence.
		dbConnection.executeUpdate(
				'insert into "sap.plc.db::basis.t_formula" \
					(formula_id,path,business_object,column_id,item_category_id,is_formula_used,formula_string,formula_description) \
				select  staging.new_id as formula_id, \
						staging.path, staging.business_object, staging.column_id, staging.item_category_id, staging.is_formula_used, \
						staging.formula_string, staging.formula_description \
				from "sap.plc.db::basis.t_formula_staging" as staging left outer join "sap.plc.db::basis.t_formula" as formula \
						on formula.path = staging.path AND formula.business_object = staging.business_object AND \
						formula.column_id = staging.column_id AND formula.item_category_id = staging.item_category_id \
				where formula.formula_id is null'
			);
		
		// generate all DB artefacts; make sure to use generateAllFilesExt in case of mode=replace; generateAllFiles would not delete CFF
		if(mParameters.mode === "replace"){
		...
		// after successfully altering the tables and generating code (DbArtefactController) it's necessary to prepare CUST_*_UNIT and CUST_*_IS_MANUAL columns; use logic
		// of the metadata provider persistency for that; only for new fields!
	...
	/**
	 * Check for rows in staging tables which already exist in real tables and delete them.
	 */
	function deleteAlreadyExistingData(mTableData) {
		Object.keys(mTableData).forEach(function(sTableName) {
			if(_.includes(aCffTables, sTableName) === true){
				// since custom fields and formulas are handled specially, the tables should not be cleared from existing data
				// see insertCustomFieldsAndFormulasToPersistentTables and importData for further comments
				return;
			}
			
			if (mDataModel[sTableName].hasStagingTable &&
				// special handling for tables with CLOB columns, they cannot be compared in WHERE clause
				aClobTables.indexOf(sTableName) === -1) {
				// get primary key columns of the table, without _VALID_FROM
				...
				// build a sql statement similar to this example:
				// delete from "sap.plc.db::basis.t_account_staging" where (account_id, controlling_area_id) in 
				//   (select t1.account_id, t1.controlling_area_id from "sap.plc.db::basis.t_account" t1 inner join "sap.plc.db::basis.t_account_staging" t2 
				//		on t1.account_id=t2.account_id and t1.controlling_area_id=t2.controlling_area_id where _valid_to is null)
				var query = 'delete from "' + getFullStagingTableName(sTableName) +
					'" where (' + sPrimaryKeyList + ') in (select ' + sPrimaryKeyListWithPrefix +  
					' from "' + getFullTableName(sTableName) + '" t1 inner join "' + getFullStagingTableName(sTableName) + '" t2 on ' +
					sJoinCondition + sWhereCondition;
				dbConnection.executeUpdate(query);
			}
		});
	}
	...
	/**
	 * Check for rows in staging tables which already exist in replicated tables and delete them.
	 * Reason: Replicated data always has priority over PLC local master data and must not be overriden.
	 * TODO: Implementation incomplete, copy from deleteAlreadyExistingData
	 */
	function deleteAlreadyExistingReplicatedData(mTableData) {
		// Implementation was removed because it was not complete
		return;
	}
	
	/**
	 * Check the integrity condition for foreign key relationships.
	 * The foreign key must appear as primary key in the referenced table or in the referenced staging table.
	 * @param {map} mTableData - map table name to table data
	 * @return {boolean} - true if all references are valid
	 */
	function checkReferenceIntegrity(mTableData) {
		var aErrors = [];
		Object.keys(mTableData).forEach(function(sTableName) {
			var aTableConstraints = oForeignKeyRelationships[sTableName];
			if (aTableConstraints !== undefined && aTableConstraints !== null) {
				aTableConstraints.forEach(function(constraint) {
					try {
						var query = buildQueryCheckReferenceKey(sTableName, constraint);
						if (query.length > 0) {
		...
		// TODO: additional checks for special tables, commented out MK
		//if (mTableData.hasOwnProperty("t_calculation")) {
		//}
	...
	/**
	 * Insert into tables which uses sequence id as t_calculation,t_calculation_version,t_item, costing_sheet_base,costing_sheet_overhead,costing_sheet_overhead_row
	 * @param {map} mTableData - map table name to table data
	 * @param {string} sTableName - the name of the table
	 */
	 function insertIntoTableUsingSequence(mTableData, sTableName) {
	 	var sQuery;
	 	var aColumns = mTableData[sTableName][0]; // first row contains column names;
	 	var sColumnsInsert;
	 	if (sTableName === "t_costing_sheet_base") {
	 		sColumnsInsert = _.without(aColumns, "COSTING_SHEET_BASE_ID").join(",");
	 		sQuery = 'INSERT INTO "' + getFullTableName(sTableName) + '"' + ' (COSTING_SHEET_BASE_ID,' + sColumnsInsert +
	 			', _VALID_FROM,_SOURCE,_CREATED_BY) ' +
	 			' SELECT NEW_ID' + ' AS COSTING_SHEET_BASE_ID,' + sColumnsInsert + ',\'' + oCurrentDate.toJSON() +
	 			'\' AS _VALID_FROM,\'' +
	 			iSource + '\' as _SOURCE,' + '\'' + $.getPlcUsername() + '\' AS _CREATED_BY' +
	 			' FROM "' + getFullStagingTableName("t_costing_sheet_base") + '"';
	 	} else {
			// if no insert stmt was generated for the table return in order to avoid executing an empty statement
			return;
		}
	 	//TO DO: for other tables: costing_sheet_overhead, costing_sheet_overhead_row
	 	dbConnection.executeUpdate(sQuery);
	 }

	/**
	 * Insert into tables which have relationship with tables using sequence id. For example: t_calculation__text (because it has relationship with calculation_id in t_calculation)
	 * @param {map} mTableData - map table name to table data
	 * @param {string} sTableName - the name of the table
	 */
	function insertIntoTableHavingRelationshipWithSequence(mTableData, sTableName) {
		var aTableData = mTableData[sTableName];
		var aColumns = aTableData[0]; // first row contains column names
		...
		//TODO: need to check versioned table or not
		var sUpsertStmt = 'UPSERT "' + getFullTableName(sTableName) + '"' + ' (' + sFromField + ',' + sColumns + ') ' + 'Select (Select NEW_ID' +
			' FROM "' + toStagingTableName + '" T1 WHERE T1.' + sToField + '= T2.' + sFromField + ') AS ' + sFromField + ',' +
			sColumns + ' FROM "' + fromStagingTableName + '" T2';

		dbConnection.executeUpdate(sUpsertStmt);
	}
    
	/**
	 * Insert into normal tables.
	 * @param {map} mTableData - map table name to table data
	 * @param {string} sTableName - the name of the table
	 */
	function insertIntoNormalTable(mTableData, sTableName) {
		var aTableData = mTableData[sTableName];
		var aColumns = aTableData[0]; // first row contains column names
		// generate list of column to insert to db
		var sColumnsInsert = aColumns.join(",");
		...
		if (mDataModel[sTableName].isVersionedTable) {
			//TODO: what value for created_by in the following query

			// invalidate existing versions of master data by setting _valid_to to current time
			var sUpdateStmt = 'UPDATE "' + getFullTableName(sTableName) + '" T1' + ' SET _VALID_TO=? FROM "' + getFullTableName(sTableName) +
				'" T1 INNER JOIN "' + getFullStagingTableName(sTableName) + '" T2 ON ' + aOnCondition.join(' AND ') + ' WHERE T1._VALID_TO IS NULL';

			dbConnection.executeUpdate(sUpdateStmt, oCurrentDate);

			//process _CREATED_USER_ID column
			var aDefaultColumnsInsert = [];
			// if using default user id (current user)
			if (bDefaultUserID) {
				aDefaultColumnsInsert.push('_VALID_FROM, _VALID_TO, _SOURCE, _CREATED_BY');
				aDefaultColumnsInsert.push('\'' + oCurrentDate.toJSON() + '\' AS _VALID_FROM, NULL AS _VALID_TO, \'' + iSource +
					'\' AS _SOURCE,' + '\'' + $.getPlcUsername() + '\' AS _CREATED_BY');
			} else {
			...
			//if using default user id
			if (bDefaultUserID) {
				//if there exists _created_by data in csv file, there needs to delete this column
			...
			var sInsertStmt = 'INSERT INTO "' + getFullTableName(sTableName) + '"' + ' (' + sColumnsInsert +
				',' + aDefaultColumnsInsert[0] + ') ' +
				' SELECT ' + sColumnsInsert + ',' + aDefaultColumnsInsert[1] + ' FROM "' +
				getFullStagingTableName(sTableName) + '"';

			dbConnection.executeUpdate(sInsertStmt);
		} else {
			// replace existing master data rows in non-versioned tables
			var sUpsertStmt = 'UPSERT "' + getFullTableName(sTableName) + '"' + ' (' + sColumnsInsert + ') ' + ' SELECT ' + sColumnsInsert +
				' FROM "' + getFullStagingTableName(sTableName) + '"';
	 ...
	 * Insert to database
	 * @param {map} mTableData - map table name to table data
	 * @param {map} mOutput - map table name to import result
	 */
	function insertIntoDB(mTableData, mParameters, mOutput) {
		Object.keys(mTableData).forEach(function(sTableName) {
			if (aTablesUsingSequence.indexOf(sTableName) !== -1) {
				insertIntoTableUsingSequence(mTableData, sTableName);
			} else if (oSpecialForeignKeyRelationships.hasOwnProperty(sTableName)) {
				insertIntoTableHavingRelationshipWithSequence(mTableData, sTableName);
				...
				insertIntoNormalTable(mTableData, sTableName);
			}
			
			mOutput[sTableName] = ["imported successfully"];
		});
		
		// special handling for CFF
		insertCustomFieldsAndFormulasToPersistentTables(mParameters);
	 ...
	 *
	 * @param {object} oRequest   - a map with table name as key and array of arrays as value containing the table data (column names are the first row)
	 * @param {array} aParameters - a array of import parameters 
	 * @returns {object} output
	 */
	this.importData = function(oRequest, mParameters) {
		mParameters = mParameters || {};
		mParameters.mode = mParameters.mode || "append";

		var oOutput = {}; // object store result returned to client in case of success
		oCurrentDate = new Date(Date.now()); // consistently use this Date for _VALID_FROM
		if (oRequest === null) {
			const sLogMessage = `Request is null during import data.`;
			$.trace.error(sLogMessage);
			// TODO: should this be GENERAL_VALIDATION_EXCEPTION? GENERAL_ENTITY_NOT_FOUND_ERROR is rather for entities not found on server.
			throw new PlcException(Code.GENERAL_ENTITY_NOT_FOUND_ERROR, sLogMessage);
		}
		// check if the system is currently locked
		// TODO: Race condition between lock checking and acquiring the lock
		if (!isUnlockStatus()) {
		...
		const resultActiveUsers = hasActiveUsers();
		...
		try {
			// step 1: delete data in staging-this step could be delete because of step 5
			//deleteStagingTables(Object.keys(oRequest));
			deleteAllStagingTables();
			// step 2: insert to table staging
			insertIntoStagingTables(oRequest);
			// step 3: check for rows in staging tables which already exist and delete them
			
			// NOTE (RF): IMHO this method is not necessary if we swicth from insert to upsert for all statements; benfit would be that also the 
			// update case is directly supported (for not versioned data); I had issues with this function and t_formula in combination with mode=replace;
			// if the replace parameter is set, and an already existing formula was sent, this function would delete it from t_formula_staging; since t_formula
			// is cleared in insertCustomFieldsAndFormulasToPersistentTables (which is called later), existing formulas cannot be restored; there are potentially
			// other ways to solve this problem, but omitting this function might be the easiest?!
			deleteAlreadyExistingData(oRequest);
			
			checkCustomFieldsAndFormulas(oRequest, mParameters);
						
			// step 4: check for rows in staging tables which already exist in replicated tables and delete them
			deleteAlreadyExistingReplicatedData(oRequest);
			// step 5: check integrity of data
			checkReferenceIntegrity(oRequest);
			
			// step 6: insert to real table
			insertIntoDB(oRequest, mParameters, oOutput);
		} finally {
			//step 5: unlock import function
			unlockImportTool();
		}
		return oOutput;
	};
	...
	/**
	 * Export data from the array of tables
	 */
	function exportTables(aTableNames) {
		var mTableData = {};
		aTableNames.forEach(function(sTableName) {
			var bIsCffTable = aCffTables.indexOf(sTableName) !== -1;

			// Get name of columns in the table; omit audit fields if there any (as specified in TS); maybe there is better option to do that;
			// it is done this way since ECC is approaching quickly (RF)
			var aColumnNames = mDataModel[sTableName].columns;
			//Filter to get columns name is exported
			...
			// add "t1." prefix for all column names, required for metadata table joins to prevent ambiguous column names 
			var aPrefixedColumnNames = aColumnNames.map(function(value) {
				return "t1." + value;
			});
			//build the query to get data
			var query = 'SELECT ' + aPrefixedColumnNames.join(",") + ' FROM "' + getFullTableName(sTableName) + '" t1';
			if (isVersionedTable) {
				// only current version and PLC masterdata (not ERP replicated)
				query += ' WHERE _valid_to IS NULL AND _source=1';
			}
			if (bIsCffTable) {
				// special handling for Cff related tables
				switch (sTableName) {
					case "t_metadata":
						query += " WHERE is_custom=1";
						break;
					case "t_metadata__text":
						query += ' INNER JOIN "' + getFullTableName("t_metadata") + '" t2 ON t1.column_id=t2.column_id AND t1.path=t2.path WHERE is_custom=1';
						break;
					case "t_metadata_item_attributes":
						query += ' INNER JOIN "' + getFullTableName("t_metadata") + '" t2 ON t1.column_id=t2.column_id AND t1.path=t2.path AND t1.business_object=t2.business_object WHERE is_custom=1';
						break;
					case "t_formula": 					
						break;
					default: {
			...
			//get the result
			var result = dbConnection.executeQuery(query);
			var tableData = []; // array of table data, first row with table column names (like a csv)
			var tableRow = []; // array of table column names
			if (result.length > 0) {
				for (let column in result[0]) {
					tableRow.push(column);
				}
				tableData.push(tableRow); // first row with table column names
				...
				for (let row in result) {
					...
					tableData.push(tableRow); //push to array
				}
			} else {
				// also export empty table, but only column names
	...
	/**
	 * export data
	 * @param: parameters from the URL of the HTTP GET request
	 */
	this.exportData = function(mParameters) {
		// evaluate service parameters
		var aTableNames = [];
		
		// parameter "businessObjects" is mandatory and due to the UrlValidator it can used right away
		// NOTE (RF): the only supported value for the parameter is currently "customizing"
		...
		// do the actual export of tables
		if (aTableNames.length > 0) {
			return exportTables(aTableNames);
		}
	};

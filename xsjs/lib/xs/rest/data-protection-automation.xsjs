...
function erasePersonalDataAfterEndOfValidity(aParameters) {
    try {
        if(isCloud()) {
            ...
            aTenantDBClients.clients.forEach(client => {
                persistency = new Persistency(client.client);
                persistency.erasePersonalDataAfterEndOfValidity();
            });

        } else {
            const DispatcherLib = $.require("../impl/dispatcher");
            const ctx = DispatcherLib.prepareDispatch($);
            ctx.persistency.DataProtection.erasePersonalDataAfterEndOfValidity();
        }
        ...
        let authToken = servicesEnv.plan === "standard" && isCloud() ? cloudUtil.getAuthorizationToken(jobschedulerCredentials) : "Basic "+ btoa(`${jobschedulerCredentials.user}:${jobschedulerCredentials.password}`);

 ...
 * 
 */
function addCalculatedValuesToOutput(oValidatedRequestContent, oServiceOutput, oPersistency, sSessionId, sUserId) {
	...
	function runCalculation() {
		...
		if(oValidatedRequestContent.parameters.compressedResult){
		    oCalculationResult.ITEM_CALCULATED_FIELDS_COMPRESSED = helpers.transposeResultArray(oResult.ITEM_CALCULATED_PRICES);
		    oCalculationResult.ITEM_CALCULATED_VALUES_COSTING_SHEET_COMPRESSED = helpers.transposeResultArray(oResult.ITEM_CALCULATED_VALUES_COSTING_SHEET);
		    oCalculationResult.ITEM_CALCULATED_VALUES_COMPONENT_SPLIT_COMPRESSED =  helpers.transposeResultArray(oResult.ITEM_CALCULATED_VALUES_COMPONENT_SPLIT);
		}
		else {
		...
		_.each(oCalculationResult.ERRORS, function(error) {
	...
	function determineCalculationVersion() {
		...
		if( (oValidatedRequestContent.parameters.action !== ServiceParameters.Copy && oValidatedRequestContent.parameters.action !== CalculationServiceParameters.CopyVersion) 
		        && tryGetExistingCalculationVersionId( oValidatedRequestContent.parameters.id ) === true ){
	...
	if (_.isBoolean(oValidatedRequestContent.parameters.calculate) && oValidatedRequestContent.parameters.calculate === true) {
		...
		} else
        {
			if(oPersistency.CalculationVersion.isFrozen(iCalculationVersionId) === true && oPersistency.CalculationVersion.isDirty(iCalculationVersionId, sSessionId, sUserId) === false) {
				...
				if(oValidatedRequestContent.parameters.compressedResult){
		            oCalculationResult.ITEM_CALCULATED_FIELDS_COMPRESSED = helpers.transposeResultArray(oResult.ITEM_CALCULATED_FIELDS);
		            oCalculationResult.ITEM_CALCULATED_VALUES_COSTING_SHEET_COMPRESSED = helpers.transposeResultArray(oResult.ITEM_CALCULATED_VALUES_COSTING_SHEET);
		            oCalculationResult.ITEM_CALCULATED_VALUES_COMPONENT_SPLIT_COMPRESSED =  helpers.transposeResultArray(oResult.ITEM_CALCULATED_VALUES_COMPONENT_SPLIT);
		        }
		        else {

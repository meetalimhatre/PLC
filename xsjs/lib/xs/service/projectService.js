...
function getCalculationTaskForProjectSplitedByUsers(aExistingCalculationTasks, sProjectId, sUserName){
	if(aExistingCalculationTasks.length > 0) {
		...
		return {oRunningTasksForProjectStartedByCurrentUser, oRunningTasksForProjectStartedByOtherUser};
	}
}
 ...
 */
function checkExists(sProjectId, oPersistency) {
 ...
 */
function checkIsWritable(sProjectId, sUserId, oPersistency) {
 ...
 */
function checkLifetimeLimitsForProjectDetails(aBodyData, sProjectId, oPersistency) {
	var oDbProject = oPersistency.Project.getProjectProperties(sProjectId);
	if(helpers.isNullOrUndefined(oDbProject.START_OF_PROJECT) || helpers.isNullOrUndefined(oDbProject.END_OF_PROJECT)){
	...
	var iLowestValidPeriodFrom = this.calculateLifecyclePeriodFrom(oDbProject.START_OF_PROJECT, LifecycleInterval.YEARLY);
	var iHighestValidPeriodFrom = this.calculateLifecyclePeriodFrom(oDbProject.END_OF_PROJECT, LifecycleInterval.YEARLY);
 ...
 */
function checkOverlappingAccountsInAccountGroups(sProjectId, sBusinessObjectType, oServiceOutput, oPersistency) {
	...
	if (aDbOverlappingAccountsAndGroups.length > 0) {
		...
		let aOverlappingAccountGroups = _.keys(
				_.groupBy(aDbOverlappingAccountsAndGroups, function(oOverlappingAccountGroup) {
						return oOverlappingAccountGroup.ACCOUNT_GROUP_ID;
				}) 
		);

		logInfo(`Account groups '${aOverlappingAccountGroups.join(",")}' have overlapping accounts.`);
		
		let oMessageDetails = new MessageDetails()
			.setLifecycleSurchargeDetailsObj({
				ACCOUNT_GROUPS_WITH_OVERLAPS: aOverlappingAccountGroups
			});
		let oMessage = new Message(MessageCode.PROJECT_SURCHARGES_ACCOUNT_GROUPS_OVERLAPPING_WARNING, Severity.WARNING, oMessageDetails);
 ...
 */
function checkUniqueSurchargeDefinitionDependencyCombination(aSurchargeDefinitions, aDefinitionProperties) {
	if (aSurchargeDefinitions.length > 0) {
		...
		var bAnyDuplicatedDefinition = _.some(_.values(mGroupedByDefinitions), function(iGroupCount){return iGroupCount > 1;});
		...
		if(bAnyDuplicatedDefinition === true){
			...
			aDuplicatedDefinitions = _.filter(aDuplicatedDefinitions, function(val){ return val !== null; });

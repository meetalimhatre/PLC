 ...
 */
function doUpdate(oItemToUpdate, oPersistency, sSessionId, bUpdateOnlyStandardFields, iOldCategoryId, iNewCategoryId) {
	...
	var iAffectedRows = oPersistency.Item.update(oItemToUpdate, sSessionId, bUpdateOnlyStandardFields, iOldCategoryId, iNewCategoryId);
	if (iAffectedRows !== 1) {
...
function processValueDeterminationMessages(aMessages, oServiceOutput) {
	...
	_.each(aMessages, function(oMessage) {
		...
		case MessageCode.PRICEDETERMINATION_PRICESOURCE_CHANGED_INFO.code:
			oServiceOutput.addMessage(new Message(MessageCode[oMessage.MSG_ID], Severity.INFO, new MessageDetails()
					.setPriceDeterminationObj({
						itemId : oMessage.ITEM_ID
					})));
			break;
		case MessageCode.PRICEDETERMINATION_STANDARDPRICE_NOT_FOUND_WARNING.code:
			oServiceOutput.addMessage(new Message(MessageCode[oMessage.MSG_ID], Severity.WARNING, new MessageDetails()
					.setPriceDeterminationObj({
						itemId : oMessage.ITEM_ID
					})));
			break;
		...
		case MessageCode.DEPENDENTFIELDSDETERMINATION_COMPANY_CODES_SET_FOR_CHANGED_PLANTS_INFO.code:
			oServiceOutput.addMessage(new Message(MessageCode[oMessage.MSG_ID], Severity.INFO, new MessageDetails()
					.setDependentFieldDeterminationObj({
						itemId : oMessage.ITEM_ID
					})));
			break;	
		default:

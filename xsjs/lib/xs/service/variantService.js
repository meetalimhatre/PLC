...
function checkVariantExists(oPersistency, iCalculationVersionId, iVariantId) {
...
*/
function isVariantNameUnique(sVariantName, iCalculationVersion, oPersistency, iVariantId) {
    let oCalculationVersionVariant = null;
    const isUpdateRequest = iVariantId;
    const aCalculationVersionVariants = oPersistency.Variant.getVariants(iCalculationVersion);
    if (isUpdateRequest) {
        oCalculationVersionVariant = aCalculationVersionVariants.find(oVariant => oVariant.VARIANT_NAME === sVariantName
            && oVariant.VARIANT_ID !== iVariantId);
    } else {
 ...
 */
function checkQuantityStateValues(oPersistency, aVariantsItems, iCalculationVersionId) {
    ...
    aVariantsItems.forEach(oItem => {
        if(oItem.ITEM_ID === iRootItemId || aTextItems.includes(oItem.ITEM_ID)) {
            if(oItem.QUANTITY_STATE !== Constants.VariantItemQuantityState.MANUAL_VALUE) {
                const oMessageDetails = new MessageDetails().addItemObjs({
                    id: oItem.ITEM_ID,
                });

 ...
 */
function run(request, response, session) {
    ...
    if (request.method === $.net.http.POST && ((isCloud() && request.parameters.get("tenantid")) || !isCloud())) {
        PrepareUpgrade(request.parameters, response);
    } else {
 ...
 *
 */
function getConnection(oReqParameters) {
    const oConnection = isCloud() ? dbConnection.getConnection({"treatDateAsUTC": true}, oReqParameters.get("tenantid")) : dbConnection.getConnection({"treatDateAsUTC": true});
    return oConnection;
}
 ...
 *
 */
function PrepareUpgrade(reqParameters, response) {

    const oPersistency = new Persistency(getConnection(reqParameters));

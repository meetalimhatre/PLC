...
var PrivilegeOrder = Object.freeze([
	Privileges.READ, Privileges.CREATE_EDIT, Privileges.FULL_EDIT, Privileges.ADMINISTRATE
]);
...
var PreparedStatements = Object.freeze({
	...
	Calculation:         'select privilege from ( '+Tables.Project_Auth+'  as auth '+
                            'inner join '+
                            Tables.Calculation+' as calculation '+
                         'on auth.project_id = calculation.project_id'+
                         ') where auth.user_id = ? and calculation.calculation_id = ? ',
    Calculation_Version: 'select privilege from ( '+Tables.Project_Auth+' as auth '+
                         'inner join '+
                         '('+
                            'select calculation.project_id, version.calculation_version_id from '+
                            Tables.Calculation+' as calculation '+
                            'inner join '+
                            '('+
                            	'select calculation_version_id, calculation_id from '+Tables.Calculation_Version+
                            	' union '+
                            	'select calculation_version_id, calculation_id from '+Tables.Version_Temporary+
                            	' where session_id = ? '+
                            ') as version '+
                            'on calculation.calculation_id = version.calculation_id '+
                         ') calculation_join '+
                         'on auth.project_id = calculation_join.project_id '+
                         ') '+
                         'where auth.user_id = ? and calculation_join.calculation_version_id = ?'
});
 ...
 */
function getUserPrivilege(sBusinessObjectType, sBusinessObjectId, oConnection, sUsername, bThrowException = true){
    ...
    if (sBusinessObjectType === BusinessObjectTypes.CalculationVersion) {
    	result = oConnection.executeQuery(stmt, sUsername, sUsername, sBusinessObjectId);
	} else {
		result = oConnection.executeQuery(stmt, sUsername, sBusinessObjectId);
	}
    ...
    if ((result.length === 0) && bThrowException) {
    	...
    	case BusinessObjectTypes.Project:
    		...
    		aCount = oConnection.executeQuery(sStmt, sBusinessObjectId);
    	...
    	case BusinessObjectTypes.CalculationVersion:
    		sStmt = 'select count(*) as rowcount from ( ' +
    				' select calculation_version_id from ' + Tables.Calculation_Version + ' where CALCULATION_VERSION_ID = ? ' +
    				' union ' +
    				' select calculation_version_id from ' + Tables.Version_Temporary + ' where CALCULATION_VERSION_ID = ? and SESSION_ID = ? )';
    		aCount = oConnection.executeQuery(sStmt, sBusinessObjectId, sBusinessObjectId, sUsername);
 ...
 */
function checkPrivilege(sBusinessObjectType, sBusinessObjectId, sPrivilege, oConnection, sUsername){

...
var Views = Object.freeze({
	V_GROUP_HIERARCHY: 'sap.plc.db.authorization.views::auth.V_GROUP_HIERARCHY'
});
 ...
 */
function unrollPrivilegesOnObjectUpdate(oConnection, sObjectType, sObjectId)
{
	...
	var p_unroll_privileges_on_object_update = oConnection.loadProcedure(Procedures.p_unroll_privileges_on_object_update);
	var withError;
 ...
 * @returns {boolean} - true if call to the actual SQL procedure was successfully, otherwise false
 */
function unrollPrivilegesOnGroupUpdate(oConnection, sUsergroupId)
{
	var p_unroll_privileges_on_group_update = oConnection.loadProcedure(Procedures.p_unroll_privileges_on_group_update);
	var withError;
 ...
 * @returns {boolean} - true if call to the actual SQL procedure was successfully, otherwise false
 */
function unrollPrivileges(oConnection, aObjects)
{
	var p_unroll_privileges = oConnection.loadProcedure(Procedures.p_unroll_privileges);
	var withError;
 ...
 */
function getObjectsFromGroupPrivileges(oConnection, sUsergroupId)
{
	var stmt =
	`
	SELECT 
	...
	try 
	{
		var oReturn = oConnection.executeQuery(stmt);
 ...
 */
function containsCycle(oConnection)
{
	var stmt =
	`
	SELECT 

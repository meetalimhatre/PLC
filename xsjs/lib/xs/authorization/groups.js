 ...
 *            oPersistency - Instance of Persistency to access data base.
 */
this.edit = function(oBodyItems, aParameters, oServiceOutput, oPersistency) {
	...
	if(aLockObjects.length === 1 && aLockObjects[0].USER_ID !== sUserId) {
		...
		oMessageDetails.addUserObj({
			id : aLockObjects[0].USER_ID
		});
		oServiceOutput.addMessage(new Message(Code.GROUPS_NOT_WRITABLE_ERROR, Severity.ERROR, oMessageDetails));
	}
	...
	var aResultErrors = [];
	createGroupsSubgroupsMembers(oBodyItems, oPersistency, aResultErrors);
	deleteGroupsSubgroupsMembers(oBodyItems, oPersistency, aResultErrors);

	//update can be done done only for group description
	if(!helpers.isNullOrUndefined(oBodyItems.UPDATE) && !helpers.isNullOrUndefined(oBodyItems.UPDATE.GROUPS) && oBodyItems.UPDATE.GROUPS.length > 0) {
		...
		if(aItemsNotUpdatedGroups.length > 0){		
			_.each(aItemsNotUpdatedGroups, function(oGroup) {
...
/**
 * Handles the CREATION of groups, and user and group memberships.
 * 
 * @param {object}
 *            oBodyItems - The JS object used for the batch. 
 * @param {object}
 *            oPersistency - Instance of Persistency to access data base.
 * @param {array}
 * 			aResultErrors - An array encapsulating all the found errors for each object.
 *
 */
function createGroupsSubgroupsMembers(oBodyItems, oPersistency, aResultErrors) {
	if(!helpers.isNullOrUndefined(oBodyItems.CREATE)) {
		if(!helpers.isNullOrUndefined(oBodyItems.CREATE.GROUPS) && oBodyItems.CREATE.GROUPS.length > 0) {
			...
			//if not all items were created send message for each object that was not created
			if(aItemsNotCreatedGroups.length > 0){		
				_.each(aItemsNotCreatedGroups, function(oGroup) {
		...
		if(!helpers.isNullOrUndefined(oBodyItems.CREATE.MEMBERS) && oBodyItems.CREATE.MEMBERS.length > 0) {
			...
			if(aItemsNotCreatedMembers.length > 0){		
				//if not all items were created send message for each object that was not created
				_.each(aItemsNotCreatedMembers, function(oMember) {
		...
		if(!helpers.isNullOrUndefined(oBodyItems.CREATE.SUBGROUPS) && oBodyItems.CREATE.SUBGROUPS.length > 0) {
			...
			if(aGroups.length !== oBodyItems.CREATE.SUBGROUPS.length) {
				...
				_.each(aUnexistingGroups, function(oSubgroup) {
			...
			if(aItemsNotCreatedSubgroups.length > 0){		
				//if not all items were created send message for each object that was not created
				_.each(aItemsNotCreatedSubgroups, function(oSubgroup) {
			...
			{
				if (authorizationUnroller.containsCycle(oPersistency.getConnection()))
				{
...
/**
 * Handles the DELETION of groups, and user and group memberships.
 * @param {object}
 *            oBodyItems - The JS object used for the batch. 
 * @param {object}
 *            oPersistency - Instance of Persistency to access data base.
 ...
 *
 */
function deleteGroupsSubgroupsMembers(oBodyItems, oPersistency, aResultErrors){
	if(!helpers.isNullOrUndefined(oBodyItems.DELETE)) {
		if(!helpers.isNullOrUndefined(oBodyItems.DELETE.GROUPS) && oBodyItems.DELETE.GROUPS.length > 0) {
			//if not all items were created send message for each object that was not deleted
			...
			if(aItemsNotDeletedGroups.length > 0){		
				_.each(aItemsNotDeletedGroups, function(oGroup) {
		...
		if(!helpers.isNullOrUndefined(oBodyItems.DELETE.MEMBERS) && oBodyItems.DELETE.MEMBERS.length > 0) {
			...
			if(aItemsNotDeletedMembers.length > 0){		
				//if not all items were created send message for each object that was not deleted
				_.each(aItemsNotDeletedMembers, function(oMember) {
		...
		if(!helpers.isNullOrUndefined(oBodyItems.DELETE.SUBGROUPS) && oBodyItems.DELETE.SUBGROUPS.length > 0) {
			...
			if(aItemsNotDeletedMembers.length > 0){		
				//if not all items were created send message for each object that was not deleted
				_.each(aItemsNotDeletedMembers, function(oSubgroup) {

 ...
 */
function HDIClient($, sContainer, oCredentials) {
    ...
    this.listChanged = (aPath) => {
        ...
        try {
            let oResult = oConnection.sync.status(aPath.map((path) => {return new Path(path)}), []);
            if (oResult.rc !== 0) {
    ...
    this.listDeployed = (aPath, bRecursive) => {
        ...
        try {
            let oResult = oConnection.sync.listDeployed(aPath.map((path) => {return new Path(path)}), aParams);
            if (oResult.rc !== 0) {
    ...
    this.upsert = (aPathContent) => {
        ...
        for (let oPathContent of aPathContent) {
            addPathContent(oPathSets, aPendingPathContent, oPathContent);
        }
    ...
    this.delete = (aPath, bRecursive) => {
        ...
        try {
            let oResult = oConnection.sync.delete(aPath.map((path) => {return new Path(path)}), aParams);
            if (oResult.rc !== 0) {
     ...
     * @param bRecursive {boolean} 
     */
    this.readDeployed = (aPath, bRecursive) => {
        if (helpers.isNullOrUndefined(aPath)) {
        ...
        let aParams = bRecursive ? [new Parameter("RECURSIVE", "TRUE")] : [];
        try {
            let oResult = oConnection.sync.readDeployed(aPath.map((path) => {return new Path(path)}), aParams);
            if (oResult.rc !== 0) {
        ...
        try {
            ...
            if (aModList.length > 0) {
                ...
                for (let oItem of aObjList) {
                    oPendingPathContent.push({PATH: oItem.path, CONTENT: oItem.content});
                }

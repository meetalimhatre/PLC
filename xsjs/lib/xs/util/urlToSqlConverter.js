...
function excludeLowerForDates(sProperty){
	const aDateProperties = ['_VALID_FROM', '_VALID_TO', 'CREATED_ON', 'LAST_MODIFIED_ON'];
	return aDateProperties.includes(sProperty) ? `((${sProperty})` : `(lower(${sProperty})`;
}

function UrlToSqlConverter() {
	 ...
	 */
	this.convertToSqlFormat = function(sFilter, aMetadata) {
		var sResult="";
		var oParsedString = {};
		...
		oParsedString = parseFilterString(sFilter, aMetadata);

		//build return string sResult
		sResult = buildSqlString(oParsedString.FieldValueOperator,oParsedString.FieldValueOperatorExclude,oParsedString.StrSplitLength,oParsedString.FieldValueOperatorExcludeLength);
		return sResult;
	};
	 ...
	 * @param	{array} aMetadata - business object's metadata
	 */
	function parseFilterString(sFilter, aMetadata) {
		var oResult = {};
	 ...
	 */
	function buildSqlString(aFieldValueOperator,aFieldValueOperatorExclude,iStrSplitLength,iFieldValueOperatorExcludeLength){
		var sResult="";
		...
		for(var i=0 ; i < iStrSplitLength - iFieldValueOperatorExcludeLength; i++){
			...
			if(aFieldValueOperator[i][3] !== 'Decimal'){
				...
				aFieldValueOperator[i][1] = parseDecimal(aFieldValueOperator[i][1]);
...
//		add final entries where condition was != ( NOT LIKE ) with AND aOperator	
		for(var m=0 ; m < iFieldValueOperatorExcludeLength; m++){
			...
			if( m == 0 && iFieldValueOperatorExcludeLength == iStrSplitLength){
				if(isNumber(aFieldValueOperatorExclude[m][3]) === true){
					aFieldValueOperatorExclude[m][1] = parseDecimal(aFieldValueOperatorExclude[m][1]);
					...
					if (aFieldValueOperatorExclude[m][2] === "LIKE" || aFieldValueOperatorExclude[m][2] === "NOT LIKE" ){
						sResult = sResult.concat( `${excludeLowerForDates(aFieldValueOperatorExclude[m][0])} ${aFieldValueOperatorExclude[m][2]} lower('${aFieldValueOperatorExclude[m][1].replace(/_/g,'__')}') ESCAPE '_'`);
					}
					else{
						sResult = sResult.concat( `${excludeLowerForDates(aFieldValueOperatorExclude[m][0])} ${aFieldValueOperatorExclude[m][2]} lower('${aFieldValueOperatorExclude[m][1]}')`);
					}
				}
			}
			else{
				if(isNumber(aFieldValueOperatorExclude[m][3]) === true){
					...
					if (aFieldValueOperatorExclude[m][2] === "LIKE" || aFieldValueOperatorExclude[m][2] === "NOT LIKE" ){
						sResult = sResult.concat( ` AND ${excludeLowerForDates(aFieldValueOperatorExclude[m][0])} ${aFieldValueOperatorExclude[m][2]} lower('${aFieldValueOperatorExclude[m][1].replace(/_/g,'__')}') ESCAPE '_'`);
					}
					else{
						sResult = sResult.concat( ` AND ${excludeLowerForDates(aFieldValueOperatorExclude[m][0])} ${aFieldValueOperatorExclude[m][2]} lower('${aFieldValueOperatorExclude[m][1]}')`);
					}
				}
			}
	 ...
	 **/
	function buildStringMatrixAndValidate(aStrSplit,aMetadata) {
		// build matrix with columns FIELD , VALUE and OPERATOR	
		var iStrSplitLength = aStrSplit.length;
		...
		for (var i=0 ; i < iStrSplitLength; i++){
			...
			if(aOperator[j] == "!=" || aOperator[j] == "<=" || aOperator[j] == ">=" || aOperator[j] == ">" || aOperator[j] == "<")
			{
				...
				aFieldValueOperatorExclude[iFieldValueOperatorExcludeLength]['3'] = getMetadataType(aMetadata,aFieldValueOperatorExclude[iFieldValueOperatorExcludeLength]['0']);
				...
				aFieldValueOperator[i-iFieldValueOperatorExcludeLength]['3'] = getMetadataType(aMetadata,aFieldValueOperator[i-iFieldValueOperatorExcludeLength]['0']);
		...
		return {aFieldValueOperatorExclude:aFieldValueOperatorExclude,aFieldValueOperator:aFieldValueOperator, iFieldValueOperatorExcludeLength:iFieldValueOperatorExcludeLength};
	}

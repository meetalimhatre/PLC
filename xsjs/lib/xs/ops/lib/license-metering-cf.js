 ...
 */
function getTenantUserCount(oClient) {
    let result = null;
 ...
 */
function sendUsageData(oParameters) {
 ...
 */
function getAuthorizationToken(clientId, clientSecret, tokenURL) {
...
function collectUsageData($, req, res) {
    ...
    aTenantDBClients.clients.filter(function (oClient) {
        ...
        let iUserCount = module.exports.getTenantUserCount(oClient.client); // module.exports is for mocking in testing
        oClient.client.close();
        ...
        if (iUserCount === null) {
            ...
            try {
                let oResult = module.exports.sendUsageData({ // module.exports is for easy mocking in testing
                ...
                if (oResult.statusCode !== 200) { //200 is the MaaS api response,means send data success
                    aFailedTenantInfo.push({
                        tenantId: sTenantId,
                        tenantName: sTenantName,

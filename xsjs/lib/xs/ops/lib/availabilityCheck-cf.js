...
async function checkAvailability($, oRequest, oResponse) {
    ...
    if (aDBClients.length === 0) {
        ...
        return oResponse.setBody(JSON.stringify({ message: oTenantDBClients.message }));
    }

    const aDisconnectedTenants = [];
    aDBClients.forEach(async function ({tenantId, tenantName, client}) {
        try {

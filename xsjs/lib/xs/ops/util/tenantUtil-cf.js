...
function getAllProvisionedTenantDBClients(){
    ...
    aTenants.forEach(function (tenant) {
        ...
        if (tenantInfo === undefined) {
            ...
            oResult.clients.push({tenantId:sTenantId, tenantName:tenant.sub_domain, subscriptionDate: null, client: null});
            return;
        }

        let oTenantDBClient= oTenantQuery.tenantQuery(tenantInfo.credentials);
        oResult.clients.push({tenantId:sTenantId, tenantName:tenant.sub_domain,subscriptionDate: dCreatedOn, client: oTenantDBClient});
    });

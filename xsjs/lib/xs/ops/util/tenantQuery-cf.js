...
TenantQuery.prototype.validateAndGetParameters = function (sqlStatement, ...options) {
    ...
    if (options.length <= 0) {
        ...
        if (!Array.isArray(value)) {
            if (parameterCount === 1) {
                return normalizeInput(options, this.treatDateAsUTC);;
            } else {
                return null;
            }
        } else {
...
TenantQuery.prototype.executeQuery = function (sqlStatement, ...options) {
...
TenantQuery.prototype.loadProcedure = function (sSchema, sSql) {
    let oConnection = this.getConnection();
    ...
    if (sSql) {
        ...
        sSql = sSchema;
        oProcedure = hdbext.sync.loadProcedure(oConnection, this.getSchema(), sSql);
    }
   

    return (...params) => {
        return async.waterfall.sync([
            (callback) => {
                oProcedure(normalizeInput(params, _this.treatDateAsUTC), callback);
            }]);
    }
}
...
TenantQuery.prototype.commit = function () {
    this.connection && this.connection.commit();
}

TenantQuery.prototype.rollback = function () {
    let oConnection = this.getConnection();
    return async.waterfall.sync([
        (callback) => {
            oConnection.rollback(callback);
    }]);
}

TenantQuery.prototype.close = function () {

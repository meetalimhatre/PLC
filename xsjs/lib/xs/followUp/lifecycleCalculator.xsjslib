 ...
 */
async function LifecycleVersionCalculator(iTaskId, oPersistency, oConnectionFactory, oTaskService, bOverwriteManualVersions, sOneTimeCostItemDescription) {

    /**	
	 * Executes the calculation. See class comment for further details.	
	 */
    this.calculate = () => {
        await $.trace.info(`Starting calculation of lifecycle versions for task id ${ iTaskId }.`);
        ...
        try {
            ...
            if (!oneTimeCostsAreValid) {
                ...
                $.trace.error(sLogMessage);
                await oTaskService.updateTask(oCalculationTask, {
            ...
            await (await oPersistency.getConnection()).commit();
            ...
            await $.trace.info(`Calculation of lifecycle versions for task id ${ iTaskId } (prokect id ${ sProjectId }) finished. Number of created lifecycle versions: ${ oCreatedLifecycleVersions.length }`);
        } catch (e) {

            await (await oPersistency.getConnection()).rollback();
            ...
            await setTaskToFailed(oCalculationTask, sExternalMessage);
        }
    };

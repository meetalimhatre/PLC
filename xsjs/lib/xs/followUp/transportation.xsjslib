...
function Transportation(iTaskId, oPersistency, oConnectionFactory, oTaskService) {

    this.importData = (oBodyData) => {
        $.trace.info(`Starting the transportation importData for task ${iTaskId}.`);
        ...
        if (helpers.isNullOrUndefined(oTransportationTask)) {
            ...
            setTaskToFailed(oTransportationTask, sLogMessage);
            return;
        }
        if (oTransportationTask.STATUS !== TaskStatus.INACTIVE) {
            const sLogMessage =
                `Cannot import data to transportation tool for task with id ${iTaskId}, since the task has the wrong status ` +
                `(expected: ${TaskStatus.INACTIVE}, actual: ${oTransportationTask.STATUS}).`;
            $.trace.error(sLogMessage);
            setTaskToFailed(oTransportationTask, sLogMessage);
            return;
        }
        if (oTransportationTask.TASK_TYPE !== TaskType.TRANSPORTATION_IMPORT) {
            const sLogMessage =
                `Cannot import data to transportation tool for task with id ${iTaskId}, since the task has the wrong type ` +
                `(expected: ${TaskType.TRANSPORTATION_IMPORT}, actual: ${oTransportationTask.TASK_TYPE}).`;
            $.trace.error(sLogMessage);
            setTaskToFailed(oTransportationTask, sLogMessage);
            return;
        }
        if (helpers.isNullOrUndefined(oTransportationTask.PARAMETERS)) {
            const sLogMessage = `Cannot import data to transportation tool for task with id ${iTaskId}, ` +
                `since the task defined has no parameters`;
            $.trace.error(sLogMessage);
            setTaskToFailed(oTransportationTask, sLogMessage);
            return;
        }
        ...
        try {
            ...
            const sLogMessage = `Cannot import data to transportation tool for task with id ${iTaskId}, ` +
                `since the task parameters cannot be parsed as JSON.`;

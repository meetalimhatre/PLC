...
//----------------------------------------------------------
$.import("testtools.db_info.excel_integration.lib","zip");
var ZIP=$.testtools.db_info.excel_integration.lib.zip; 

if ((typeof ZIP.JSZip === 'undefined' || !ZIP.JSZip) && typeof require === 'function') {
...
function xlsx(file) { 
	...
	var result, zip = new ZIP.JSZip(), zipTime, processTime, s, f, i, j, k, l, t, w, sharedStrings, styles, index, data, val, style, borders, border, borderIndex, fonts, font, fontIndex, fills, fillFormat, fillIndex,
	...
	function unescapeXML(s) { return typeof s === 'string' ? s.replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"').replace(/&#x27;/g, '\'') : ''; }
	 ...
	 if (typeof file === 'string') { // Load
			...
			if (s == zip.file('xl/styles.xml')) { // Get style info from "xl/styles.xml"
				//console.log(s.asText());
				
				//font information- fill in fonts array
				var sfont=s.asText().split('<font>'); var fonts_length=sfont.length;
				//console.log(fonts_length);
				while(--fonts_length){
					console.log('fonts_length'+ fonts_length);
					var fontname=getAttr(sfont[fonts_length],'name val');
					var fontsize=getAttr(sfont[fonts_length],'sz val');
					var fontcolor=null;
					if (sfont[fonts_length].indexOf('color rgb')>-1)
						fontcolor=getAttr(sfont[fonts_length],'color rgb');
					var fontbold=false;
					var fontitalic=false;
					if (sfont[fonts_length].indexOf('<b/>')>-1)
						fontbold=true;
					if (sfont[fonts_length].indexOf('<i/>')>-1)
						fontitalic=true;
					if ( fontcolor!=null)
						fonts.unshift({name:fontname, size:fontsize,color:fontcolor,bold:fontbold, italic:fontitalic});
					else
						fonts.unshift({name:fontname, size:fontsize,bold:fontbold, italic:fontitalic});
					

				}
				//console.log('fonts:'+fonts.length);	
				//for (var fi=0;fi<fonts.length;fi++)
					//console.log(JSON.stringify(fonts[fi]));
				
				//fill information- fill in fills array
				var sfill=s.asText().split('<fill>'); var fills_length=sfill.length;
				while(--fills_length){
					var fillcolor=null;
					if (sfill[fills_length].indexOf('fgColor rgb')>-1)
						fillcolor=getAttr(sfill[fills_length],'fgColor rgb');
					if ( fillcolor!=null)
						fills.unshift({color:fillcolor});
					else
						fills.unshift({});
				}
			...
			while (i--) {
				...
				while (--j) { // Don't process j === 0, because s[0] is the text before the first row element
					...
					if (s[j].indexOf('outlineLevel')>-1){
						var ilevel=parseInt(getAttr(s[j], 'outlineLevel'));
						   if (ilevel>=0){

							   var blevelfound=false;
							   
							   for (var m=0; m<outline.length;m++){
								   
								   if (typeof outline[m].outlineLevel_id!='undefined' && outline[m].outlineLevel_id==ilevel){
									   outline[m].rows.push(+getAttr(s[j], 'r') - 1);
									   blevelfound=true;
									   break;
								   }				   
							   }
							   if (!blevelfound){	
								   outline.push({outlineLevel_id:ilevel,rows:[+getAttr(s[j], 'r') - 1]});
							   }
						   }
					   }
					...
					while (--k) { // Don't process l === 0, because k[0] is the text before the first c (cell) element
						...
						var current_cell=row[alphaNum(getAttr(cell, 'r').match(/[a-zA-Z]*/g)[0])];
						
						// Add font information
						var fontName=fonts[styles[+getAttr(cell, 's')].font].name;
						var fontSize=fonts[styles[+getAttr(cell, 's')].font].size;
						
						current_cell={value: val, formatCode: f.formatCode, fontName:fontName, fontSize:fontSize};
						if(fonts[styles[+getAttr(cell, 's')].font].bold)
							current_cell.bold=true;
						if (fonts[styles[+getAttr(cell, 's')].font].italic)
							current_cell.italic=true;
						if (typeof fonts[styles[+getAttr(cell, 's')].font].color!='undefined' ){
							var color_info=fonts[styles[+getAttr(cell, 's')].font].color;
							current_cell.fontColor=color_info.substr(2);
						}	
						// Add fill information
						if (typeof fills[styles[+getAttr(cell, 's')].fill].color!='undefined'){
							var color_info=fills[styles[+getAttr(cell, 's')].fill].color;

							if (color_info.indexOf('ffff')>-1||color_info.indexOf('FFFF')>-1)
								current_cell.interiorColor=color_info.substr(2);
							else
								current_cell.interiorColor=color_info;
							
						}
						// Add formula information
						if (cell.indexOf('<f>')>-1)
							current_cell.formula=cell.substring(cell.indexOf('<f>')+3 , cell.indexOf('</f>'));
						
						
						//row[alphaNum(getAttr(cell, 'r').match(/[a-zA-Z]*/g)[0])] = { value: val, formatCode: f.formatCode};
						row[alphaNum(getAttr(cell, 'r').match(/[a-zA-Z]*/g)[0])]=current_cell;
						//console.log(JSON.stringify(current_cell));
					}
				}
				result.worksheets[i].outlineLevel=outline;
		...
		while (w--) { 
			...
			//added - store information about groupped rows of the current worksheet
			var outlineLevel=worksheet.outlineLevel;
			s = '';
			columns = [];
			merges = [];
			i = -1; l = data.length;
			...
			while (++i < l) {
				...
				//add some code the previous line was not commented
				var groupped=false;
				var collapsed=false;
				var outlineLevel_id=1;
				//added - Check if the current row is groupped or not
				if (outlineLevel != null){	//if defined at all
					for(var m=0; m<outlineLevel.length;m++){
						if (outlineLevel[m].rows.indexOf(i+1)>=0){
							groupped=true;
							outlineLevel_id=outlineLevel[m].outlineLevel_id;
							if(outlineLevel[m].rows.indexOf(i+1)==0){
								collapsed=true;
				...
				//added - if the current row is groupped then add the attribute outlineLevel at the row tag
				if (groupped){
					/*if (collapsed){
						s+='<row r="' + (i+1 ) + '" collapsed="1" x14ac:dyDescent="0.25">';
						////console.log('<row r="' + (i+1 ) + '" collapsed="1" x14ac:dyDescent="0.25">');
					}
					else {*/
						s += '<row r="' + (i +1) + '" outlineLevel="'+outlineLevel_id+'" x14ac:dyDescent="0.25">';
						////console.log('<row r="' + (i+1 ) + '" outlineLevel="'+outlineLevel_id+'" x14ac:dyDescent="0.25">');
					//}
				...
				while (++j < k) {
					...
					//////console.log("point1: "+cell.hasOwnProperty('value'));
					...
					style = { // supported styles: borders, hAlign, formatCode and font style
						...
						fontColor:cell.fontColor,//added
						formatCode: cell.formatCode || 'General',
						interiorColor:cell.interiorColor //added
					};

					colWidth = 0;
					//check the type of the value of the cell
					...
					if (typeof cell.colSpan!='undefined' && cell.colSpan > 1) {
						...
						k += cell.colSpan-1;
					} else if (typeof cell.rSpan!='undefined' && cell.rowSpan > 1) {
					...
					if ((typeof cell.rowSpan!='undefined' && cell.rowSpan > 1 )|| (typeof cell.colSpan!='undefined' && cell.colSpan > 1)) {
			...
			s = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" mc:Ignorable="x14ac" xmlns:x14ac="http://schemas.microsoft.com/office/spreadsheetml/2009/9/ac">'
				+'<sheetPr> <outlinePr summaryBelow="0" />  </sheetPr>' //added
				...
				//added 1 line code here the above line was not commented
				+ ' workbookViewId="0"/></sheetViews><sheetFormatPr defaultRowHeight="15" outlineLevelRow="1" x14ac:dyDescent="0.25"/>'
				+ cols
				+ '<sheetData>'
				+ s 
				+ '</sheetData>';
		...
		while (--i) { 
			...
			if (style.bold || style.italic || style.fontSize || style.fontName) {
				...
				if(style.fontColor) //added - check if user has defined any color for font
					font.push('<color rgb="'+style.fontColor+'"/>'); // added - if yes then add the attribute rgb to the color tag
				else					
					font.push('<color theme="1"/>'); // else use the default color (black)
			...
			// start added - fills declaration :add a new declaration and refer to it in style
			fillIndex=0;
			
			if (style.interiorColor){
				fillFormat=['<fill>'];
				fillFormat.push('<patternFill patternType="solid">');
				fillFormat.push('<fgColor rgb="'+style.interiorColor+'"/>');
				//fillFormat.push('<bgColor indexed="64"/>');
				fillFormat.push('</patternFill>');
				fillFormat.push('</fill>');
				fillFormat=fillFormat.join('');
				fillIndex=fills.indexOf(fillFormat);
				if(fillIndex<0){
					fillIndex = fills.push(fillFormat) - 1;
			...
			// declares style, and refer to optionnal formatCode, font and borders and fills
			styles[i] = ['<xf xfId="0" fillId="',fillIndex, // fill index is now dynamically declared 
				'" borderId="', borderIndex, 
				'" fontId="',fontIndex,
				'" numFmtId="',style.formatCode,'" ',
		...
		xl.file('styles.xml', '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" mc:Ignorable="x14ac" xmlns:x14ac="http://schemas.microsoft.com/office/spreadsheetml/2009/9/ac">'
			...
			//--before-- + '<scheme val="minor"/></font>' + fonts.join('') + '</fonts><fills count="2"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill></fills>'
			+ '<scheme val="minor"/></font>' + fonts.join('') + '</fonts><fills count="'+fills.length+'"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill>' + fills.join('') + '</fills>' //added - fills are dynamicall declared 
		...
		/*////console.log('styles.xml', '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" mc:Ignorable="x14ac" xmlns:x14ac="http://schemas.microsoft.com/office/spreadsheetml/2009/9/ac">'
				+ t + '<fonts count="'+ fonts.length + '" x14ac:knownFonts="1"><font><sz val="' + defaultFontSize + '"/><color theme="1"/><name val="' + defaultFontName + '"/><family val="2"/>'
				+ '<scheme val="minor"/></font>' + fonts.join('') + '</fonts><fills count="'+fills.length+'"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill>' + fills.join('') + '</fills>'
				+ '<borders count="' + borders.length + '"><border><left/><right/><top/><bottom/><diagonal/></border>'
				+ borders.join('') + '</borders><cellStyleXfs count="1">'
				+ '<xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="' + styles.length + '"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/>'
				+ styles.join('') + '</cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles><dxfs count="0"/>'
